...
#ifndef INCLUDE_VOX_INTERSECTION_QUERY_ENGINE2_H_
#define INCLUDE_VOX_INTERSECTION_QUERY_ENGINE2_H_
...
public:
    ...
    virtual bool intersects(const BoundingBox2D &box,
                            const BoxIntersectionTestFunc2<T> &testFunc) const = 0;
    
    //! Returns true if given \p ray intersects with any of the stored items.
    virtual bool intersects(const Ray2D &ray,
                            const RayIntersectionTestFunc2<T> &testFunc) const = 0;
    
    //! Invokes \p visitorFunc for every intersecting items.
    virtual void forEachIntersectingItem(const BoundingBox2D &box, const BoxIntersectionTestFunc2<T> &testFunc,
                                         const IntersectionVisitorFunc2<T> &visitorFunc) const = 0;
    
    //! Invokes \p visitorFunc for every intersecting items.
    virtual void forEachIntersectingItem(const Ray2D &ray, const RayIntersectionTestFunc2<T> &testFunc,
                                         const IntersectionVisitorFunc2<T> &visitorFunc) const = 0;
    
    //! Returns the closest intersection for given \p ray.
    virtual ClosestIntersectionQueryResult2<T>
    closestIntersection(const Ray2D &ray, const GetRayIntersectionFunc2<T> &testFunc) const = 0;
};

}  // namespace vox

#endif  // INCLUDE_VOX_INTERSECTION_QUERY_ENGINE2_H_
