...
static bool isectSegAABB(
    ...
    float d[3];
    dtVsub(d, sq, sp);
    tmin = 0;        // set to -FLT_MAX to get first hit online
...
CrowdToolState::~CrowdToolState() { dtFreeObstacleAvoidanceDebugData(m_vod); }

void CrowdToolState::init(NavigationManager* sample) {
    if (m_sample != sample) {
        m_sample = sample;
    }
    ...
    if (nav && crowd && (m_nav != nav || m_crowd != crowd)) {
        ...
        crowd->getEditableFilter(0)->setExcludeFlags(POLY_FLAGS_DISABLED);
...
void CrowdToolState::handleRender() {
    ...
    if (m_toolParams.m_showNodes && crowd->getPathQueue()) {
        ...
        if (navquery) debug::debugDrawNavMeshNodes(&dd, *navquery);
    }
    ...
    if (m_toolParams.m_showPath) {
        for (int i = 0; i < crowd->getAgentCount(); i++) {
            if (!m_toolParams.m_showDetailAll && i != m_agentDebug.idx) continue;
            const dtCrowdAgent* ag = crowd->getAgent(i);
            if (!ag->active) continue;
            const dtPolyRef* path = ag->corridor.getPath();
            const int npath = ag->corridor.getPathCount();
            for (int j = 0; j < npath; ++j)
                debug::debugDrawNavMeshPoly(&dd, *nav, path[j], debug::int2RGBA(255, 255, 255, 24));
        }
    }

    if (m_targetRef)
        debug::debugDrawCross(&dd, m_targetPos[0], m_targetPos[1] + 0.1f, m_targetPos[2], rad,
                              debug::int2RGBA(255, 255, 255, 192), 2.0f);
    ...
    if (m_toolParams.m_showGrid) {
        ...
        for (int y = bounds[1]; y <= bounds[3]; ++y) {
            for (int x = bounds[0]; x <= bounds[2]; ++x) {
                ...
                if (!count) continue;
                unsigned int col = debug::int2RGBA(128, 0, 0, dtMin(count * 40, 255));
    ...
    for (int i = 0; i < crowd->getAgentCount(); ++i) {
        ...
        for (int j = 0; j < AGENT_MAX_TRAIL - 1; ++j) {
            ...
            dd.vertex(prev[0], prev[1] + 0.1f, prev[2], debug::int2RGBA(0, 0, 0, (int)(128 * preva)));
            dd.vertex(v[0], v[1] + 0.1f, v[2], debug::int2RGBA(0, 0, 0, (int)(128 * a)));
            preva = a;
            dtVcopy(prev, v);
        }
        dd.end();
    ...
    for (int i = 0; i < crowd->getAgentCount(); i++) {
        ...
        if (m_toolParams.m_showCorners) {
            if (ag->ncorners) {
                ...
                for (int j = 0; j < ag->ncorners; ++j) {
                    ...
                    dd.vertex(va[0], va[1] + radius, va[2], debug::int2RGBA(128, 0, 0, 192));
                    dd.vertex(vb[0], vb[1] + radius, vb[2], debug::int2RGBA(128, 0, 0, 192));
                }
                if (ag->ncorners && ag->cornerFlags[ag->ncorners - 1] & DT_STRAIGHTPATH_OFFMESH_CONNECTION) {
                    ...
                    dd.vertex(v[0], v[1], v[2], debug::int2RGBA(192, 0, 0, 192));
                    dd.vertex(v[0], v[1] + radius * 2, v[2], debug::int2RGBA(192, 0, 0, 192));
                }
                ...
                if (m_toolParams.m_anticipateTurns) {
                    /*					float dvel[3], pos[3];
                     ...
                     dd.vertex(ag->pos[0],y,ag->pos[2], debug::int2RGBA(255,0,0,192));
                     dd.vertex(pos[0],y,pos[2], debug::int2RGBA(255,0,0,192));

                     dd.vertex(pos[0],y,pos[2], debug::int2RGBA(255,0,0,192));
                     dd.vertex(tgt[0],y,tgt[2], debug::int2RGBA(255,0,0,192));
        ...
        if (m_toolParams.m_showCollisionSegments) {
            ...
            debug::debugDrawCross(&dd, center[0], center[1] + radius, center[2], 0.2f,
                                  debug::int2RGBA(192, 0, 128, 255), 2.0f);
            debug::debugDrawCircle(&dd, center[0], center[1] + radius, center[2], ag->params.collisionQueryRange,
                                   debug::int2RGBA(192, 0, 128, 128), 2.0f);

            dd.begin(wgpu::PrimitiveTopology::LineList, 3.0f);
            for (int j = 0; j < ag->boundary.getSegmentCount(); ++j) {
                const float* s = ag->boundary.getSegment(j);
                unsigned int col = debug::int2RGBA(192, 0, 128, 192);
                if (dtTriArea2D(pos, s, s + 3) < 0.0f) col = debug::darkenCol(col);

                debug::debugAppendArrow(&dd, s[0], s[1] + 0.2f, s[2], s[3], s[4] + 0.2f, s[5], 0.0f, 0.3f, col);
            }
            dd.end();
        }

        if (m_toolParams.m_showNeis) {
            debug::debugDrawCircle(&dd, pos[0], pos[1] + radius, pos[2], ag->params.collisionQueryRange,
                                   debug::int2RGBA(0, 192, 128, 128), 2.0f);
            ...
            for (int j = 0; j < ag->nneis; ++j) {
                ...
                if (nei) {
                    dd.vertex(pos[0], pos[1] + radius, pos[2], debug::int2RGBA(0, 192, 128, 128));
                    dd.vertex(nei->npos[0], nei->npos[1] + radius, nei->npos[2], debug::int2RGBA(0, 192, 128, 128));
                }
            }
            dd.end();
        }
    ...
    for (int i = 0; i < crowd->getAgentCount(); ++i) {
        ...
        unsigned int col = debug::int2RGBA(0, 0, 0, 32);
        if (m_agentDebug.idx == i) col = debug::int2RGBA(255, 0, 0, 128);

        debug::debugDrawCircle(&dd, pos[0], pos[1], pos[2], radius, col, 2.0f);
    }
    ...
    for (int i = 0; i < crowd->getAgentCount(); ++i) {
        ...
        unsigned int col = debug::int2RGBA(220, 220, 220, 128);
        if (ag->targetState == DT_CROWDAGENT_TARGET_REQUESTING ||
            ag->targetState == DT_CROWDAGENT_TARGET_WAITING_FOR_QUEUE)
            col = debug::lerpCol(col, debug::int2RGBA(128, 0, 255, 128), 32);
        else if (ag->targetState == DT_CROWDAGENT_TARGET_WAITING_FOR_PATH)
            col = debug::lerpCol(col, debug::int2RGBA(128, 0, 255, 128), 128);
        else if (ag->targetState == DT_CROWDAGENT_TARGET_FAILED)
            col = debug::int2RGBA(255, 32, 16, 128);
        else if (ag->targetState == DT_CROWDAGENT_TARGET_VELOCITY)
            col = debug::lerpCol(col, debug::int2RGBA(64, 255, 0, 128), 128);

        debug::debugDrawCylinder(&dd, pos[0] - radius, pos[1] + radius * 0.1f, pos[2] - radius, pos[0] + radius,
    ...
    if (m_toolParams.m_showVO) {
        for (int i = 0; i < crowd->getAgentCount(); i++) {
            ...
            debug::debugDrawCircle(&dd, dx, dy, dz, ag->params.maxSpeed, debug::int2RGBA(255, 255, 255, 64), 2.0f);
            ...
            for (int j = 0; j < vod->getSampleCount(); ++j) {
                ...
                unsigned int col = debug::lerpCol(debug::int2RGBA(255, 255, 255, 220), debug::int2RGBA(128, 96, 0, 220),
                                                  (int)(pen * 255));
                col = debug::lerpCol(col, debug::int2RGBA(128, 0, 0, 220), (int)(pen2 * 128));
    ...
    for (int i = 0; i < crowd->getAgentCount(); ++i) {
        ...
        unsigned int col = debug::int2RGBA(220, 220, 220, 192);
        if (ag->targetState == DT_CROWDAGENT_TARGET_REQUESTING ||
            ag->targetState == DT_CROWDAGENT_TARGET_WAITING_FOR_QUEUE)
            col = debug::lerpCol(col, debug::int2RGBA(128, 0, 255, 192), 32);
        else if (ag->targetState == DT_CROWDAGENT_TARGET_WAITING_FOR_PATH)
            col = debug::lerpCol(col, debug::int2RGBA(128, 0, 255, 192), 128);
        else if (ag->targetState == DT_CROWDAGENT_TARGET_FAILED)
            col = debug::int2RGBA(255, 32, 16, 192);
        else if (ag->targetState == DT_CROWDAGENT_TARGET_VELOCITY)
            col = debug::lerpCol(col, debug::int2RGBA(64, 255, 0, 192), 128);

        debug::debugDrawCircle(&dd, pos[0], pos[1] + height, pos[2], radius, col, 2.0f);

        debug::debugDrawArrow(&dd, pos[0], pos[1] + height, pos[2], pos[0] + dvel[0], pos[1] + height + dvel[1],
                              pos[2] + dvel[2], 0.0f, 0.4f, debug::int2RGBA(0, 192, 255, 192),
                              (m_agentDebug.idx == i) ? 2.0f : 1.0f);

        debug::debugDrawArrow(&dd, pos[0], pos[1] + height, pos[2], pos[0] + vel[0], pos[1] + height + vel[1],
                              pos[2] + vel[2], 0.0f, 0.4f, debug::int2RGBA(0, 0, 0, 160), 2.0f);
    }
...
void CrowdToolState::setMoveTarget(const float* p, bool adjust) {
    ...
    // Find the nearest point on navmesh and set move request to that location.
