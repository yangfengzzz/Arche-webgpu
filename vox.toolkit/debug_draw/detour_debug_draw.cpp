...
void drawPolyBoundaries(DebugDraw* dd, const dtMeshTile* tile, const unsigned int col, const float linew, bool inner) {
    static const float thr = 0.01f * 0.01f;

    dd->begin(wgpu::PrimitiveTopology::LineList, linew);
        ...
        for (int j = 0, nj = (int)p->vertCount; j < nj; ++j) {
            ...
            for (int k = 0; k < pd->triCount; ++k) {
                ...
                for (int m = 0, n = 2; m < 3; n = m++) {
                    if ((dtGetDetailTriEdgeFlags(t[3], n) & DT_DETAIL_EDGE_BOUNDARY) == 0) continue;
...
void drawMeshTile(DebugDraw* dd,
    ...
    if (flags & DRAW_NAVMESH_OFF_MESH_CONS) {
        ...
        for (int i = 0; i < tile->header->polyCount; ++i) {
            ...
            bool endSet = false;
            for (unsigned int k = p->firstLink; k != DT_NULL_LINK; k = tile->links[k].next) {
                if (tile->links[k].edge == 0) startSet = true;
                if (tile->links[k].edge == 1) endSet = true;
            }
...
void drawMeshTileBVTree(DebugDraw* dd, const dtMeshTile* tile) {
    // Draw BV nodes.
...
void drawMeshTilePortal(DebugDraw* dd, const dtMeshTile* tile) {
    // Draw portals
...
void debugDrawNavMeshPolysWithFlags(DebugDraw* dd,
    ...
    for (int i = 0; i < mesh.getMaxTiles(); ++i) {
        ...
        for (int j = 0; j < tile->header->polyCount; ++j) {
            ...
            debugDrawNavMeshPoly(dd, mesh, base | (dtPolyRef)j, col);
        }
    }
}

void debugDrawNavMeshPoly(DebugDraw* dd, const dtNavMesh& mesh, dtPolyRef ref, const unsigned int col) {
    if (!dd) return;

    const dtMeshTile* tile = nullptr;
    const dtPoly* poly = nullptr;
    if (dtStatusFailed(mesh.getTileAndPolyByRef(ref, &tile, &poly))) return;
...
void debugDrawTileCacheContours(
        DebugDraw* dd, const struct dtTileCacheContourSet& lcset, const float* orig, const float cs, const float ch) {
    if (!dd) return;
...
void debugDrawTileCachePolyMesh(
        DebugDraw* dd, const struct dtTileCachePolyMesh& lmesh, const float* orig, const float cs, const float ch) {
    if (!dd) return;
    ...
    for (int i = 0; i < lmesh.npolys; ++i) {
        ...
        for (int j = 2; j < nvp; ++j) {
            ...
            for (unsigned short k : vi) {
                const unsigned short* v = &lmesh.verts[k * 3];
