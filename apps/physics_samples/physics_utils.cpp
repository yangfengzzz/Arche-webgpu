...
ShapeRefC PhysicsUtils::createTorusMesh(float inTorusRadius,
                                        float inTubeRadius,
                                        uint inTorusSegments,
                                        uint inTubeSegments) {
    ...
    for (uint torus_segment = 0; torus_segment < inTorusSegments; ++torus_segment) {
        ...
        for (uint tube_segment = 0; tube_segment < inTubeSegments; ++tube_segment) {
            ...
            Vec3 pos =
                    rotation * Vec3(inTorusRadius + inTubeRadius * Sin(tube_angle), inTubeRadius * Cos(tube_angle), 0);
            Float3 v{};
            pos.StoreFloat3(&v);
            ...
            mesh.mIndexedTriangles.emplace_back(start_idx, (start_idx + 1) % cNumVertices,
                                                (start_idx + inTubeSegments) % cNumVertices);
            mesh.mIndexedTriangles.emplace_back((start_idx + 1) % cNumVertices,
                                                (start_idx + inTubeSegments + 1) % cNumVertices,
                                                (start_idx + inTubeSegments) % cNumVertices);
        }
    }
