...
bool PhysicsScaledCapsuleShapeApp::prepare(Platform &platform) {
...
void PhysicsScaledCapsuleShapeApp::loadScene() {
    auto scene = _sceneManager->currentScene();
    ...
    JPH::BodyInterface &body_interface = PhysicsManager::GetSingleton().getBodyInterface();
    {
        ...
        // Original shape
        Body &body1 =
                *body_interface.CreateBody(BodyCreationSettings(capsule_shape, Vec3(-20, 10, 0), Quat::sIdentity(),
        ...
        Body &body2 = *body_interface.CreateBody(
                BodyCreationSettings(new ScaledShape(capsule_shape, Vec3::sReplicate(0.25f)), Vec3(-10, 10, 0),
        ...
        Body &body3 = *body_interface.CreateBody(
                BodyCreationSettings(new ScaledShape(capsule_shape, Vec3::sReplicate(2.0f)), Vec3(0, 10, 0),
                                     Quat::sIdentity(), EMotionType::Dynamic, PhysicsManager::Layers::MOVING));
        body_interface.AddBody(body3.GetID(), EActivation::Activate);

        // Flipped in 2 axis
        Body &body4 = *body_interface.CreateBody(
                BodyCreationSettings(new ScaledShape(capsule_shape, Vec3(-1.5f, -1.5f, 1.5f)), Vec3(10, 10, 0),
                                     Quat::sIdentity(), EMotionType::Dynamic, PhysicsManager::Layers::MOVING));
        body_interface.AddBody(body4.GetID(), EActivation::Activate);

        // Inside out
        Body &body5 = *body_interface.CreateBody(
                BodyCreationSettings(new ScaledShape(capsule_shape, Vec3(-0.75f, 0.75f, 0.75f)), Vec3(20, 10, 0),
                                     Quat::sIdentity(), EMotionType::Dynamic, PhysicsManager::Layers::MOVING));
