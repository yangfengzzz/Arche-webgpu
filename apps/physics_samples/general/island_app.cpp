...
bool PhysicsIslandApp::prepare(Platform& platform) {
...
void PhysicsIslandApp::loadScene() {
    auto scene = _sceneManager->currentScene();
        ...
        for (int i = 0; i < 10; ++i)
            for (int j = i / 2; j < 10 - (i + 1) / 2; ++j)
                for (int k = 0; k < 8; ++k) {
                    ...
                    Body& wall = *body_interface.CreateBody(BodyCreationSettings(box_shape, position, Quat::sIdentity(),
                                                                                 EMotionType::Dynamic,
