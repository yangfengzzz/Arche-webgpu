...
bool PhysicsFrictionApp::prepare(Platform& platform) {
...
void PhysicsFrictionApp::loadScene() {
    auto scene = _sceneManager->currentScene();
    ...
    JPH::BodyInterface& body_interface = PhysicsManager::GetSingleton().getBodyInterface();
    {
        ...
        Body& floor = *body_interface.CreateBody(
                BodyCreationSettings(new BoxShape(Vec3(100.0f, 1.0f, 100.0f), 0.0f), Vec3::sZero(),
        ...
        for (int i = 0; i <= 10; ++i) {
            Body& body = *body_interface.CreateBody(BodyCreationSettings(
                    box, Vec3(-50.0f + i * 10.0f, 55.0f, -50.0f), Quat::sRotation(Vec3::sAxisX(), 0.25f * JPH_PI),
        ...
        for (int i = 0; i <= 10; ++i) {
            Body& body = *body_interface.CreateBody(BodyCreationSettings(
                    sphere, Vec3(-50.0f + i * 10.0f, 47.0f, -40.0f), Quat::sRotation(Vec3::sAxisX(), 0.25f * JPH_PI),
                    EMotionType::Dynamic, PhysicsManager::Layers::MOVING));
            body.SetFriction(0.1f * i);
            body_interface.AddBody(body.GetID(), EActivation::Activate);
        }
