...
bool PhysicsActiveEdgesApp::prepare(Platform& platform) {
...
void PhysicsActiveEdgesApp::loadScene() {
    auto scene = _sceneManager->currentScene();
    ...
    JPH::BodyInterface& body_interface = PhysicsManager::GetSingleton().getBodyInterface();
    {
        ...
        BodyCreationSettings box_settings(box_shape, Vec3::sZero(), Quat::sIdentity(), EMotionType::Dynamic,
        ...
        for (int angle = -90; angle <= 90; angle++) {
            ...
            for (int total_angle = 0, cur_segment = 0; abs(total_angle) <= 90 && cur_segment < 90;
                 total_angle += angle, ++cur_segment) {
            ...
            Body& body = *body_interface.CreateBody(box_settings);
            body_interface.AddBody(body.GetID(), EActivation::Activate);

            // For convex segments give the block a push
            if (angle >= 0) body.SetLinearVelocity(Vec3(0, 0, 2.0f));
        }

        // Mesh
        BodyCreationSettings mesh_settings(new MeshShapeSettings(triangles), Vec3::sZero(), Quat::sIdentity(),
                                           EMotionType::Static, PhysicsManager::Layers::NON_MOVING);
        mesh_settings.mFriction = 0.0f;
        Body& mesh = *body_interface.CreateBody(mesh_settings);
        body_interface.AddBody(mesh.GetID(), EActivation::DontActivate);
    }
    scene->play();
}
