...
#define EXPECT_EQ_LOG_LOGV(_expression, _eq, _re) \
    EXPECT_EQ_LOG(_expression, _eq, std::clog, ozz::log::kVerbose <= ozz::log::GetLevel() ? _re : nullptr)

// Specialises EXPECT_EQ_LOG* for standard clog output type.
#define EXPECT_EQ_LOG_LOG(_expression, _eq, _re) \
    EXPECT_EQ_LOG(_expression, _eq, std::clog, ozz::log::kStandard <= ozz::log::GetLevel() ? _re : nullptr)

// Specialises EXPECT_EQ_LOG* for standard cout output type.
#define EXPECT_EQ_LOG_OUT(_expression, _eq, _re) \
    EXPECT_EQ_LOG(_expression, _eq, std::cout, ozz::log::kStandard <= ozz::log::GetLevel() ? _re : nullptr)

// Specialises EXPECT_EQ_LOG* for standard cerr output type.
#define EXPECT_EQ_LOG_ERR(_expression, _eq, _re) \
    EXPECT_EQ_LOG(_expression, _eq, std::cerr, ozz::log::kStandard <= ozz::log::GetLevel() ? _re : nullptr)
...
#define EXPECT_LOG_LOGV(_expression, _re) \
    EXPECT_LOG(_expression, std::clog, ozz::log::kVerbose <= ozz::log::GetLevel() ? _re : nullptr)

// Specialises EXPECT_LOG* for standard clog output type.
#define EXPECT_LOG_LOG(_expression, _re) \
    EXPECT_LOG(_expression, std::clog, ozz::log::kStandard <= ozz::log::GetLevel() ? _re : nullptr)

// Specialises EXPECT_LOG* for standard cout output type.
#define EXPECT_LOG_OUT(_expression, _re) \
    EXPECT_LOG(_expression, std::cout, ozz::log::kStandard <= ozz::log::GetLevel() ? _re : nullptr)

// Specialises EXPECT_LOG* for standard cerr output type.
#define EXPECT_LOG_ERR(_expression, _re) \
    EXPECT_LOG(_expression, std::cerr, ozz::log::kStandard <= ozz::log::GetLevel() ? _re : nullptr)
