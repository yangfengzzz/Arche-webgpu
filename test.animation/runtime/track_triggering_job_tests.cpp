...
TEST(Iterator, TrackEdgeTriggerJob) {
    ...
    {  // ->
        EXPECT_TRUE(iterator->rising);
        bool rising;
        EXPECT_ASSERTION(rising = job.end()->rising, "Can't dereference end iterator");
        (void)rising;
    }
...
size_t CountEdges(TrackTriggeringJob::Iterator _begin, const TrackTriggeringJob::Iterator& _end) {
    size_t count = 0;
...
void TestEdgesExpectationBackward(TrackTriggeringJob::Iterator _fw_iterator, const TrackTriggeringJob& _fw_job) {
...
void TestEdgesExpectation(const vox::animation::offline::RawFloatTrack& _raw_track,
                          float _threshold,
                          const TrackTriggeringJob::Edge* _expected,
                          size_t _size) {
    assert(_size >= 2);
    ...
    {  // Forward, first edge to last, last can be included.
        ...
        ASSERT_EQ(CountEdges(iterator, job.end()), last_included ? _size : _size - 1);
    ...
    {  // Forward, 0 to before last edge + 1
        ...
        ASSERT_EQ(CountEdges(iterator, job.end()), last_included ? _size * 2 : _size * 2 - 1);
        ...
        for (size_t i = _size; i < (last_included ? _size * 2 : _size * 2 - 1); ++i, ++it) {
    ...
    {  // Forward [-21.3999996, -21.0000019]
        ...
        for (size_t i = 0; i < _size; ++i) {
            if (_expected[i].ratio - 22.f >= job.from && _expected[i].ratio - 22.f < job.to) {
                found++;
            }
        }
    ...
    {  // Randomized tests forward/backward coherency
        ...
        for (size_t i = 0; i < kMaxIterations; ++i) {
            job.from = kMaxRange * (1.f - 2.f * static_cast<float>(rand()) / static_cast<float>(RAND_MAX));
            job.to = kMaxRange * (1.f - 2.f * static_cast<float>(rand()) / static_cast<float>(RAND_MAX));
            TrackTriggeringJob::Iterator iterator;
            job.iterator = &iterator;
            ASSERT_TRUE(job.Run());
            TestEdgesExpectationBackward(iterator, job);
    ...
    {  // Randomized tests rising/falling coherency
        ...
        for (size_t i = 0; i < kMaxIterations; ++i) {
            ...
            float new_time =
                    ratio + kMaxRange * (1.f - 2.f * static_cast<float>(rand()) / static_cast<float>(RAND_MAX));
            ...
            switch (rand() % 20) {
                ...
                case 1: {
                    ...
                    new_time = nexttowardf(_expected[rand() % _size].ratio + floorf(new_time), 1e15f);
                    break;
                }
                case 2: {
            ...
            // ratio is going.
            for (const TrackTriggeringJob::Iterator end = job.end(); iterator != end; ++iterator) {
...
inline void TestEdgesExpectation(const vox::animation::offline::RawFloatTrack& _raw_track,
                                 float _threshold,
...
TEST(SquareStep, TrackEdgeTriggerJob) {
    ...
    {  // Rising edge at t = 0.6, no falling edge at end
        ...
        const vox::animation::offline::RawFloatTrack::Keyframe key0 = {RawTrackInterpolation::kStep, 0.f, 0.f};
        raw_track.keyframes.push_back(key0);
        const vox::animation::offline::RawFloatTrack::Keyframe key1 = {RawTrackInterpolation::kStep, .6f, 2.f};
        raw_track.keyframes.push_back(key1);
    ...
    {  // Falling edge at t = 0.5
        ...
        // Keyframe values oscillate in range [0,2].
        const vox::animation::offline::RawFloatTrack::Keyframe key0 = {RawTrackInterpolation::kStep, 0.f, 2.f};
        raw_track.keyframes.push_back(key0);
        const vox::animation::offline::RawFloatTrack::Keyframe key1 = {RawTrackInterpolation::kStep, .5f, 0.f};
        raw_track.keyframes.push_back(key1);
    ...
    {  // Negative values
        ...
        const vox::animation::offline::RawFloatTrack::Keyframe key0 = {RawTrackInterpolation::kStep, 0.f, -1.f};
        raw_track.keyframes.push_back(key0);
        const vox::animation::offline::RawFloatTrack::Keyframe key1 = {RawTrackInterpolation::kStep, .5f, 1.f};
        raw_track.keyframes.push_back(key1);
        const vox::animation::offline::RawFloatTrack::Keyframe key2 = {RawTrackInterpolation::kStep, 1.f, -1.f};
        raw_track.keyframes.push_back(key2);
    ...
    {  // More edges
        ...
        const vox::animation::offline::RawFloatTrack::Keyframe key0 = {RawTrackInterpolation::kStep, .0f, 0.f};
        raw_track.keyframes.push_back(key0);
        const vox::animation::offline::RawFloatTrack::Keyframe key1 = {RawTrackInterpolation::kStep, .2f, 2.f};
        raw_track.keyframes.push_back(key1);
        const vox::animation::offline::RawFloatTrack::Keyframe key2 = {RawTrackInterpolation::kStep, .3f, 0.f};
        raw_track.keyframes.push_back(key2);
        const vox::animation::offline::RawFloatTrack::Keyframe key3 = {RawTrackInterpolation::kStep, .4f, 1.f};
        raw_track.keyframes.push_back(key3);
        const vox::animation::offline::RawFloatTrack::Keyframe key4 = {RawTrackInterpolation::kStep, .5f, 0.f};
        raw_track.keyframes.push_back(key4);

        const TrackTriggeringJob::Edge expected0[] = {{.2f, true}, {.3f, false}, {.4f, true}, {.5f, false}};
...
TEST(Linear, TrackEdgeTriggerJob) {
    {  // Higher point at t = 0.5
        ...
        raw_track.keyframes.push_back(key0);
        const vox::animation::offline::RawFloatTrack::Keyframe key1 = {RawTrackInterpolation::kLinear, .5f, 2.f};
        raw_track.keyframes.push_back(key1);
        const vox::animation::offline::RawFloatTrack::Keyframe key2 = {RawTrackInterpolation::kLinear, 1.f, 0.f};
        raw_track.keyframes.push_back(key2);

        const TrackTriggeringJob::Edge expected0[] = {{.25f, true}, {.75f, false}};
        TestEdgesExpectation(raw_track, 1.f, expected0);

        const TrackTriggeringJob::Edge expected1[] = {{.125f, true}, {.875f, false}};
        TestEdgesExpectation(raw_track, .5f, expected1);

        const TrackTriggeringJob::Edge expected2[] = {{.375f, true}, {.625f, false}};
    ...
    {  // Negative values
        ...
        // Keyframe values oscillate in range [-1,1].
        const vox::animation::offline::RawFloatTrack::Keyframe key0 = {RawTrackInterpolation::kLinear, 0.f, -1.f};
        raw_track.keyframes.push_back(key0);
        const vox::animation::offline::RawFloatTrack::Keyframe key1 = {RawTrackInterpolation::kLinear, .5f, 1.f};
        raw_track.keyframes.push_back(key1);
...
TEST(LinearThreshold, TrackEdgeTriggerJob) {
    ...
    const vox::animation::offline::RawFloatTrack::Keyframe key1 = {RawTrackInterpolation::kLinear, .5f, 1.f};
    raw_track.keyframes.push_back(key1);
    const vox::animation::offline::RawFloatTrack::Keyframe key2 = {RawTrackInterpolation::kLinear, 1.f, -1.f};
    raw_track.keyframes.push_back(key2);
