...
static const size_t kParserPlaceholerSize = (sizeof(Parser) + sizeof(void*) - 1) / sizeof(void*);
...
ParseResult ParseCommandLine(int _argc, const char* const* _argv, const char* _version, const char* _usage) {
...
// has not been called.
vox::string ParsedExecutablePath() {
    Parser* parser = internal::g_global_registrer.parser();
    if (!parser) {
        return {};
    }
...
// found.
const char* ParseOption(const char* _argv, const char* _prefix, const char* _option) {
...
Option::Option(const char* _name, const char* _help, bool _required, ValidateFn _validate)
    : name_(_name ? _name : ""),
      help_(_help ? _help : "..."),
      required_(_required),
      parsed_(false),
...
Parser::Parser()
    : options_count_(0),
      ...
      builtin_help_("help", "Displays help", false, false, &ValidateExclusiveOption) {
    // Set default values.
    set_version(nullptr);
    set_usage(nullptr);
...
ParseResult Parser::Parse(int _argc, const char* const* _argv) {
    ...
    const char* path_end = std::max(std::strrchr(_argv[0], '/'), strrchr(_argv[0], '\\'));
    if (path_end) {
    ...
    int argc_trunc = 0;
    for (; argc_trunc < _argc && std::strcmp(_argv[argc_trunc], "--") != 0; ++argc_trunc) {
    }
    ...
    for (int i = 0; i < argc_trunc; ++i) {
        ...
        if (j == options_count_) {
            std::cout << "Invalid command line argument:\"" << argv << "\"." << std::endl;
            result = kExitFailure;
            break;
        }
    }
    ...
    if (!builtin_help_.Validate(argc_trunc) || !builtin_version_.Validate(argc_trunc)) {
        result = kExitFailure;
    }
    ...
    if (result == kSuccess) {
        for (int i = 0; i < options_count_; ++i) {
            if (!options_[i]->statisfied()) {
                std::cout << "Required option \"" << options_[i]->name() << "\" is not specified." << std::endl;
                result = kExitFailure;
                break;
            }
        }
...
void Parser::Help() {
    ...
    for (int i = 0; i < options_count_; ++i) {
        const Option& option = *options_[i];
        const std::string option_str = std::string(" --") + option.name() + "=<" + option.FormatType() + ">";
        std::cout << std::setiosflags(std::ios::left) << std::setw(28) << option_str
                  << std::resetiosflags(std::ios::left) << option.help() << "(default is " << option.FormatDefault()
                  << ")" << std::endl;
    }
...
bool sort_options(Option* _left, Option* _right) {
    return (_left->required() && !_right->required()) ||
           (_left->required() == _right->required() && std::strcmp(_left->name(), _right->name()) < 0);
}
}  // namespace
...
bool Parser::RegisterOption(Option* _option) {
    ...
    for (int i = 0; i < options_count_; ++i) {
        if (StrICmp(options_[i]->name(), _option->name()) == 0) {
            std::cerr << "Option name:\"" << _option->name() << "\" already registered." << std::endl;
            return false;
        }
    }
...
int Parser::max_options() const { return sizeof(options_) / sizeof(options_[0]) - builtin_options_count_; }

void Parser::set_usage(const char* _usage) { usage_ = _usage ? _usage : "unspecified usage"; }

void Parser::set_version(const char* _version) { version_ = _version ? _version : "unspecified version"; }
