...
void ed::EditorContext::ShowMetrics(const Control &control) {
    ...
    auto liveNodeCount = (int) std::count_if(m_Nodes.begin(), m_Nodes.end(), [](Node *node) {
        return node->m_IsLive;
    });
    auto livePinCount = (int) std::count_if(m_Pins.begin(), m_Pins.end(), [](Pin *pin) {
        return pin->m_IsLive;
    });
    auto liveLinkCount = (int) std::count_if(m_Links.begin(), m_Links.end(), [](Link *link) {
