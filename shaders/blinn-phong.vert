...
//----------------------------------------------------------------------------------------------------------------------
    ...
    layout(location = Weights_0) in vec4 WEIGHTS_0;
        ...
        layout(set = 0, binding = 2) uniform sampler2D u_jointTexture;
...
void main() {
    ...
    #ifdef HAS_NORMAL
        #if defined(HAS_TANGENT) && defined(NORMAL_TEXTURE)
            vec3 normalW = normalize(mat3(normal_mat) * normal.xyz);
            vec3 tangentW = normalize(mat3(normal_mat) * tangent.xyz);
    ...
    //------------------------------------------------------------------------------------------------------------------
    #ifdef NEED_WORLDPOS
        vec4 temp_pos = model_mat * position;
        v_pos = temp_pos.xyz / temp_pos.w;
    #endif

    #ifdef SHADOW_MAP_COUNT
        view_pos = (view_mat * model_mat * position).xyz;
    #endif
