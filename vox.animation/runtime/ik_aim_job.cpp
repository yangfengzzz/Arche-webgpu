...
bool IKAimJob::Validate() const {
    ...
    valid &= vox::simd_math::AreAllTrue1(vox::simd_math::IsNormalizedEst3(forward));
    return valid;
}
...
bool ComputeOffsettedForward(_SimdFloat4 _forward,
                             _SimdFloat4 _offset,
                             _SimdFloat4 _target,
                             SimdFloat4* _offsetted_forward) {
    // AO is projected offset vector onto the normalized forward vector.
    assert(vox::simd_math::AreAllTrue1(vox::simd_math::IsNormalizedEst3(_forward)));
    ...
    // If offset is outside the sphere defined by target length, the target
    // isn't reachable.
...
bool IKAimJob::Run() const {
    ...
    using simd_math::Float4x4;
    using simd_math::SimdFloat4;
    using simd_math::SimdQuaternion;
    ...
    bool lreached = ComputeOffsettedForward(forward, offset, joint_to_target_js, &offsetted_forward);
    // Copies reachability result.
    // If offsetted forward vector doesn't exist, target position cannot be
    // aimed.
    if (reached != nullptr) {
        *reached = lreached;
    }
    ...
    const SimdQuaternion joint_to_target_rot_js = SimdQuaternion::FromVectors(offsetted_forward, joint_to_target_js);

    // Calculates rotate_plane_js quaternion which aligns joint up to the pole
    // vector.
    const SimdFloat4 corrected_up_js = TransformVector(joint_to_target_rot_js, up);
    ...
    const SimdFloat4 ref_joint_normal_js = Cross3(pole_vector_js, joint_to_target_js);
    const SimdFloat4 joint_normal_js = Cross3(corrected_up_js, joint_to_target_js);
    ...
    const SimdFloat4 denoms = SetZ(SetY(joint_to_target_js_len2, joint_normal_js_len2), ref_joint_normal_js_len2);
    ...
    if (AreAllTrue3(CmpNe(denoms, simd_float4::zero()))) {
        const SimdFloat4 rsqrts =
                RSqrtEstNR(SetZ(SetY(joint_to_target_js_len2, joint_normal_js_len2), ref_joint_normal_js_len2));
        ...
        const SimdFloat4 rotate_plane_cos_angle =
                Dot3(joint_normal_js * SplatY(rsqrts), ref_joint_normal_js * SplatZ(rsqrts));
        const SimdFloat4 axis_flip = And(SplatX(Dot3(ref_joint_normal_js, corrected_up_js)), simd_int4::mask_sign());
        const SimdFloat4 rotate_plane_axis_flipped_js = Xor(rotate_plane_axis_js, axis_flip);

        // Builds quaternion along rotation axis.
        const SimdFloat4 one = simd_float4::one();
        rotate_plane_js = SimdQuaternion::FromAxisCosAngle(rotate_plane_axis_flipped_js,
                                                           Clamp(-one, rotate_plane_cos_angle, one));
    } else {
    ...
    if (twist_angle != 0.f) {
        ...
        const SimdQuaternion twist_ss =
                SimdQuaternion::FromAxisAngle(rotate_plane_axis_js, simd_float4::Load1(twist_angle));
    ...
    const SimdFloat4 twisted_fu =
            Xor(twisted.xyzw, And(simd_int4::mask_sign(), CmpLt(SplatW(twisted.xyzw), simd_float4::zero())));
