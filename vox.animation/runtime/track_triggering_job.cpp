...
TrackTriggeringJob::TrackTriggeringJob() : from(0.f), to(0.f), threshold(0.f), track(nullptr), iterator(nullptr) {}
...
namespace {
inline bool DetectEdge(
        ptrdiff_t _i0, ptrdiff_t _i1, bool _forward, const TrackTriggeringJob& _job, TrackTriggeringJob::Edge* _edge) {
    ...
    if (detected) {
        ...
        if (step) {
            ...
            if (_i1 == 0) {
                ...
                _edge->ratio = vox::lerp(tk0, tk1, alpha);
            }
        }
    }
    return detected;
...
TrackTriggeringJob::Iterator::Iterator(const TrackTriggeringJob* _job) : job_(_job) {
        ...
        for (; outer_ < job_->to; outer_ += 1.f) {
            for (; inner_ < num_keys; ++inner_) {
                ...
                if (DetectEdge(i0, inner_, true, *job_, &edge_)) {
                    ...
                    if (edge_.ratio >= job_->from && (edge_.ratio < job_->to || job_->to >= 1.f + outer_)) {
                        ++inner_;
        ...
        for (; outer_ + 1.f > job_->to; outer_ -= 1.f) {
            for (; inner_ >= 0; --inner_) {
                ...
                if (DetectEdge(i0, inner_, false, *job_, &edge_)) {
                    edge_.ratio += outer_;  // Convert to global ratio space.
                    if (edge_.ratio >= job_->to && (edge_.ratio < job_->from || job_->from >= 1.f + outer_)) {
                        --inner_;
                        return *this;  // Yield found edge.
                    }
                }
