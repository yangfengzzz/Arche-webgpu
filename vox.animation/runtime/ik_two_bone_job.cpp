...
using namespace vox::simd_math;

namespace vox::animation {
IKTwoBoneJob::IKTwoBoneJob()
    : target(simd_math::simd_float4::zero()),
      mid_axis(simd_math::simd_float4::z_axis()),
      pole_vector(simd_math::simd_float4::y_axis()),
      twist_angle(0.f),
      soften(1.f),
      weight(1.f),
      start_joint(nullptr),
...
bool IKTwoBoneJob::Validate() const {
    ...
    valid &= vox::simd_math::AreAllTrue1(vox::simd_math::IsNormalizedEst3(mid_axis));
    return valid;
}
...
struct IKConstantSetup {
    IKConstantSetup(const IKTwoBoneJob& _job) {
        ...
        const SimdFloat4 start_ms = TransformPoint(inv_mid_joint, _job.start_joint->cols[3]);
        const SimdFloat4 end_ms = TransformPoint(inv_mid_joint, _job.end_joint->cols[3]);

        // Transform some positions to start joint space (_ss)
        const SimdFloat4 mid_ss = TransformPoint(inv_start_joint, _job.mid_joint->cols[3]);
        const SimdFloat4 end_ss = TransformPoint(inv_start_joint, _job.end_joint->cols[3]);
...
bool SoftenTarget(const IKTwoBoneJob& _job,
    ...
    const SimdFloat4 start_target_original_ss = TransformPoint(_setup.inv_start_joint, _job.target);
    const SimdFloat4 start_target_original_ss_len2 = Length3Sqr(start_target_original_ss);
    const SimdFloat4 lengths =
            Sqrt(SetZ(SetY(_setup.start_mid_ss_len2, _setup.mid_end_ss_len2), start_target_original_ss_len2));
    ...
    const SimdFloat4 bone_len_diff_abs = AndNot(start_mid_ss_len - mid_end_ss_len, _setup.mask_sign);
    const SimdFloat4 bones_chain_len = start_mid_ss_len + mid_end_ss_len;
    const SimdFloat4 da =  // da.yzw needs to be 0
            bones_chain_len * Clamp(simd_float4::zero(), simd_float4::LoadX(_job.soften), _setup.one);
    ...
    if ((comp_mask & 0xb) == 0xb) {
        ...
        // The derivative must be 1 for x = 0, and y must never exceed 1.
        ...
        *_start_target_ss =
                start_target_original_ss * SplatX(start_target_ss_len * RcpEstX(start_target_original_ss_len));
    } else {
        *_start_target_ss = start_target_original_ss;
        *_start_target_ss_len2 = start_target_original_ss_len2;
    }

    // The maximum distance we can reach is to soften bone chain length: da
...
SimdQuaternion ComputeMidJoint(const IKTwoBoneJob& _job,
    ...
    const SimdFloat4 start_mid_end_sum_ss_len2 = _setup.start_mid_ss_len2 + _setup.mid_end_ss_len2;
    const SimdFloat4 start_mid_end_ss_half_rlen =
            SplatX(simd_float4::Load1(.5f) * RSqrtEstXNR(_setup.start_mid_ss_len2 * _setup.mid_end_ss_len2));
    ...
    const SimdFloat4 mid_cos_angles_unclamped =
            (SplatX(start_mid_end_sum_ss_len2) - SetY(_start_target_ss_len2, _setup.start_end_ss_len2)) *
            start_mid_end_ss_half_rlen;
    const SimdFloat4 mid_cos_angles = Clamp(_setup.m_one, mid_cos_angles_unclamped, _setup.one);
    ...
    const SimdInt4 bent_side_flip = SplatX(CmpLt(Dot3(bent_side_ref, _setup.mid_end_ms), simd_float4::zero()));
    const SimdFloat4 mid_initial_angle = Xor(ACosX(SplatY(mid_cos_angles)), And(bent_side_flip, _setup.mask_sign));
...
SimdQuaternion ComputeStartJoint(const IKTwoBoneJob& _job,
    ...
    const SimdFloat4 pole_ss = TransformVector(_setup.inv_start_joint, _job.pole_vector);

    // start_mid_ss with quaternion mid_rot_ms applied.
    const SimdFloat4 mid_end_ss_final = TransformVector(
            _setup.inv_start_joint, TransformVector(*_job.mid_joint, TransformVector(_mid_rot_ms, _setup.mid_end_ms)));
    ...
    const SimdQuaternion end_to_target_rot_ss = SimdQuaternion::FromVectors(start_end_ss_final, _start_target_ss);
    ...
    if (AreAllTrue1(CmpGt(_start_target_ss_len2, simd_float4::zero()))) {
        ...
        const vox::simd_math::SimdFloat4 ref_plane_normal_ss = Cross3(_start_target_ss, pole_ss);
        const vox::simd_math::SimdFloat4 ref_plane_normal_ss_len2 = vox::simd_math::Length3Sqr(ref_plane_normal_ss);
        // Computes joint chain plane normal, which is the same as mid joint axis
        // (same triangle).
        const vox::simd_math::SimdFloat4 mid_axis_ss =
                TransformVector(_setup.inv_start_joint, TransformVector(*_job.mid_joint, _job.mid_axis));
        const vox::simd_math::SimdFloat4 joint_plane_normal_ss = TransformVector(end_to_target_rot_ss, mid_axis_ss);
        const vox::simd_math::SimdFloat4 joint_plane_normal_ss_len2 = vox::simd_math::Length3Sqr(joint_plane_normal_ss);
        // Computes all reciprocal square roots at once.
        const SimdFloat4 rsqrts =
                RSqrtEstNR(SetZ(SetY(_start_target_ss_len2, ref_plane_normal_ss_len2), joint_plane_normal_ss_len2));

        // Computes angle cosine between the 2 normalized normals.
        const SimdFloat4 rotate_plane_cos_angle =
                vox::simd_math::Dot3(ref_plane_normal_ss * SplatY(rsqrts), joint_plane_normal_ss * SplatZ(rsqrts));
        ...
        const SimdFloat4 start_axis_flip = And(SplatX(Dot3(joint_plane_normal_ss, pole_ss)), _setup.mask_sign);
        const SimdFloat4 rotate_plane_axis_flipped_ss = Xor(rotate_plane_axis_ss, start_axis_flip);

        // Builds quaternion along rotation axis.
        const SimdQuaternion rotate_plane_ss = SimdQuaternion::FromAxisCosAngle(
                rotate_plane_axis_flipped_ss, Clamp(_setup.m_one, rotate_plane_cos_angle, _setup.one));

        if (_job.twist_angle != 0.f) {
            // If a twist angle is provided, rotation angle is rotated along
            // rotation plane axis.
            const SimdQuaternion twist_ss =
                    SimdQuaternion::FromAxisAngle(rotate_plane_axis_ss, simd_float4::Load1(_job.twist_angle));
...
void WeightOutput(const IKTwoBoneJob& _job,
                  const IKConstantSetup& _setup,
    ...
    const SimdFloat4 start_rot_fu = Xor(_start_rot.xyzw, And(_setup.mask_sign, CmpLt(SplatW(_start_rot.xyzw), zero)));
    const SimdFloat4 mid_rot_fu = Xor(_mid_rot.xyzw, And(_setup.mask_sign, CmpLt(SplatW(_mid_rot.xyzw), zero)));
    ...
    if (_job.weight < 1.f) {
        ...
        // Normalize
        const SimdFloat4 rsqrts = RSqrtEstNR(SetY(Length4Sqr(start_lerp), Length4Sqr(mid_lerp)));
...
bool IKTwoBoneJob::Run() const {
    ...
    if (weight <= 0.f) {
        // No correction.
        *start_joint_correction = *mid_joint_correction = SimdQuaternion::identity();
    ...
    // rotation.
    const SimdQuaternion mid_rot_ms = ComputeMidJoint(*this, setup, start_target_ss_len2);

    // Calculates end_to_target_rot_ss quaternion which solves for effector
    // rotating onto the target.
    const SimdQuaternion start_rot_ss =
            ComputeStartJoint(*this, setup, mid_rot_ms, start_target_ss, start_target_ss_len2);
