...
InputGeom::InputGeom()
    : m_chunkyMesh(nullptr), m_mesh(nullptr), m_hasBuildSettings(false), m_offMeshConCount(0), m_volumeCount(0) {}
...
bool InputGeom::loadMesh(rcContext* ctx, const std::string& filepath) {
    ...
    if (!m_mesh) {
        LOGE("loadMesh: Out of memory 'm_mesh'.")
        return false;
    }
    if (!m_mesh->load(filepath)) {
        LOGE("buildTiledNavigation: Could not load {}", filepath.c_str())
        return false;
    }
    ...
    if (!m_chunkyMesh) {
        LOGE("buildTiledNavigation: Out of memory 'm_chunkyMesh'.")
        return false;
    }
    if (!rcCreateChunkyTriMesh(m_mesh->getVerts(), m_mesh->getTris(), m_mesh->getTriCount(), 256, m_chunkyMesh)) {
        LOGE("buildTiledNavigation: Failed to build chunky mesh.")
        return false;
    }
...
bool InputGeom::loadGeomSet(rcContext* ctx, const std::string& filepath) {
    ...
    while (src < srcEnd) {
        ...
        if (row[0] == 'f') {
            ...
            sscanf(row + 1, "%f %f %f %f %f %f %f %f %f %f %f %f %f %d %f %f %f %f %f %f %f", &m_buildSettings.cellSize,
                   &m_buildSettings.cellHeight, &m_buildSettings.agentHeight, &m_buildSettings.agentRadius,
                   &m_buildSettings.agentMaxClimb, &m_buildSettings.agentMaxSlope, &m_buildSettings.regionMinSize,
                   &m_buildSettings.regionMergeSize, &m_buildSettings.edgeMaxLen, &m_buildSettings.edgeMaxError,
...
bool InputGeom::load(rcContext* ctx, const std::string& filepath) {
    ...
    if (extension == ".gset") return loadGeomSet(ctx, filepath);
    if (extension == ".obj") return loadMesh(ctx, filepath);
...
bool InputGeom::saveGeomSet(const BuildSettings* settings) {
    ...
    if (extPos != std::string::npos) filepath = filepath.substr(0, extPos);
    ...
    if (settings) {
        fprintf(fp, "s %f %f %f %f %f %f %f %f %f %f %f %f %f %d %f %f %f %f %f %f %f\n", settings->cellSize,
                settings->cellHeight, settings->agentHeight, settings->agentRadius, settings->agentMaxClimb,
                settings->agentMaxSlope, settings->regionMinSize, settings->regionMergeSize, settings->edgeMaxLen,
                settings->edgeMaxError, settings->vertsPerPoly, settings->detailSampleDist,
                settings->detailSampleMaxError, settings->partitionType, settings->navMeshBMin[0],
                settings->navMeshBMin[1], settings->navMeshBMin[2], settings->navMeshBMax[0], settings->navMeshBMax[1],
                settings->navMeshBMax[2], settings->tileSize);
    }
...
bool InputGeom::raycastMesh(float* src, float* dst, float& tmin) {
    ...
    for (int i = 0; i < ncid; ++i) {
        ...
        for (int j = 0; j < ntris * 3; j += 3) {
            float t = 1;
            if (intersectSegmentTriangle(src, dst, &verts[tris[j] * 3], &verts[tris[j + 1] * 3],
                                         &verts[tris[j + 2] * 3], t)) {
                if (t < tmin) tmin = t;
                hit = true;
            }
...
void InputGeom::addConvexVolume(
        const float* verts, const int nverts, const float minh, const float maxh, unsigned char area) {
