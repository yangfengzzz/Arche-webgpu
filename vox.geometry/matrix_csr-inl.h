...
template <typename T>
MatrixCsr<T>::Element::Element(size_t i_, size_t j_, const T &value_) : i(i_), j(j_), value(value_) {}
...
void MatrixCsr<T>::compress(const std::initializer_list<std::initializer_list<T>> &lst, T epsilon) {
    ...
    for (size_t i = 0; i < numRows; ++i) {
        VOX_ASSERT(numCols == rowIter->size());
...
void MatrixCsr<T>::addRow(const NonZeroContainerType &nonZeros, const IndexContainerType &columnIndices) {
    VOX_ASSERT(nonZeros.size() == columnIndices.size());
    ...
    std::sort(zipped.begin(), zipped.end(),
              [](std::pair<T, size_t> a, std::pair<T, size_t> b) { return a.second < b.second; });
...
MatrixCsr<T> MatrixCsr<T>::add(const T &s) const {
    MatrixCsr ret(*this);
    parallelFor(kZeroSize, ret._nonZeros.size(), [&](size_t i) { ret._nonZeros[i] += s; });
    return ret;
}
...
MatrixCsr<T> MatrixCsr<T>::rsub(const T &s) const {
    MatrixCsr ret(*this);
    parallelFor(kZeroSize, ret._nonZeros.size(), [&](size_t i) { ret._nonZeros[i] = s - ret._nonZeros[i]; });
    return ret;
}
...
T MatrixCsr<T>::min() const {
    ...
    return parallelReduce(
            kZeroSize, numberOfNonZeros(), std::numeric_limits<T>::max(),
            [&](size_t start, size_t end, T init) {
                T result = init;
                for (size_t i = start; i < end; ++i) {
...
T MatrixCsr<T>::max() const {
    ...
    return parallelReduce(
            kZeroSize, numberOfNonZeros(), std::numeric_limits<T>::min(),
            [&](size_t start, size_t end, T init) {
                T result = init;
                for (size_t i = start; i < end; ++i) {
...
MatrixCsr<U> MatrixCsr<T>::castTo() const {
    ...
    parallelFor(kZeroSize, _rowPointers.size(), [&](size_t i) { rp[i] = _rowPointers[i]; });
...
size_t MatrixCsr<T>::hasElement(size_t i, size_t j) const {
    ...
    auto iter = binaryFind(_columnIndices.begin() + rowBegin, _columnIndices.begin() + rowEnd, j);
