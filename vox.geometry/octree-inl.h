...
void Octree<T>::build(const std::vector<T>& items,
    ...
    _bbox.upper_corner = _bbox.lower_corner + Vector3D(maxEdgeLen, maxEdgeLen, maxEdgeLen);
...
template <typename T>
NearestNeighborQueryResult3<T> Octree<T>::nearest(const Point3D& pt,
    ...
    while (node != nullptr) {
        if (node->isLeaf()) {
            ...
            for (int i = 0; i < 8; ++i) {
                ...
                const auto childBound = BoundingBox3D(bound.corner(i), midPoint);
                Point3D cp = childBound.clamp(pt);
...
template <typename T>
bool Octree<T>::intersects(const BoundingBox3D& box, const BoxIntersectionTestFunc3<T>& testFunc) const {
    return intersects(box, testFunc, 0, _bbox);
}

template <typename T>
bool Octree<T>::intersects(const Ray3D& ray, const RayIntersectionTestFunc3<T>& testFunc) const {
    return intersects(ray, testFunc, 0, _bbox);
}

template <typename T>
void Octree<T>::forEachIntersectingItem(const BoundingBox3D& box,
                                        const BoxIntersectionTestFunc3<T>& testFunc,
...
template <typename T>
void Octree<T>::forEachIntersectingItem(const Ray3D& ray,
...
template <typename T>
ClosestIntersectionQueryResult3<T> Octree<T>::closestIntersection(const Ray3D& ray,
...
template <typename T>
void Octree<T>::build(size_t nodeIdx,
                      size_t depth,
                      const BoundingBox3D& bound,
                      const BoxIntersectionTestFunc3<T>& testFunc) {
...
bool Octree<T>::intersects(const BoundingBox3D& box,
    ...
    if (node.firstChild != kMaxSize) {
        for (int i = 0; i < 8; ++i) {
            if (intersects(box, testFunc, node.firstChild + i, BoundingBox3D(bound.corner(i), bound.midPoint()))) {
                return true;
            }
        }
    }
...
bool Octree<T>::intersects(const Ray3D& ray,
    ...
    if (node.firstChild != kMaxSize) {
        for (int i = 0; i < 8; ++i) {
            if (intersects(ray, testFunc, node.firstChild + i, BoundingBox3D(bound.corner(i), bound.midPoint()))) {
                return true;
            }
        }
    }
