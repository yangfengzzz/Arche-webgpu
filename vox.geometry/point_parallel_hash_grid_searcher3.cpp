...
void PointParallelHashGridSearcher3::build(const ConstArrayAccessor1<Point3D>& points) {
    _points.clear();
    _keys.clear();
...
void PointParallelHashGridSearcher3::forEachNearbyPoint(const Point3D& origin,
                                                        double radius,
...
bool PointParallelHashGridSearcher3::hasNearbyPoint(const Point3D& origin, double radius) const {
    size_t nearbyKeys[8];
    getNearbyKeys(origin, nearbyKeys);
...
Point3I PointParallelHashGridSearcher3::getBucketIndex(const Point3D& position) const {
    Point3I bucketIndex;
...
size_t PointParallelHashGridSearcher3::getHashKeyFromPosition(const Point3D& position) const {
...
void PointParallelHashGridSearcher3::getNearbyKeys(const Point3D& position, size_t* nearbyKeys) const {
