...
VolumeParticleEmitter3::VolumeParticleEmitter3(ImplicitSurface3Ptr implicitSurface,
                                               const BoundingBox3D& maxRegion,
                                               double spacing,
                                               const Vector3D& initialVel,
                                               const Vector3D& linearVel,
...
void VolumeParticleEmitter3::emit(const ParticleSystemData3Ptr& particles,
    ...
    if (_allowOverlapping || _isOneShot) {
        ...
        PointHashGridSearcher3 neighborSearcher(
                Size3(kDefaultHashGridResolution, kDefaultHashGridResolution, kDefaultHashGridResolution),
                2.0 * _spacing);
...
void VolumeParticleEmitter3::setPointGenerator(const PointGenerator3Ptr& newPointsGen) { _pointsGen = newPointsGen; }

const ImplicitSurface3Ptr& VolumeParticleEmitter3::surface() const { return _implicitSurface; }

void VolumeParticleEmitter3::setSurface(const ImplicitSurface3Ptr& newSurface) { _implicitSurface = newSurface; }

const BoundingBox3D& VolumeParticleEmitter3::maxRegion() const { return _bounds; }

void VolumeParticleEmitter3::setMaxRegion(const BoundingBox3D& newMaxRegion) { _bounds = newMaxRegion; }

double VolumeParticleEmitter3::jitter() const { return _jitter; }

void VolumeParticleEmitter3::setJitter(double newJitter) { _jitter = clamp(newJitter, 0.0, 1.0); }

bool VolumeParticleEmitter3::isOneShot() const { return _isOneShot; }

void VolumeParticleEmitter3::setIsOneShot(bool newValue) { _isOneShot = newValue; }

bool VolumeParticleEmitter3::allowOverlapping() const { return _allowOverlapping; }

void VolumeParticleEmitter3::setAllowOverlapping(bool newValue) { _allowOverlapping = newValue; }

size_t VolumeParticleEmitter3::maxNumberOfParticles() const { return _maxNumberOfParticles; }
...
void VolumeParticleEmitter3::setSpacing(double newSpacing) { _spacing = newSpacing; }

Vector3D VolumeParticleEmitter3::initialVelocity() const { return _initialVel; }

void VolumeParticleEmitter3::setInitialVelocity(const Vector3D& newInitialVel) { _initialVel = newInitialVel; }

Vector3D VolumeParticleEmitter3::linearVelocity() const { return _linearVel; }

void VolumeParticleEmitter3::setLinearVelocity(const Vector3D& newLinearVel) { _linearVel = newLinearVel; }

Vector3D VolumeParticleEmitter3::angularVelocity() const { return _angularVel; }

void VolumeParticleEmitter3::setAngularVelocity(const Vector3D& newAngularVel) { _angularVel = newAngularVel; }
...
Vector3D VolumeParticleEmitter3::velocityAt(const Point3D& point) const {
    Point3D r = point - _implicitSurface->transform.translation();
    return _linearVel + _angularVel.cross(Vector3D(r.x, r.y, r.z)) + _initialVel;
}

VolumeParticleEmitter3::Builder VolumeParticleEmitter3::builder() { return {}; }

VolumeParticleEmitter3::Builder& VolumeParticleEmitter3::Builder::withImplicitSurface(
...
VolumeParticleEmitter3::Builder& VolumeParticleEmitter3::Builder::withMaxNumberOfParticles(
        size_t maxNumberOfParticles) {
    _maxNumberOfParticles = maxNumberOfParticles;
    return *this;
}
...
VolumeParticleEmitter3 VolumeParticleEmitter3::Builder::build() const {
    return VolumeParticleEmitter3(_implicitSurface, _bounds, _spacing, _initialVel, _linearVel, _angularVel,
...
VolumeParticleEmitter3Ptr VolumeParticleEmitter3::Builder::makeShared() const {
    return std::shared_ptr<VolumeParticleEmitter3>(
            new VolumeParticleEmitter3(_implicitSurface, _bounds, _spacing, _initialVel, _linearVel, _angularVel,
