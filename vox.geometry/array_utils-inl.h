...
void copyRange1(const ArrayType1 &input, size_t begin, size_t end, ArrayType2 *output) {
    parallelFor(begin, end, [&input, &output](size_t i) { (*output)[i] = input[i]; });
}
...
void copyRange2(const ArrayType1 &input, size_t beginX, size_t endX, size_t beginY, size_t endY, ArrayType2 *output) {
    parallelFor(beginX, endX, beginY, endY, [&input, &output](size_t i, size_t j) { (*output)(i, j) = input(i, j); });
}
...
template <typename ArrayType1, typename ArrayType2>
void copyRange3(const ArrayType1 &input,
                size_t beginX,
                size_t endX,
                size_t beginY,
                size_t endY,
    ...
    parallelFor(beginX, endX, beginY, endY, beginZ, endZ,
                [&input, &output](size_t i, size_t j, size_t k) { (*output)(i, j, k) = input(i, j, k); });
}
