...
#include "vox.geometry/marching_cubes_table.h"
#include "vox.geometry/marching_squares_table.h"
...
inline Vector3D grad(
        const ConstArrayAccessor3<double>& grid, ssize_t i, ssize_t j, ssize_t k, const Vector3D& invGridSize) {
    Vector3D ret;
    ssize_t ip = i + 1;
    ssize_t im = i - 1;
    ssize_t jp = j + 1;
...
static void singleSquare(const std::array<double, 4>& data,
    ...
    for (itrEdge = 0; itrEdge < 4; itrEdge++) {
        ...
        if (idxEdgeFlags & (1 << itrEdge)) {
            ...
            pos = Point3D((1.f - alpha) * pos0.x + alpha * pos1.x, (1.f - alpha) * pos0.y + alpha * pos1.y,
                          (1.f - alpha) * pos0.z + alpha * pos1.z);
...
static void singleCube(const std::array<double, 8>& data,
    ...
    Point3D pos, pos0, pos1;
    Vector3D normal, normal0, normal1;
    double phi0, phi1;
    double alpha;
    Point3D e[12];
    Vector3D n[12];
    ...
    for (itrEdge = 0; itrEdge < 12; itrEdge++) {
        ...
        if (idxEdgeFlags & (1 << itrEdge)) {
            ...
            pos = Point3D((1.0 - alpha) * pos0.x + alpha * pos1.x, (1.0 - alpha) * pos0.y + alpha * pos1.y,
...
void marchingCubes(const ConstArrayAccessor3<double>& grid,
    ...
    auto pos = [origin, gridSize](ssize_t i, ssize_t j, ssize_t k) { return origin + gridSize * Vector3D({i, j, k}); };
    ...
    for (ssize_t k = 0; k < dimz - 1; ++k) {
        for (ssize_t j = 0; j < dimy - 1; ++j) {
            for (ssize_t i = 0; i < dimx - 1; ++i) {
                ...
                bound.lower_corner = pos(i, j, k);
                bound.upper_corner = pos(i + 1, j + 1, k + 1);
