...
public:
    ...
    //!
    [[nodiscard]] size_t getHashKeyFromBucketIndex(const Point3I& bucketIndex) const;
    ...
    //!
    [[nodiscard]] Point3I getBucketIndex(const Point3D& position) const;
...
private:
    ...
    [[nodiscard]] size_t getHashKeyFromPosition(const Point3D& position) const;

    void getNearbyKeys(const Point3D& position, size_t* bucketIndices) const;
};
...
//!        step.
//!
class PointParallelHashGridSearcher3::Builder final : public PointNeighborSearcherBuilder3 {
public:
