...
#include "vox.geometry/point_particle_emitter3.h"

#include "vox.geometry/samplers.h"

namespace vox {

PointParticleEmitter3::PointParticleEmitter3(const Point3D& origin,
...
size_t PointParticleEmitter3::maxNumberOfNewParticlesPerSecond() const { return _maxNumberOfNewParticlesPerSecond; }

void PointParticleEmitter3::setMaxNumberOfNewParticlesPerSecond(size_t rate) {
    _maxNumberOfNewParticlesPerSecond = rate;
}

size_t PointParticleEmitter3::maxNumberOfParticles() const { return _maxNumberOfParticles; }
...
void PointParticleEmitter3::onUpdate(double currentTimeInSeconds, double timeIntervalInSeconds) {
    ...
    if (maxNumberOfNewParticles > 0) {
        Array1<Point3D> candidatePositions;
...
void PointParticleEmitter3::emit(Array1<Point3D>* newPositions,
                                 Array1<Vector3D>* newVelocities,
                                 size_t maxNewNumberOfParticles) {
    for (size_t i = 0; i < maxNewNumberOfParticles; ++i) {
        Vector3D newDirection = uniformSampleCone(random(), random(), _direction, _spreadAngleInRadians);
...
PointParticleEmitter3::Builder PointParticleEmitter3::builder() { return Builder(); }

PointParticleEmitter3::Builder& PointParticleEmitter3::Builder::withOrigin(const Point3D& origin) {
    _origin = origin;
    return *this;
}
...
PointParticleEmitter3::Builder& PointParticleEmitter3::Builder::withMaxNumberOfNewParticlesPerSecond(
        size_t maxNumOfNewParticlesPerSec) {
    _maxNumberOfNewParticlesPerSecond = maxNumOfNewParticlesPerSec;
    return *this;
}
...
PointParticleEmitter3 PointParticleEmitter3::Builder::build() const {
    return PointParticleEmitter3(_origin, _direction, _speed, _spreadAngleInDegrees, _maxNumberOfNewParticlesPerSecond,
...
PointParticleEmitter3Ptr PointParticleEmitter3::Builder::makeShared() const {
    return std::shared_ptr<PointParticleEmitter3>(
            new PointParticleEmitter3(_origin, _direction, _speed, _spreadAngleInDegrees,
