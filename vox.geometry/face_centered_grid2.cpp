...
FaceCenteredGrid2::FaceCenteredGrid2()
    : _dataOriginU(0.0, 0.5),
      ...
      _uLinearSampler(LinearArraySampler2<double, double>(_dataU.constAccessor(), Vector2D(1, 1), _dataOriginU)),
      _vLinearSampler(LinearArraySampler2<double, double>(_dataV.constAccessor(), Vector2D(1, 1), _dataOriginV)) {}

FaceCenteredGrid2::FaceCenteredGrid2(size_t resolutionX,
                                     size_t resolutionY,
                                     double gridSpacingX,
                                     double gridSpacingY,
                                     double originX,
...
const double &FaceCenteredGrid2::u(size_t i, size_t j) const { return _dataU(i, j); }

double &FaceCenteredGrid2::v(size_t i, size_t j) { return _dataV(i, j); }

const double &FaceCenteredGrid2::v(size_t i, size_t j) const { return _dataV(i, j); }

Vector2D FaceCenteredGrid2::valueAtCellCenter(size_t i, size_t j) const {
    VOX_ASSERT(i < resolution().x && j < resolution().y);

    return 0.5 * Vector2D(_dataU(i, j) + _dataU(i + 1, j), _dataV(i, j) + _dataV(i, j + 1));
}
...
FaceCenteredGrid2::ScalarDataAccessor FaceCenteredGrid2::uAccessor() { return _dataU.accessor(); }

FaceCenteredGrid2::ConstScalarDataAccessor FaceCenteredGrid2::uConstAccessor() const { return _dataU.constAccessor(); }

FaceCenteredGrid2::ScalarDataAccessor FaceCenteredGrid2::vAccessor() { return _dataV.accessor(); }

FaceCenteredGrid2::ConstScalarDataAccessor FaceCenteredGrid2::vConstAccessor() const { return _dataV.constAccessor(); }

VectorGrid2::DataPositionFunc FaceCenteredGrid2::uPosition() const {
    Vector2D h = gridSpacing();

    return [this, h](size_t i, size_t j) -> Point2D { return _dataOriginU + h * Vector2D({i, j}); };
}

VectorGrid2::DataPositionFunc FaceCenteredGrid2::vPosition() const {
    Vector2D h = gridSpacing();

    return [this, h](size_t i, size_t j) -> Point2D { return _dataOriginV + h * Vector2D({i, j}); };
}
...
Point2D FaceCenteredGrid2::uOrigin() const { return _dataOriginU; }

Point2D FaceCenteredGrid2::vOrigin() const { return _dataOriginV; }

void FaceCenteredGrid2::fill(const Vector2D &value, ExecutionPolicy policy) {
    parallelFor(
            kZeroSize, _dataU.width(), kZeroSize, _dataU.height(),
            [this, value](size_t i, size_t j) { _dataU(i, j) = value.x; }, policy);

    parallelFor(
            kZeroSize, _dataV.width(), kZeroSize, _dataV.height(),
            [this, value](size_t i, size_t j) { _dataV(i, j) = value.y; }, policy);
}

void FaceCenteredGrid2::fill(const std::function<Vector2D(const Point2D &)> &func, ExecutionPolicy policy) {
    DataPositionFunc uPos = uPosition();
    parallelFor(
            kZeroSize, _dataU.width(), kZeroSize, _dataU.height(),
            [this, &func, &uPos](size_t i, size_t j) { _dataU(i, j) = func(uPos(i, j)).x; }, policy);
...
void FaceCenteredGrid2::parallelForEachUIndex(const std::function<void(size_t, size_t)> &func) const {
    _dataU.parallelForEachIndex(func);
}

void FaceCenteredGrid2::forEachVIndex(const std::function<void(size_t, size_t)> &func) const {
    _dataV.forEachIndex(func);
}

void FaceCenteredGrid2::parallelForEachVIndex(const std::function<void(size_t, size_t)> &func) const {
    _dataV.parallelForEachIndex(func);
}

Vector2D FaceCenteredGrid2::sample(const Point2D &x) const { return _sampler(x); }

std::function<Vector2D(const Point2D &)> FaceCenteredGrid2::sampler() const { return _sampler; }

double FaceCenteredGrid2::divergence(const Point2D &x) const {
    ssize_t i, j;
    double fx, fy;
    Point2D cellCenterOrigin = origin() + 0.5 * gridSpacing();
    ...
    for (int n = 0; n < 4; ++n) {
        result += weights[n] * divergenceAtCellCenter(indices[n].x, indices[n].y);
    }

    return result;
}

double FaceCenteredGrid2::curl(const Point2D &x) const {
    ssize_t i, j;
    double fx, fy;
    Point2D cellCenterOrigin = origin() + 0.5 * gridSpacing();
...
void FaceCenteredGrid2::resetSampler() {
    LinearArraySampler2<double, double> uSampler(_dataU.constAccessor(), gridSpacing(), _dataOriginU);
    LinearArraySampler2<double, double> vSampler(_dataV.constAccessor(), gridSpacing(), _dataOriginV);
...
void FaceCenteredGrid2::setData(const std::vector<double> &data) {
    VOX_ASSERT(uSize().x * uSize().y + vSize().x * vSize().y == data.size());

    size_t cnt = 0;
    _dataU.forEachIndex([&](size_t i, size_t j) { _dataU(i, j) = data[cnt++]; });
    _dataV.forEachIndex([&](size_t i, size_t j) { _dataV(i, j) = data[cnt++]; });
}
...
FaceCenteredGrid2::Builder &FaceCenteredGrid2::Builder::withOrigin(const Point2D &gridOrigin) {
    _gridOrigin = gridOrigin;
    return *this;
}

FaceCenteredGrid2::Builder &FaceCenteredGrid2::Builder::withOrigin(double gridOriginX, double gridOriginY) {
    _gridOrigin.x = gridOriginX;
    _gridOrigin.y = gridOriginY;
    return *this;
}

FaceCenteredGrid2::Builder &FaceCenteredGrid2::Builder::withInitialValue(const Vector2D &initialVal) {
    _initialVal = initialVal;
    return *this;
}

FaceCenteredGrid2::Builder &FaceCenteredGrid2::Builder::withInitialValue(double initialValX, double initialValY) {
...
FaceCenteredGrid2 FaceCenteredGrid2::Builder::build() const {
    return FaceCenteredGrid2(_resolution, _gridSpacing, _gridOrigin, _initialVal);
}
...
VectorGrid2Ptr FaceCenteredGrid2::Builder::build(const Size2 &resolution,
    ...
    return std::shared_ptr<FaceCenteredGrid2>(new FaceCenteredGrid2(resolution, gridSpacing, gridOrigin, initialVal),
                                              [](FaceCenteredGrid2 *obj) { delete obj; });
}
