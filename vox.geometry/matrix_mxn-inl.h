...
T MatrixMxN<T>::min() const {
    ...
    return parallelReduce(
            kZeroSize, rows() * cols(), std::numeric_limits<T>::max(),
            [&](size_t start, size_t end, T init) {
                T result = init;
                for (size_t i = start; i < end; ++i) {
...
T MatrixMxN<T>::max() const {
    ...
    return parallelReduce(
            kZeroSize, rows() * cols(), std::numeric_limits<T>::min(),
            [&](size_t start, size_t end, T init) {
                T result = init;
                for (size_t i = start; i < end; ++i) {
...
T MatrixMxN<T>::absmin() const {
    return parallelReduce(
            ...
            [&](size_t start, size_t end, T init) {
                T result = init;
                for (size_t i = start; i < end; ++i) {
                    result = vox::absmin(result, _elements[i]);
                }
                return result;
            },
            vox::absmin<T>);
...
T MatrixMxN<T>::absmax() const {
    return parallelReduce(
            ...
            [&](size_t start, size_t end, T init) {
                T result = init;
                for (size_t i = start; i < end; ++i) {
                    result = vox::absmax(result, _elements[i]);
                }
                return result;
            },
            vox::absmax<T>);
...
void MatrixMxN<T>::parallelForEach(Callback func) {
    parallelFor(kZeroSize, cols(), kZeroSize, rows(), [&](size_t j, size_t i) { func((*this)(i, j)); });
}

template <typename T>
template <typename Callback>
void MatrixMxN<T>::parallelForEachIndex(Callback func) const {
    parallelFor(kZeroSize, cols(), kZeroSize, rows(), [&](size_t j, size_t i) { func(i, j); });
}
