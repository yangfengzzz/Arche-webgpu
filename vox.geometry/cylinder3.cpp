...
Cylinder3::Cylinder3(const Transform3D& transform, bool isNormalFlipped) : Surface3(transform, isNormalFlipped) {}

Cylinder3::Cylinder3(
        const Point3D& center_, double radius_, double height_, const Transform3D& transform, bool isNormalFlipped)
    : Surface3(transform, isNormalFlipped), center(center_), radius(radius_), height(height_) {}

Cylinder3::Cylinder3(const Cylinder3& other) = default;
...
Point3D Cylinder3::closestPointLocal(const Point3D& otherPoint) const {
    ...
    Point2D cp = box.closestPoint(rr);
...
bool Cylinder3::intersectsLocal(const Ray3D& ray) const {
    ...
    Plane3 upperPlane(Vector3D(0, 1, 0), bbox.upper_corner);
    Plane3 lowerPlane(Vector3D(0, -1, 0), bbox.lower_corner);
    ...
    Point3D pointOnCylinder = ray.pointAt(tCylinder);

    if (pointOnCylinder.y >= center.y - 0.5 * height && pointOnCylinder.y <= center.y + 0.5 * height) {
        return true;
    }
...
Cylinder3::Builder Cylinder3::builder() { return {}; }
...
Cylinder3Ptr Cylinder3::Builder::makeShared() const {
    return {new Cylinder3(_center, _radius, _height, _transform, _isNormalFlipped), [](Cylinder3* obj) { delete obj; }};
}
