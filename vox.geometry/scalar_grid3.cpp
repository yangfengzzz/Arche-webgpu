...
void ScalarGrid3::resize(size_t resolutionX,
    ...
    resize(Size3(resolutionX, resolutionY, resolutionZ), Vector3D(gridSpacingX, gridSpacingY, gridSpacingZ),
           Point3D(originX, originY, originZ), initialValue);
}

void ScalarGrid3::resize(const Size3 &resolution,
...
void ScalarGrid3::resize(const Vector3D &gridSpacing, const Point3D &origin) {
...
double ScalarGrid3::sample(const Point3D &x) const { return _sampler(x); }

std::function<double(const Point3D &)> ScalarGrid3::sampler() const { return _sampler; }
...
ScalarGrid3::DataPositionFunc ScalarGrid3::dataPosition() const {
    Point3D o = dataOrigin();
    return [this, o](size_t i, size_t j, size_t k) -> Point3D { return o + gridSpacing() * Vector3D({i, j, k}); };
}

void ScalarGrid3::fill(double value, ExecutionPolicy policy) {
    parallelFor(
            kZeroSize, _data.width(), kZeroSize, _data.height(), kZeroSize, _data.depth(),
            [this, value](size_t i, size_t j, size_t k) { _data(i, j, k) = value; }, policy);
}

void ScalarGrid3::fill(const std::function<double(const Point3D &)> &func, ExecutionPolicy policy) {
...
void ScalarGrid3::setData(const std::vector<double> &data) {
    VOX_ASSERT(dataSize().x * dataSize().y * dataSize().z == data.size());
