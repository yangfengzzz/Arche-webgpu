...
template <typename T>
Array<T, 3>::Array(const std::initializer_list<std::initializer_list<std::initializer_list<T>>> &lst) {
    set(lst);
}
...
void Array<T, 3>::set(const std::initializer_list<std::initializer_list<std::initializer_list<T>>> &lst) {
    ...
    for (size_t k = 0; k < depth; ++k) {
        ...
        VOX_ASSERT(height == pageIter->size());
        for (size_t j = 0; j < height; ++j) {
            auto colIter = rowIter->begin();
            VOX_ASSERT(width == rowIter->size());
