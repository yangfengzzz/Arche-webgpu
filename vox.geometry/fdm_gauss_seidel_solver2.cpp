...
bool FdmGaussSeidelSolver2::solveCompressed(FdmCompressedLinearSystem2 *system) {
    ...
    for (unsigned int iter = 0; iter < _maxNumberOfIterations; ++iter) {
        ...
        if (iter != 0 && iter % _residualCheckInterval == 0) {
            FdmCompressedBlas2::residual(system->A, system->x, system->b, &_residualComp);
...
unsigned int FdmGaussSeidelSolver2::maxNumberOfIterations() const { return _maxNumberOfIterations; }

unsigned int FdmGaussSeidelSolver2::lastNumberOfIterations() const { return _lastNumberOfIterations; }
...
bool FdmGaussSeidelSolver2::useRedBlackOrdering() const { return _useRedBlackOrdering; }

void FdmGaussSeidelSolver2::relax(const FdmMatrix2 &A, const FdmVector2 &b, double sorFactor, FdmVector2 *x_) {
    Size2 size = A.size();
    FdmVector2 &x = *x_;
    ...
    A.forEachIndex([&](size_t i, size_t j) {
        double r = ((i > 0) ? A(i - 1, j).right * x(i - 1, j) : 0.0) +
                   ...
                   ((j > 0) ? A(i, j - 1).up * x(i, j - 1) : 0.0) + ((j + 1 < size.y) ? A(i, j).up * x(i, j + 1) : 0.0);

        x(i, j) = (1.0 - sorFactor) * x(i, j) + sorFactor * (b(i, j) - r) / A(i, j).center;
    });
}
...
void FdmGaussSeidelSolver2::relaxRedBlack(const FdmMatrix2 &A, const FdmVector2 &b, double sorFactor, FdmVector2 *x_) {
    Size2 size = A.size();
    FdmVector2 &x = *x_;

    // Red update
    parallelRangeFor(kZeroSize, size.x, kZeroSize, size.y, [&](size_t iBegin, size_t iEnd, size_t jBegin, size_t jEnd) {
