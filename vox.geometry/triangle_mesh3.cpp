...
struct WindingNumberGatherData {
    ...
    Point3D areaWeightedPositionSums;
    ...
    WindingNumberGatherData operator+(const WindingNumberGatherData &other) const {
        ...
        sum.areaWeightedPositionSums = Point3D(areaWeightedPositionSums.x + other.areaWeightedPositionSums.x,
                                               areaWeightedPositionSums.y + other.areaWeightedPositionSums.y,
                                               areaWeightedPositionSums.z + other.areaWeightedPositionSums.z);
...
WindingNumberGatherData postOrderTraversal(const Bvh3<size_t> &bvh,
                                           size_t nodeIndex,
...
inline std::ostream &operator<<(std::ostream &strm, const Point3D &v) {
    strm << v.x << ' ' << v.y << ' ' << v.z;
    return strm;
}
...
Point3D TriangleMesh3::closestPointLocal(const Point3D &otherPoint) const {
    buildBvh();

    const auto distanceFunc = [this](const size_t &triIdx, const Point3D &pt) {
...
Vector3D TriangleMesh3::closestNormalLocal(const Point3D &otherPoint) const {
    buildBvh();
...
bool TriangleMesh3::isInsideLocal(const Point3D &otherPoint) const {
...
double TriangleMesh3::closestDistanceLocal(const Point3D &otherPoint) const {
    buildBvh();
...
const Point3D &TriangleMesh3::point(size_t i) const { return _points[i]; }

Point3D &TriangleMesh3::point(size_t i) {
    invalidateCache();
    return _points[i];
}
...
void TriangleMesh3::addPoint(const Point3D &pt) { _points.append(pt); }
        ...
        for (size_t f = 0; f < shape.mesh.num_face_vertices.size(); ++f) {
            ...
            if (fv == 3) {
                if (!attrib.vertices.empty()) {
                    addPointTriangle({shape.mesh.indices[idx].vertex_index, shape.mesh.indices[idx + 1].vertex_index,
                                      shape.mesh.indices[idx + 2].vertex_index});
                }

                if (!attrib.normals.empty()) {
                    addNormalTriangle({shape.mesh.indices[idx].normal_index, shape.mesh.indices[idx + 1].normal_index,
                                       shape.mesh.indices[idx + 2].normal_index});
                }

                if (!attrib.texcoords.empty()) {
                    addUvTriangle({shape.mesh.indices[idx].texcoord_index, shape.mesh.indices[idx + 1].texcoord_index,
...
void TriangleMesh3::buildWindingNumbers() const {
    ...
    if (_wnInvalidated) {
        ...
        const auto leafFunc = [&](size_t nodeIndex) -> WindingNumberGatherData {
            ...
            VOX_ASSERT(iter != _bvh.end());

            Triangle3 tri = triangle(*iter);
            double area = tri.area();
            result.areaSums = area;
            result.areaWeightedNormalSums = area * tri.faceNormal();
            result.areaWeightedPositionSums = Point3D(tri.points[0].x + tri.points[1].x + tri.points[2].x,
                                                      tri.points[0].y + tri.points[1].y + tri.points[2].y,
                                                      tri.points[0].z + tri.points[1].z + tri.points[2].z) *
                                              area / 3.0;
...
double TriangleMesh3::windingNumber(const Point3D &queryPoint, size_t triIndex) const {
    // Jacobson et al., Robust Inside-Outside Segmentation using Generalized
    // Winding Numbers, ACM SIGGRAPH 2013.
    const Point3D &vi = _points[_pointIndices[triIndex][0]];
    const Point3D &vj = _points[_pointIndices[triIndex][1]];
    const Point3D &vk = _points[_pointIndices[triIndex][2]];
...
double TriangleMesh3::fastWindingNumber(const Point3D &queryPoint, double accuracy) const {
    buildWindingNumbers();

    return fastWindingNumber(queryPoint, 0, accuracy);
}

double TriangleMesh3::fastWindingNumber(const Point3D &q, size_t rootNodeIndex, double accuracy) const {
    // Barill et al., Fast Winding Numbers for Soups and Clouds, ACM SIGGRAPH
    // 2018.
    const Point3D &treeP = _wnAreaWeightedAvgPositions[rootNodeIndex];
    ...
    const Vector3D treeRVec = vox::max(treeP - treeBound.lower_corner, treeBound.upper_corner - treeP);
...
TriangleMesh3 TriangleMesh3::Builder::build() const {
    return TriangleMesh3(_points, _normals, _uvs, _pointIndices, _normalIndices, _uvIndices, _transform,
                         _isNormalFlipped);
}
