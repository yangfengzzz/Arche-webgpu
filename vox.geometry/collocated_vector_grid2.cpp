...
Vector2D &CollocatedVectorGrid2::operator()(size_t i, size_t j) { return _data(i, j); }
...
Vector2D CollocatedVectorGrid2::sample(const Point2D &x) const { return _sampler(x); }
...
std::function<Vector2D(const Point2D &)> CollocatedVectorGrid2::sampler() const { return _sampler; }

VectorGrid2::VectorDataAccessor CollocatedVectorGrid2::dataAccessor() { return _data.accessor(); }

VectorGrid2::ConstVectorDataAccessor CollocatedVectorGrid2::constDataAccessor() const { return _data.constAccessor(); }

VectorGrid2::DataPositionFunc CollocatedVectorGrid2::dataPosition() const {
    Point2D dataOrigin_ = dataOrigin();
    return [this, dataOrigin_](size_t i, size_t j) -> Point2D {
        return dataOrigin_ + gridSpacing() * Vector2D({i, j});
    };
}

void CollocatedVectorGrid2::forEachDataPointIndex(const std::function<void(size_t, size_t)> &func) const {
    _data.forEachIndex(func);
}

void CollocatedVectorGrid2::parallelForEachDataPointIndex(const std::function<void(size_t, size_t)> &func) const {
    _data.parallelForEachIndex(func);
}

void CollocatedVectorGrid2::swapCollocatedVectorGrid(CollocatedVectorGrid2 *other) {
    swapGrid(other);
...
void CollocatedVectorGrid2::setCollocatedVectorGrid(const CollocatedVectorGrid2 &other) {
    setGrid(other);
...
void CollocatedVectorGrid2::resetSampler() {
    _linearSampler = LinearArraySampler2<Vector2D, double>(_data.constAccessor(), gridSpacing(), dataOrigin());
...
void CollocatedVectorGrid2::setData(const std::vector<double> &data) {
    VOX_ASSERT(2 * dataSize().x * dataSize().y == data.size());
