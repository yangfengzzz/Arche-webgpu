...
#include "vox.geometry/volume_grid_emitter3.h"
...
VolumeGridEmitter3::~VolumeGridEmitter3() = default;

void VolumeGridEmitter3::addSignedDistanceTarget(const ScalarGrid3Ptr& scalarGridTarget) {
    auto mapper = [](double sdf, const Point3D&, double oldVal) { return std::min(oldVal, sdf); };
    addTarget(scalarGridTarget, mapper);
}

void VolumeGridEmitter3::addStepFunctionTarget(const ScalarGrid3Ptr& scalarGridTarget,
                                               double minValue,
                                               double maxValue) {
    double smoothingWidth = scalarGridTarget->gridSpacing().min();
    auto mapper = [minValue, maxValue, smoothingWidth, scalarGridTarget](double sdf, const Point3D&, double oldVal) {
...
VolumeGridEmitter3::Builder VolumeGridEmitter3::builder() { return {}; }
...
VolumeGridEmitter3Ptr VolumeGridEmitter3::Builder::makeShared() const {
    return {new VolumeGridEmitter3(_sourceRegion, _isOneShot), [](VolumeGridEmitter3* obj) { delete obj; }};
}
