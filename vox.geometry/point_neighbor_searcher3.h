...
public:
    ...
    typedef std::function<void(size_t, const Point3D&)> ForEachNearbyPointFunc;
    ...
    virtual void build(const ConstArrayAccessor1<Point3D>& points) = 0;
    ...
    //!
    virtual void forEachNearbyPoint(const Point3D& origin,
                                    double radius,
    ...
    //!
    [[nodiscard]] virtual bool hasNearbyPoint(const Point3D& origin, double radius) const = 0;
    ...
    //!
    [[nodiscard]] virtual std::shared_ptr<PointNeighborSearcher3> clone() const = 0;
};
