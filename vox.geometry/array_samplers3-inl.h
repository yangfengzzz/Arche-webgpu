...
NearestArraySampler3<T, R>::NearestArraySampler(const ConstArrayAccessor3<T> &accessor,
...
template <typename T, typename R>
T NearestArraySampler3<T, R>::operator()(const Point3<R> &x) const {
    ssize_t i, j, k;
    R fx, fy, fz;
...
template <typename T, typename R>
void NearestArraySampler3<T, R>::getCoordinate(const Point3<R> &x, Point3UI *index) const {
    ssize_t i, j, k;
    R fx, fy, fz;
...
template <typename T, typename R>
LinearArraySampler3<T, R>::LinearArraySampler(const ConstArrayAccessor3<T> &accessor,
                                              const Vector3<R> &gridSpacing,
                                              const Point3<R> &gridOrigin) {
    _gridSpacing = gridSpacing;
...
template <typename T, typename R>
T LinearArraySampler3<T, R>::operator()(const Point3<R> &x) const {
    ssize_t i, j, k;
    R fx, fy, fz;
    ...
    return trilerp(_accessor(i, j, k), _accessor(ip1, j, k), _accessor(i, jp1, k), _accessor(ip1, jp1, k),
                   _accessor(i, j, kp1), _accessor(ip1, j, kp1), _accessor(i, jp1, kp1), _accessor(ip1, jp1, kp1), fx,
                   fy, fz);
}

template <typename T, typename R>
void LinearArraySampler3<T, R>::getCoordinatesAndWeights(const Point3<R> &x,
...
template <typename T, typename R>
void LinearArraySampler3<T, R>::getCoordinatesAndGradientWeights(const Point3<R> &x,
                                                                 std::array<Point3UI, 8> *indices,
    ...
    (*weights)[0] = Vector3<R>(-_invGridSpacing.x * (1 - fy) * (1 - fz), -_invGridSpacing.y * (1 - fx) * (1 - fz),
                               -_invGridSpacing.z * (1 - fx) * (1 - fy));
    (*weights)[1] = Vector3<R>(_invGridSpacing.x * (1 - fy) * (1 - fz), fx * (-_invGridSpacing.y) * (1 - fz),
                               fx * (1 - fy) * (-_invGridSpacing.z));
    (*weights)[2] = Vector3<R>((-_invGridSpacing.x) * fy * (1 - fz), (1 - fx) * _invGridSpacing.y * (1 - fz),
                               (1 - fx) * fy * (-_invGridSpacing.z));
    (*weights)[3] = Vector3<R>(_invGridSpacing.x * fy * (1 - fz), fx * _invGridSpacing.y * (1 - fz),
                               fx * fy * (-_invGridSpacing.z));
    (*weights)[4] = Vector3<R>((-_invGridSpacing.x) * (1 - fy) * fz, (1 - fx) * (-_invGridSpacing.y) * fz,
                               (1 - fx) * (1 - fy) * _invGridSpacing.z);
    (*weights)[5] = Vector3<R>(_invGridSpacing.x * (1 - fy) * fz, fx * (-_invGridSpacing.y) * fz,
                               fx * (1 - fy) * _invGridSpacing.z);
    (*weights)[6] = Vector3<R>((-_invGridSpacing.x) * fy * fz, (1 - fx) * _invGridSpacing.y * fz,
                               (1 - fx) * fy * _invGridSpacing.z);
    (*weights)[7] = Vector3<R>(_invGridSpacing.x * fy * fz, fx * _invGridSpacing.y * fz, fx * fy * _invGridSpacing.z);
}

template <typename T, typename R>
std::function<T(const Point3<R> &)> LinearArraySampler3<T, R>::functor() const {
    LinearArraySampler sampler(*this);
    return std::bind(&LinearArraySampler::operator(), sampler, std::placeholders::_1);
}

template <typename T, typename R>
CubicArraySampler3<T, R>::CubicArraySampler(const ConstArrayAccessor3<T> &accessor,
                                            const Vector3<R> &gridSpacing,
                                            const Point3<R> &gridOrigin) {
    _gridSpacing = gridSpacing;
    _origin = gridOrigin;
...
T CubicArraySampler3<T, R>::operator()(const Point3<R> &x) const {
    ...
    ssize_t is[4] = {std::max(i - 1, kZeroSSize), i, std::min(i + 1, iSize - 1), std::min(i + 2, iSize - 1)};
    ssize_t js[4] = {std::max(j - 1, kZeroSSize), j, std::min(j + 1, jSize - 1), std::min(j + 2, jSize - 1)};
    ssize_t ks[4] = {std::max(k - 1, kZeroSSize), k, std::min(k + 1, kSize - 1), std::min(k + 2, kSize - 1)};
    ...
    for (int kk = 0; kk < 4; ++kk) {
        ...
        for (int jj = 0; jj < 4; ++jj) {
            jValues[jj] = monotonicCatmullRom(_accessor(is[0], js[jj], ks[kk]), _accessor(is[1], js[jj], ks[kk]),
                                              _accessor(is[2], js[jj], ks[kk]), _accessor(is[3], js[jj], ks[kk]), fx);
        }
...
template <typename T, typename R>
std::function<T(const Point3<R> &)> CubicArraySampler3<T, R>::functor() const {
