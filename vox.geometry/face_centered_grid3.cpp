...
FaceCenteredGrid3::FaceCenteredGrid3()
    : _dataOriginU(0.0, 0.5, 0.5),
      ...
      _uLinearSampler(LinearArraySampler3<double, double>(_dataU.constAccessor(), Vector3D(1, 1, 1), _dataOriginU)),
      _vLinearSampler(LinearArraySampler3<double, double>(_dataV.constAccessor(), Vector3D(1, 1, 1), _dataOriginV)),
      _wLinearSampler(LinearArraySampler3<double, double>(_dataW.constAccessor(), Vector3D(1, 1, 1), _dataOriginW)) {}
...
FaceCenteredGrid3::FaceCenteredGrid3(size_t resolutionX,
    ...
    : FaceCenteredGrid3(Size3(resolutionX, resolutionY, resolutionZ),
                        ...
                        Point3D(originX, originY, originZ),
...
double &FaceCenteredGrid3::u(size_t i, size_t j, size_t k) { return _dataU(i, j, k); }

const double &FaceCenteredGrid3::u(size_t i, size_t j, size_t k) const { return _dataU(i, j, k); }

double &FaceCenteredGrid3::v(size_t i, size_t j, size_t k) { return _dataV(i, j, k); }

const double &FaceCenteredGrid3::v(size_t i, size_t j, size_t k) const { return _dataV(i, j, k); }

double &FaceCenteredGrid3::w(size_t i, size_t j, size_t k) { return _dataW(i, j, k); }

const double &FaceCenteredGrid3::w(size_t i, size_t j, size_t k) const { return _dataW(i, j, k); }
...
Vector3D FaceCenteredGrid3::valueAtCellCenter(size_t i, size_t j, size_t k) const {
    ...
    return 0.5 * Vector3D(_dataU(i, j, k) + _dataU(i + 1, j, k), _dataV(i, j, k) + _dataV(i, j + 1, k),
...
double FaceCenteredGrid3::divergenceAtCellCenter(size_t i, size_t j, size_t k) const {
    ...
    return (rightU - leftU) / gs.x + (topV - bottomV) / gs.y + (frontW - backW) / gs.z;
}
...
FaceCenteredGrid3::ConstScalarDataAccessor FaceCenteredGrid3::uConstAccessor() const { return _dataU.constAccessor(); }

FaceCenteredGrid3::ScalarDataAccessor FaceCenteredGrid3::vAccessor() { return _dataV.accessor(); }

FaceCenteredGrid3::ConstScalarDataAccessor FaceCenteredGrid3::vConstAccessor() const { return _dataV.constAccessor(); }

FaceCenteredGrid3::ScalarDataAccessor FaceCenteredGrid3::wAccessor() { return _dataW.accessor(); }

FaceCenteredGrid3::ConstScalarDataAccessor FaceCenteredGrid3::wConstAccessor() const { return _dataW.constAccessor(); }

VectorGrid3::DataPositionFunc FaceCenteredGrid3::uPosition() const {
    Vector3D h = gridSpacing();

    return [this, h](size_t i, size_t j, size_t k) -> Point3D { return _dataOriginU + h * Vector3D({i, j, k}); };
}

VectorGrid3::DataPositionFunc FaceCenteredGrid3::vPosition() const {
    Vector3D h = gridSpacing();

    return [this, h](size_t i, size_t j, size_t k) -> Point3D { return _dataOriginV + h * Vector3D({i, j, k}); };
}

VectorGrid3::DataPositionFunc FaceCenteredGrid3::wPosition() const {
    Vector3D h = gridSpacing();

    return [this, h](size_t i, size_t j, size_t k) -> Point3D { return _dataOriginW + h * Vector3D({i, j, k}); };
}
...
Point3D FaceCenteredGrid3::uOrigin() const { return _dataOriginU; }

Point3D FaceCenteredGrid3::vOrigin() const { return _dataOriginV; }

Point3D FaceCenteredGrid3::wOrigin() const { return _dataOriginW; }

void FaceCenteredGrid3::fill(const Vector3D &value, ExecutionPolicy policy) {
    parallelFor(
            kZeroSize, _dataU.width(), kZeroSize, _dataU.height(), kZeroSize, _dataU.depth(),
            [this, value](size_t i, size_t j, size_t k) { _dataU(i, j, k) = value.x; }, policy);

    parallelFor(
            kZeroSize, _dataV.width(), kZeroSize, _dataV.height(), kZeroSize, _dataV.depth(),
            [this, value](size_t i, size_t j, size_t k) { _dataV(i, j, k) = value.y; }, policy);

    parallelFor(
            kZeroSize, _dataW.width(), kZeroSize, _dataW.height(), kZeroSize, _dataW.depth(),
            [this, value](size_t i, size_t j, size_t k) { _dataW(i, j, k) = value.z; }, policy);
}

void FaceCenteredGrid3::fill(const std::function<Vector3D(const Point3D &)> &func, ExecutionPolicy policy) {
    DataPositionFunc uPos = uPosition();
    parallelFor(
            kZeroSize, _dataU.width(), kZeroSize, _dataU.height(), kZeroSize, _dataU.depth(),
            [this, &func, &uPos](size_t i, size_t j, size_t k) { _dataU(i, j, k) = func(uPos(i, j, k)).x; }, policy);
    DataPositionFunc vPos = vPosition();
    parallelFor(
            kZeroSize, _dataV.width(), kZeroSize, _dataV.height(), kZeroSize, _dataV.depth(),
            [this, &func, &vPos](size_t i, size_t j, size_t k) { _dataV(i, j, k) = func(vPos(i, j, k)).y; }, policy);
...
void FaceCenteredGrid3::parallelForEachUIndex(const std::function<void(size_t, size_t, size_t)> &func) const {
    _dataU.parallelForEachIndex(func);
}

void FaceCenteredGrid3::forEachVIndex(const std::function<void(size_t, size_t, size_t)> &func) const {
    _dataV.forEachIndex(func);
}

void FaceCenteredGrid3::parallelForEachVIndex(const std::function<void(size_t, size_t, size_t)> &func) const {
    _dataV.parallelForEachIndex(func);
}

void FaceCenteredGrid3::forEachWIndex(const std::function<void(size_t, size_t, size_t)> &func) const {
    _dataW.forEachIndex(func);
}

void FaceCenteredGrid3::parallelForEachWIndex(const std::function<void(size_t, size_t, size_t)> &func) const {
    _dataW.parallelForEachIndex(func);
}

Vector3D FaceCenteredGrid3::sample(const Point3D &x) const { return _sampler(x); }

std::function<Vector3D(const Point3D &)> FaceCenteredGrid3::sampler() const { return _sampler; }

double FaceCenteredGrid3::divergence(const Point3D &x) const {
    Size3 res = resolution();
    ssize_t i, j, k;
    double fx, fy, fz;
    Point3D cellCenterOrigin = origin() + 0.5 * gridSpacing();
    ...
    for (int n = 0; n < 8; ++n) {
        result += weights[n] * divergenceAtCellCenter(indices[n].x, indices[n].y, indices[n].z);
    }

    return result;
}

Vector3D FaceCenteredGrid3::curl(const Point3D &x) const {
    Size3 res = resolution();
    ssize_t i, j, k;
    double fx, fy, fz;
    Point3D cellCenterOrigin = origin() + 0.5 * gridSpacing();
...
void FaceCenteredGrid3::resetSampler() {
    LinearArraySampler3<double, double> uSampler(_dataU.constAccessor(), gridSpacing(), _dataOriginU);
    LinearArraySampler3<double, double> vSampler(_dataV.constAccessor(), gridSpacing(), _dataOriginV);
    LinearArraySampler3<double, double> wSampler(_dataW.constAccessor(), gridSpacing(), _dataOriginW);

    _uLinearSampler = uSampler;
    _vLinearSampler = vSampler;
    _wLinearSampler = wSampler;

    _sampler = [uSampler, vSampler, wSampler](const Point3D &x) -> Vector3D {
        double u = uSampler(x);
        double v = vSampler(x);
        double w = wSampler(x);
        return {u, v, w};
...
void FaceCenteredGrid3::getData(std::vector<double> *data) const {
    size_t size =
            uSize().x * uSize().y * uSize().z + vSize().x * vSize().y * vSize().z + wSize().x * wSize().y * wSize().z;
    data->resize(size);
    size_t cnt = 0;
...
void FaceCenteredGrid3::setData(const std::vector<double> &data) {
    VOX_ASSERT(uSize().x * uSize().y * uSize().z + vSize().x * vSize().y * vSize().z +
                       wSize().x * wSize().y * wSize().z ==
               data.size());

    size_t cnt = 0;
    _dataU.forEachIndex([&](size_t i, size_t j, size_t k) { _dataU(i, j, k) = data[cnt++]; });
    _dataV.forEachIndex([&](size_t i, size_t j, size_t k) { _dataV(i, j, k) = data[cnt++]; });
    _dataW.forEachIndex([&](size_t i, size_t j, size_t k) { _dataW(i, j, k) = data[cnt++]; });
}

FaceCenteredGrid3::Builder &FaceCenteredGrid3::Builder::withResolution(const Size3 &resolution) {
    _resolution = resolution;
    return *this;
}

FaceCenteredGrid3::Builder &FaceCenteredGrid3::Builder::withResolution(size_t resolutionX,
                                                                       size_t resolutionY,
                                                                       size_t resolutionZ) {
...
FaceCenteredGrid3::Builder &FaceCenteredGrid3::Builder::withGridSpacing(const Vector3D &gridSpacing) {
    _gridSpacing = gridSpacing;
    return *this;
}

FaceCenteredGrid3::Builder &FaceCenteredGrid3::Builder::withGridSpacing(double gridSpacingX,
                                                                        double gridSpacingY,
                                                                        double gridSpacingZ) {
...
FaceCenteredGrid3::Builder &FaceCenteredGrid3::Builder::withOrigin(const Point3D &gridOrigin) {
    _gridOrigin = gridOrigin;
    return *this;
}

FaceCenteredGrid3::Builder &FaceCenteredGrid3::Builder::withOrigin(double gridOriginX,
                                                                   double gridOriginY,
                                                                   double gridOriginZ) {
...
FaceCenteredGrid3::Builder &FaceCenteredGrid3::Builder::withInitialValue(const Vector3D &initialVal) {
    _initialVal = initialVal;
    return *this;
}

FaceCenteredGrid3::Builder &FaceCenteredGrid3::Builder::withInitialValue(double initialValX,
                                                                         double initialValY,
                                                                         double initialValZ) {
...
FaceCenteredGrid3 FaceCenteredGrid3::Builder::build() const {
    return FaceCenteredGrid3(_resolution, _gridSpacing, _gridOrigin, _initialVal);
}
...
VectorGrid3Ptr FaceCenteredGrid3::Builder::build(const Size3 &resolution,
    ...
    return std::shared_ptr<FaceCenteredGrid3>(new FaceCenteredGrid3(resolution, gridSpacing, gridOrigin, initialVal),
                                              [](FaceCenteredGrid3 *obj) { delete obj; });
}
