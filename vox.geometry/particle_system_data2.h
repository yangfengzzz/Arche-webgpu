...
public:
    ...
    [[nodiscard]] ConstArrayAccessor1<Point2D> positions() const;

    //! Returns the position array (mutable).
    ArrayAccessor1<Point2D> positions();

    //! Returns the velocity array (immutable).
    [[nodiscard]] ConstArrayAccessor1<Vector2D> velocities() const;

    //! Returns the velocity array (mutable).
    ArrayAccessor1<Vector2D> velocities();

    //! Returns the force array (immutable).
    [[nodiscard]] ConstArrayAccessor1<Vector2D> forces() const;
    ...
    //!
    void addParticle(const Point2D& newPosition,
    ...
    //!
    void addParticles(const ConstArrayAccessor1<Point2D>& newPositions,
    ...
    //!
    [[nodiscard]] const PointNeighborSearcher2Ptr& neighborSearcher() const;
