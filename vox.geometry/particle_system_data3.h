...
public:
    ...
    [[nodiscard]] ConstArrayAccessor1<Point3D> positions() const;

    //! Returns the position array (mutable).
    ArrayAccessor1<Point3D> positions();

    //! Returns the velocity array (immutable).
    [[nodiscard]] ConstArrayAccessor1<Vector3D> velocities() const;

    //! Returns the velocity array (mutable).
    ArrayAccessor1<Vector3D> velocities();

    //! Returns the force array (immutable).
    [[nodiscard]] ConstArrayAccessor1<Vector3D> forces() const;
    ...
    [[nodiscard]] ConstArrayAccessor1<Vector3D> vectorDataAt(size_t idx) const;
    ...
    //!
    void addParticle(const Point3D& newPosition,
    ...
    //!
    void addParticles(const ConstArrayAccessor1<Point3D>& newPositions,
    ...
    //!
    [[nodiscard]] const PointNeighborSearcher3Ptr& neighborSearcher() const;
    ...
    //!
    [[nodiscard]] const std::vector<std::vector<size_t>>& neighborLists() const;
