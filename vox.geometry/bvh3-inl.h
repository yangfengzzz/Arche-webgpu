...
template <typename T>
inline NearestNeighborQueryResult3<T> Bvh3<T>::nearest(const Point3D &pt,
    ...
    while (node != nullptr) {
        if (node->isLeaf()) {
            ...
            // priority.
            Point3D closestLeft = left->bound.clamp(pt);
            Point3D closestRight = right->bound.clamp(pt);
...
template <typename T>
inline bool Bvh3<T>::intersects(const BoundingBox3D &box, const BoxIntersectionTestFunc3<T> &testFunc) const {
...
template <typename T>
inline bool Bvh3<T>::intersects(const Ray3D &ray, const RayIntersectionTestFunc3<T> &testFunc) const {
...
template <typename T>
inline void Bvh3<T>::forEachIntersectingItem(const BoundingBox3D &box,
...
template <typename T>
inline void Bvh3<T>::forEachIntersectingItem(const Ray3D &ray,
...
size_t Bvh3<T>::build(size_t nodeIndex, size_t *itemIndices, size_t nItems, size_t currentDepth) {
    ...
    Vector3D d = nodeBound.upper_corner - nodeBound.lower_corner;
