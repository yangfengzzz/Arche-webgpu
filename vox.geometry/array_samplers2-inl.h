...
NearestArraySampler2<T, R>::NearestArraySampler(const ConstArrayAccessor2<T> &accessor,
...
template <typename T, typename R>
NearestArraySampler2<T, R>::NearestArraySampler(const NearestArraySampler &other) {
...
template <typename T, typename R>
T NearestArraySampler2<T, R>::operator()(const Point2<R> &x) const {
    ssize_t i, j;
    R fx, fy;
...
template <typename T, typename R>
void NearestArraySampler2<T, R>::getCoordinate(const Point2<R> &x, Point2UI *index) const {
    ssize_t i, j;
    R fx, fy;
...
template <typename T, typename R>
std::function<T(const Point2<R> &)> NearestArraySampler2<T, R>::functor() const {
    NearestArraySampler sampler(*this);
    return std::bind(&NearestArraySampler::operator(), sampler, std::placeholders::_1);
}

template <typename T, typename R>
LinearArraySampler2<T, R>::LinearArraySampler(const ConstArrayAccessor2<T> &accessor,
                                              const Vector2<R> &gridSpacing,
                                              const Point2<R> &gridOrigin) {
    _gridSpacing = gridSpacing;
...
template <typename T, typename R>
LinearArraySampler2<T, R>::LinearArraySampler(const LinearArraySampler &other) {
    _gridSpacing = other._gridSpacing;
...
template <typename T, typename R>
T LinearArraySampler2<T, R>::operator()(const Point2<R> &x) const {
    ssize_t i, j;
    R fx, fy;
...
template <typename T, typename R>
void LinearArraySampler2<T, R>::getCoordinatesAndWeights(const Point2<R> &x,
...
template <typename T, typename R>
void LinearArraySampler2<T, R>::getCoordinatesAndGradientWeights(const Point2<R> &x,
                                                                 std::array<Point2UI, 4> *indices,
...
template <typename T, typename R>
std::function<T(const Point2<R> &)> LinearArraySampler2<T, R>::functor() const {
    LinearArraySampler sampler(*this);
    return std::bind(&LinearArraySampler::operator(), sampler, std::placeholders::_1);
}

template <typename T, typename R>
CubicArraySampler2<T, R>::CubicArraySampler(const ConstArrayAccessor2<T> &accessor,
                                            const Vector2<R> &gridSpacing,
                                            const Point2<R> &gridOrigin) {
    _gridSpacing = gridSpacing;
    _origin = gridOrigin;
...
template <typename T, typename R>
CubicArraySampler2<T, R>::CubicArraySampler(const CubicArraySampler &other) {
    _gridSpacing = other._gridSpacing;
    _origin = other._origin;
    _accessor = other._accessor;
}
...
T CubicArraySampler2<T, R>::operator()(const Point2<R> &x) const {
    ...
    T values[4];
    for (int n = 0; n < 4; ++n) {
        values[n] = monotonicCatmullRom(_accessor(is[0], js[n]), _accessor(is[1], js[n]), _accessor(is[2], js[n]),
                                        _accessor(is[3], js[n]), fx);
    }
...
template <typename T, typename R>
std::function<T(const Point2<R> &)> CubicArraySampler2<T, R>::functor() const {
