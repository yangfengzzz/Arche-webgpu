...
void ScalarGrid2::resize(size_t resolutionX,
                         size_t resolutionY,
                         double gridSpacingX,
                         double gridSpacingY,
                         double originX,
...
void ScalarGrid2::resize(const Size2 &resolution,
...
void ScalarGrid2::resize(const Vector2D &gridSpacing, const Point2D &origin) {
...
double ScalarGrid2::sample(const Point2D &x) const { return _sampler(x); }

std::function<double(const Point2D &)> ScalarGrid2::sampler() const { return _sampler; }
...
ScalarGrid2::DataPositionFunc ScalarGrid2::dataPosition() const {
    Point2D o = dataOrigin();
    return [this, o](size_t i, size_t j) -> Point2D { return o + gridSpacing() * Vector2D({i, j}); };
}

void ScalarGrid2::fill(double value, ExecutionPolicy policy) {
    parallelFor(
            kZeroSize, _data.width(), kZeroSize, _data.height(),
            [this, value](size_t i, size_t j) { _data(i, j) = value; }, policy);
}

void ScalarGrid2::fill(const std::function<double(const Point2D &)> &func, ExecutionPolicy policy) {
...
void ScalarGrid2::setData(const std::vector<double> &data) {
    VOX_ASSERT(dataSize().x * dataSize().y == data.size());
