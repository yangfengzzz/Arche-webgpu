...
public:
    ...
    typedef std::function<double(double, const Point3D&, double)> ScalarMapper;

    //! Maps to a vector value for given signed-dist, location, and old value.
    typedef std::function<Vector3D(double, const Point3D&, const Vector3D&)> VectorMapper;
