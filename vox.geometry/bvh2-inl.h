...
template <typename T>
inline NearestNeighborQueryResult2<T> Bvh2<T>::nearest(const Point2D &pt,
    ...
    while (node != nullptr) {
        if (node->isLeaf()) {
            ...
            // priority.
            Point2D closestLeft = left->bound.clamp(pt);
            Point2D closestRight = right->bound.clamp(pt);
...
template <typename T>
inline bool Bvh2<T>::intersects(const BoundingBox2D &box, const BoxIntersectionTestFunc2<T> &testFunc) const {
...
template <typename T>
inline bool Bvh2<T>::intersects(const Ray2D &ray, const RayIntersectionTestFunc2<T> &testFunc) const {
...
template <typename T>
inline void Bvh2<T>::forEachIntersectingItem(const BoundingBox2D &box,
...
template <typename T>
inline void Bvh2<T>::forEachIntersectingItem(const Ray2D &ray,
...
size_t Bvh2<T>::build(size_t nodeIndex, size_t *itemIndices, size_t nItems, size_t currentDepth) {
    ...
    Vector2D d = nodeBound.upper_corner - nodeBound.lower_corner;
    ...
    double pivot = 0.5 * (nodeBound.upper_corner[axis] + nodeBound.lower_corner[axis]);
    ...
    size_t d1 = build(_nodes[nodeIndex].child, itemIndices + midPoint, nItems - midPoint, currentDepth + 1);
...
size_t Bvh2<T>::qsplit(size_t *itemIndices, size_t numItems, double pivot, uint8_t axis) {
    ...
    for (size_t i = 0; i < numItems; ++i) {
        ...
        centroid = 0.5f * (b.lower_corner[axis] + b.upper_corner[axis]);
        if (centroid < pivot) {
