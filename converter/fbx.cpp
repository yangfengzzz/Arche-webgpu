...
FbxManagerInstance::FbxManagerInstance() : fbx_manager_(nullptr) {
    ...
    // Logs SDK version.
    vox::log::Log() << "FBX importer version " << fbx_manager_->GetVersion() << "." << std::endl;
}
...
void FbxSceneLoader::ImportScene(FbxImporter* _importer,
    ...
    if (!_initialized)  // Problem with the file to be imported.
    {
        ...
        vox::log::Err() << "FbxImporter initialization failed with error: " << error.Buffer() << std::endl;

        if (_importer->GetStatus().GetCode() == FbxStatus::eInvalidFileVersion) {
            vox::log::Err() << "FBX file version is " << major << "." << minor << "." << revision << "." << std::endl;
        }
    } else {
        if (_importer->IsFBX()) {
            vox::log::Log() << "FBX file version is " << major << "." << minor << "." << revision << "." << std::endl;
        }
...
bool FbxSystemConverter::ConvertTransform(const FbxAMatrix& _m, math::Transform* _transform) const {
    assert(_transform);
