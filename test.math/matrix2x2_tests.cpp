...
TEST(Matrix2x2, SetMethods) {
    ...
    mat.set(0.0);
    mat.setRow(0, Vector2D(1.0, 3.0));
    mat.setRow(1, Vector2D(2.0, 4.0));
    for (int i = 0; i < 4; ++i) {
        EXPECT_DOUBLE_EQ(static_cast<double>(i + 1), mat[i]);
    }

    mat.set(0.0);
    mat.setColumn(0, Vector2D(1.0, 2.0));
    mat.setColumn(1, Vector2D(3.0, 4.0));
    for (int i = 0; i < 4; ++i) {
        EXPECT_DOUBLE_EQ(static_cast<double>(i + 1), mat[i]);
    }
}
...
TEST(Matrix2x2, BinaryOperators) {
    ...
    EXPECT_TRUE(vec.isSimilar(Vector2D(-8.0, 5.0)));
...
TEST(Matrix2x2, AugmentedOperators) {
    ...
    EXPECT_TRUE(mat.isSimilar(Matrix2x2D(-8.0, 5.0, -20.0, 13.0)));
...
TEST(Matrix2x2, GetterOperatorOverloadings) {
    ...
    EXPECT_DOUBLE_EQ(3.0, mat(1, 0));
    EXPECT_DOUBLE_EQ(-2.0, mat(0, 1));
