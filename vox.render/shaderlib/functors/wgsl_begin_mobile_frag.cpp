...
void WGSLBeginMobileFrag::operator()(std::string& source, const ShaderMacroCollection& macros) {
    ...
    if (macros.contains(HAS_EMISSIVE_TEXTURE)) {
        source += fmt::format(
                "var emissiveTextureColor = textureSample(u_emissiveTexture, u_emissiveSampler, {}.v_uv);\n", _input);
    ...
    if (macros.contains(HAS_SPECULAR_TEXTURE)) {
        source += fmt::format(
                "var specularTextureColor = textureSample(u_specularTexture, u_specularSampler, {}.v_uv);\n", _input);
