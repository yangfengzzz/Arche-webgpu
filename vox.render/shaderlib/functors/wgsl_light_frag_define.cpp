...
namespace vox {
void WGSLLightFragDefine::operator()(WGSLEncoder& encoder, const ShaderMacroCollection& macros) {
    ...
    if (macros.contains(HAS_DIFFUSE_ENV)) {
        encoder.addSampledTextureBinding("u_env_diffuseTexture", TextureType::TextureCubef32, "u_env_diffuseSampler",
                                         SamplerType::Sampler);
    }

    if (macros.contains(HAS_SPECULAR_ENV)) {
        encoder.addSampledTextureBinding("u_env_specularTexture", TextureType::TextureCubef32, "u_env_specularSampler",
                                         SamplerType::Sampler);
    }
}
