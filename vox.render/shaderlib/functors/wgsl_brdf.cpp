...
namespace vox {
WGSLBRDF::WGSLBRDF(const std::string& outputStructName) :
_outputStructName(outputStructName) {
    _paramName = "in";
}

void WGSLBRDF::setParamName(const std::string& name) {
    _paramName = name;
}

const std::string& WGSLBRDF::paramName() const {
    return _paramName;
}

void WGSLBRDF::operator()(WGSLEncoder& encoder,
    ...
    function = "fn BRDF_Specular_GGX(incidentDirection:vec3<f32>, geometry:GeometricContext, specularColor:vec3<f32>, roughness:f32)->vec3<f32> {\n";
    function += "\n";
