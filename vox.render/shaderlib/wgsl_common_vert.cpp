...
void WGSLCommonVert::operator()(WGSLEncoder& encoder, const ShaderMacroCollection& macros) {
    if (macros.contains("HAS_UV")) {
        encoder.addInoutType("Input", Attributes::UV_0, UniformType::Vec2f32);
    }
    
    if (macros.contains("HAS_SKIN")) {
        encoder.addInoutType("Input", Attributes::Joints_0, UniformType::Vec4f32);
        encoder.addInoutType("Input", Attributes::Weights_0, UniformType::Vec4f32);
    }
}
