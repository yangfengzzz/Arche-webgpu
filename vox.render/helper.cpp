...
bool ContainsString(const std::string& src, const std::string& dst) { return src.find(dst) != std::string::npos; }
...
bool StringEndsWith(const std::string& src, const std::string& tar) {
    // https://stackoverflow.com/a/42844629/1255535
    return src.size() >= tar.size() && 0 == src.compare(src.size() - tar.size(), tar.size(), tar);
}
