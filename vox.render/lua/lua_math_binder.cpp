...
void LuaMathsBinder::bindMaths(sol::state &luaState) {
    // MARK: - Vector2
    luaState.new_usertype<Vector2F>(
            "Vector2F",
            /* Constructors */
            ...
            sol::meta_function::equal_to, &Vector2F::operator==, sol::meta_function::to_string,
            ...
            /* Methods */
            "length", &Vector2F::length, "lengthSquared", &Vector2F::lengthSquared, "distanceTo", &Vector2F::distanceTo,
            "distanceSquaredTo", &Vector2F::distanceSquaredTo, "dot", &Vector2F::dot, "cross", &Vector2F::cross,
    ...
    luaState.new_usertype<Vector3F>(
            ...
            sol::meta_function::equal_to, &Vector3F::operator==, sol::meta_function::to_string,
            ...
            /* Methods */
            "length", &Vector3F::length, "lengthSquared", &Vector3F::lengthSquared, "distanceTo", &Vector3F::distanceTo,
            "distanceSquaredTo", &Vector3F::distanceSquaredTo, "dot", &Vector3F::dot, "cross", &Vector3F::cross,
            "normalize", &Vector3F::normalize);

    // MARK: - Vector4
    luaState.new_usertype<Vector4F>(
            "Vector4F",
            /* Constructors */
            ...
            sol::meta_function::equal_to, &Vector4F::operator==, sol::meta_function::to_string,
            [](const Vector4F &target) {
                return "(" + std::to_string(target.x) + "," + std::to_string(target.y) + "," +
                       std::to_string(target.z) + "," + std::to_string(target.w) + ")";
            },
            /* Variables */
            "x", &Vector4F::x, "y", &Vector4F::y, "z", &Vector4F::z, "w", &Vector4F::w,

            /* Data */
            "zero", []() { return zero<Vector4F>(); },

            /* Methods */
            "length", &Vector4F::length, "lengthSquared", &Vector4F::lengthSquared, "distanceTo", &Vector4F::distanceTo,
            "distanceSquaredTo", &Vector4F::distanceSquaredTo, "dot", &Vector4F::dot, "normalize",
            &Vector4F::normalize);

    // MARK: - Point3
    luaState.new_usertype<Point3F>(
            "Point3F",
            /* Constructors */
            ...
            sol::meta_function::equal_to, &Point3F::operator==, sol::meta_function::to_string,
            [](const Point3F &target) {
                return "(" + std::to_string(target.x) + "," + std::to_string(target.y) + "," +
                       std::to_string(target.z) + ")";
            },
            /* Variables */
            "x", &Point3F::x, "y", &Point3F::y, "z", &Point3F::z,

            /* Methods */
            "distanceTo", &Point3F::distanceTo, "distanceSquaredTo", &Point3F::distanceSquaredTo, "dot", &Point3F::dot,
            "cross", &Point3F::cross);

    // MARK: - Matrix3x3
    luaState.new_usertype<Matrix3x3F>(
            "Matrix3x3F",
            /* Constructors */
            ...
            sol::overload(sol::resolve<Matrix3x3F(const Matrix3x3F &a, const Matrix3x3F &b)>(&operator+),
            ...
            sol::overload(sol::resolve<Matrix3x3F(const Matrix3x3F &a, float b)>(&operator*),
                          sol::resolve<Matrix3x3F(float a, const Matrix3x3F &b)>(&operator*),
            ...
            /* Data */
            "makeZero", &Matrix3x3F::makeZero, "makeIdentity", &Matrix3x3F::makeIdentity, "makeScaleMatrix",
            sol::overload(sol::resolve<Matrix3x3F(float a, float b, float c)>(&Matrix3x3F::makeScaleMatrix),
    ...
    // MARK: - Matrix4x4
    luaState.new_usertype<Matrix4x4F>(
            "Matrix4x4F",
            /* Constructors */
            sol::constructors<Matrix4x4F(), Matrix4x4F(float),
                              Matrix3x3F(float, float, float, float, float, float, float, float, float, float, float,
                                         float, float, float, float, float)>(),

            /* Operators */
            sol::meta_function::unary_minus, sol::resolve<Matrix4x4F(const Matrix4x4F &a)>(&operator-),
            sol::meta_function::addition,
            sol::overload(sol::resolve<Matrix4x4F(const Matrix4x4F &a, const Matrix4x4F &b)>(&operator+),
            ...
            sol::overload(sol::resolve<Matrix4x4F(const Matrix4x4F &a, const Matrix4x4F &b)>(&operator-),
                          ...
                          sol::resolve<Matrix4x4F(float a, const Matrix4x4F &b)>(&operator-)),
            sol::meta_function::multiplication,
            sol::overload(sol::resolve<Matrix4x4F(const Matrix4x4F &a, float b)>(&operator*),
                          sol::resolve<Matrix4x4F(float a, const Matrix4x4F &b)>(&operator*),
            ...
            /* Data */
            "makeZero", &Matrix4x4F::makeZero, "makeIdentity", &Matrix4x4F::makeIdentity);

    // MARK: - Quaternion
    luaState.new_usertype<QuaternionF>(
            "QuaternionF",
            /* Constructors */
            sol::constructors<QuaternionF(), QuaternionF(float, float, float, float),
                              QuaternionF(const Vector3F &, float), QuaternionF(const Vector3F &, const Vector3F &),
                              QuaternionF(const Vector3F &, const Vector3F &, const Vector3F &)>(),

            /* Operators */
            sol::meta_function::addition,
            sol::resolve<QuaternionF(const QuaternionF &a, const QuaternionF &b)>(&operator+),
            sol::meta_function::multiplication, sol::resolve<QuaternionF(const QuaternionF &a, float b)>(&operator*),
            sol::meta_function::equal_to, &QuaternionF::operator==, sol::meta_function::to_string,
            [](const QuaternionF &target) {
                return "(" + std::to_string(target.x) + "," + std::to_string(target.y) + "," +
                       std::to_string(target.z) + "," + std::to_string(target.w) + ")";
            },
            /* Variables */
            "x", &QuaternionF::x, "y", &QuaternionF::y, "z", &QuaternionF::z, "w", &QuaternionF::w,

            /* Data */
            "makeIdentity", &QuaternionF::makeIdentity, "makeRotationEuler", &QuaternionF::makeRotationEuler,
            "makeRotationYawPitchRoll", &QuaternionF::makeRotationYawPitchRoll, "makeRotationX",
            &QuaternionF::makeRotationX, "makeRotationY", &QuaternionF::makeRotationY, "makeRotationZ",
            &QuaternionF::makeRotationZ,

            /* Methods */
            "length", &QuaternionF::length, "lengthSquared", &QuaternionF::lengthSquared, "dot", &QuaternionF::dot,
            "normalize", &QuaternionF::normalize, "slerp",
            sol::resolve<QuaternionF(const QuaternionF &, const QuaternionF &, float)>(&slerp));
}
