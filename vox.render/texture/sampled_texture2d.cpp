...
SampledTexture2D::SampledTexture2D(wgpu::Device& device) : SampledTexture(device) {}
...
void SampledTexture2D::setImageSource(const Image* image) {
    for (const auto& mipmap : image->mipmaps()) {
        ...
        wgpu::TextureDataLayout textureLayout = _createTextureDataLayout(
                mipmap.offset, bytesPerPixel(image->format()) * mipmap.extent.width, wgpu::kCopyStrideUndefined);
        wgpu::Extent3D copySize = {mipmap.extent.width, mipmap.extent.height, 1};
        _device.GetQueue().WriteTexture(&imageCopyTexture, image->data().data(), image->data().size(), &textureLayout,
                                        &copySize);
    }
}
