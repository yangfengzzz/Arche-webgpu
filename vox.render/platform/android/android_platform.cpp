...
JNIEXPORT void JNICALL Java_com_khronos_vulkan_1samples_SampleLauncherActivity_initFilePath(JNIEnv *env,
    ...
    vox::Platform::set_external_storage_directory(std::string(external_dir_cstr) + "/");
    env->ReleaseStringUTFChars(external_dir, external_dir_cstr);

    const char *temp_dir_cstr = env->GetStringUTFChars(temp_dir, 0);
    vox::Platform::set_temp_directory(std::string(temp_dir_cstr) + "/");
    env->ReleaseStringUTFChars(temp_dir, temp_dir_cstr);
}

JNIEXPORT jobjectArray JNICALL Java_com_khronos_vulkan_1samples_SampleLauncherActivity_getSamples(JNIEnv *env,
                                                                                                  jobject thiz) {
    ...
    for (int sample_index = 0; sample_index < sample_list.
        ...
        jobjectArray j_tag_list = env->NewObjectArray(sample_info->tags.size(), env->FindClass("java/lang/String"),
                                                      env->NewStringUTF(""));
        for (int tag_index = 0; tag_index < sample_info->tags.

                                            size();

             ++tag_index) {
            env->SetObjectArrayElement(j_tag_list, tag_index,
                                       env->NewStringUTF(sample_info->tags[tag_index].
...
JNIEXPORT void JNICALL Java_com_khronos_vulkan_1samples_SampleLauncherActivity_sendArgumentsToPlatform(
        JNIEnv *env, jobject thiz, jobjectArray arg_strings) {
    ...
    vox::Platform::set_arguments(args);
}
}
...
int32_t on_input_event(android_app *app, AInputEvent *input_event) {
    ...
    if (event_source == AINPUT_SOURCE_KEYBOARD) {
        ...
        platform->input_event(KeyInputEvent{translate_key_code(key_code), translate_key_action(action)});
        ...
        platform->input_event(MouseButtonInputEvent{translate_mouse_button(0), translate_mouse_action(action), x, y});
        ...
        platform->input_event(TouchInputEvent{pointer_id, pointer_count, translate_touch_action(action), x, y});
    } else {
        return 0;
    }
...
AndroidPlatform::AndroidPlatform(android_app *app) : app{app} {}
...
const char *AndroidPlatform::get_surface_extension() { return VK_KHR_ANDROID_SURFACE_EXTENSION_NAME; }
...
void AndroidPlatform::set_surface_ready() { surface_ready = true; }

ANativeActivity *AndroidPlatform::get_activity() { return app->activity; }

android_app *AndroidPlatform::get_android_app() { return app; }

std::unique_ptr<RenderContext> AndroidPlatform::create_render_context(
