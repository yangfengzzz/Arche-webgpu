...
void ParticleRenderer::_allocBuffer() {
    ...
    /* Atomic */
    _atomicBuffer[0] =
            std::make_unique<Buffer>(device, sizeof(uint32_t), wgpu::BufferUsage::Storage | wgpu::BufferUsage::CopySrc);
    _atomicBuffer[1] =
            std::make_unique<Buffer>(device, sizeof(uint32_t), wgpu::BufferUsage::Storage | wgpu::BufferUsage::CopySrc);
    shaderData.setBufferFunctor(_readAtomicBufferProp, [this]() -> Buffer { return *_atomicBuffer[_read]; });
    shaderData.setBufferFunctor(_writeAtomicBufferProp, [this]() -> Buffer { return *_atomicBuffer[_write]; });
    ...
    _appendConsumeBuffer[1] = std::make_unique<Buffer>(device, sizeof(TParticle) * numParticles,
                                                       wgpu::BufferUsage::Storage | wgpu::BufferUsage::Vertex);
    shaderData.setBufferFunctor(_readConsumeBufferProp, [this]() -> Buffer { return *_appendConsumeBuffer[_read]; });
    shaderData.setBufferFunctor(_writeConsumeBufferProp, [this]() -> Buffer { return *_appendConsumeBuffer[_write]; });
    ...
    _dpBuffer = std::make_unique<Buffer>(device, sizeof(float) * sort_buffer_max_count, wgpu::BufferUsage::Storage);
    _sortIndicesBuffer =
            std::make_unique<Buffer>(device, sizeof(uint32_t) * sort_buffer_max_count * 2, wgpu::BufferUsage::Storage);
    shaderData.setBufferFunctor(
            _dpBufferProp, [this]() -> auto{ return *_dpBuffer; });
    shaderData.setBufferFunctor(
            _sortIndicesBufferProp, [this]() -> auto{ return *_sortIndicesBuffer; });
}
...
void ParticleRenderer::_updateBounds(BoundingBox3F &worldBounds) {
    worldBounds.lower_corner.x = -std::numeric_limits<float>::max();
    worldBounds.lower_corner.y = -std::numeric_limits<float>::max();
    worldBounds.lower_corner.z = -std::numeric_limits<float>::max();
    worldBounds.upper_corner.x = std::numeric_limits<float>::max();
    worldBounds.upper_corner.y = std::numeric_limits<float>::max();
    worldBounds.upper_corner.z = std::numeric_limits<float>::max();
}
...
const uint32_t ParticleRenderer::numAliveParticles() const { return _numAliveParticles; }

ParticleMaterial &ParticleRenderer::material() { return *_material; }

float ParticleRenderer::timeStep() const { return _simulationData.timeStep; }
...
float ParticleRenderer::bboxSize() const { return _simulationData.bboxSize; }
...
float ParticleRenderer::scatteringFactor() const { return _simulationData.scatteringFactor; }
...
float ParticleRenderer::vectorFieldFactor() const { return _simulationData.vectorFieldFactor; }
...
float ParticleRenderer::curlNoiseFactor() const { return _simulationData.curlNoiseFactor; }
...
float ParticleRenderer::curlNoiseScale() const { return _simulationData.curlNoiseScale; }
...
float ParticleRenderer::velocityFactor() const { return _simulationData.velocityFactor; }
...
// MARK: - Emitter
uint32_t ParticleRenderer::emitCount() const { return _emitterData.emitCount; }
...
ParticleRenderer::EmitterType ParticleRenderer::emitterType() const { return _emitterData.emitterType; }
...
Vector3F ParticleRenderer::emitterPosition() const { return _emitterData.emitterPosition; }
...
Vector3F ParticleRenderer::emitterDirection() const { return _emitterData.emitterDirection; }
...
float ParticleRenderer::emitterRadius() const { return _emitterData.emitterRadius; }
...
float ParticleRenderer::particleMinAge() const { return _emitterData.particleMinAge; }
...
float ParticleRenderer::particleMaxAge() const { return _emitterData.particleMaxAge; }
