...
void WGSLParticleEmission::_createShaderSource(size_t hash, const ShaderMacroCollection& macros) {
    ...
    _bindGroupInfo.clear();
    {
        ...
        encoder.addEntry(
                {ParticleManager::PARTICLES_KERNEL_GROUP_WIDTH, 1, 1},
                [&](std::string& source) {
                    ...
                    source +=
                            " // Random vector.\n"
                            "    let rn = vec3<f32>(u_randomBuffer[global_id.x / 256u].x, u_randomBuffer[global_id.x / "
                            "256u].y, u_randomBuffer[global_id.x / 256u].z);\n"
                            "    \n"
                            "    // Position\n"
                            "    var pos = u_emitterData.emitterPosition;\n"
                            "    if (u_emitterData.emitterType == 1u) {\n"
                            "        //pos += disk_distribution(uEmitterRadius, rn.xy);\n"
                            "        pos = pos + disk_even_distribution(u_emitterData.emitterRadius, global_id.x, "
                            "u_emitterData.emitCount);\n"
                            ...
                            "    \n"
                            "    let age = mix( u_emitterData.particleMinAge, u_emitterData.particleMaxAge, "
                            "u_randomBuffer[global_id.x / 256u].w);\n"
                            "\n"
                            "    // Emit particle id.\n"
