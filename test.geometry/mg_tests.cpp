...
void rest(const typename BlasType::VectorType& finer, typename BlasType::VectorType* coarser) {
    ...
    coarser->parallelForEachIndex([&](size_t i) {
        ...
        (*coarser)[i] = 0.125 * (finer[_2im1] + 3.0 * finer[2 * i] + 3.0 * finer[2 * i + 1] + finer[_2ip2]);
    });
}

void corr(const typename BlasType::VectorType& coarser, typename BlasType::VectorType* finer) {
    size_t n = coarser.size();
