...
TEST(VolumeGridEmitter3, Velocity) {
    auto sphere = Sphere3::builder().withCenter({0.5, 0.75, 0.5}).withRadius(0.15).makeShared();

    auto emitter = VolumeGridEmitter3::builder().withSourceRegion(sphere).makeShared();
    ...
    auto mapper = [](double sdf, const Point3D& pt, const Vector3D& oldVal) {
        if (sdf < 0.0) {
