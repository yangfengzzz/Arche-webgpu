...
#include "vox.geometry/list_query_engine2.h"
...
TEST(ListQueryEngine2, BoxIntersection) {
    ...
    auto testFunc = [](const Point2D& pt, const BoundingBox2D& bbox) { return bbox.contains(pt); };
...
TEST(ListQueryEngine2, ClosestIntersection) {
    ...
    for (i = 0; i < numSamples / 2; ++i) {
        ...
        if (ansInts.item != nullptr && engInts.item != nullptr) {
            EXPECT_VECTOR2_EQ(ansInts.item->lower_corner, engInts.item->lower_corner);
            EXPECT_VECTOR2_EQ(ansInts.item->upper_corner, engInts.item->upper_corner);
        } else {
