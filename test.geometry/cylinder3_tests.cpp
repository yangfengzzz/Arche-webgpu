...
#include "vox.geometry/cylinder3.h"
...
TEST(Cylinder3, Constructors) {
    ...
    EXPECT_DOUBLE_EQ(-1.0, cyl1.boundingBox().lower_corner.x);
    EXPECT_DOUBLE_EQ(-0.5, cyl1.boundingBox().lower_corner.y);
    EXPECT_DOUBLE_EQ(-1.0, cyl1.boundingBox().lower_corner.z);
    EXPECT_DOUBLE_EQ(1.0, cyl1.boundingBox().upper_corner.x);
    EXPECT_DOUBLE_EQ(0.5, cyl1.boundingBox().upper_corner.y);
    EXPECT_DOUBLE_EQ(1.0, cyl1.boundingBox().upper_corner.z);

    Cylinder3 cyl2(Point3D(1, 2, 3), 4.0, 5.0);
    ...
    EXPECT_DOUBLE_EQ(-3.0, cyl2.boundingBox().lower_corner.x);
    EXPECT_DOUBLE_EQ(-0.5, cyl2.boundingBox().lower_corner.y);
    EXPECT_DOUBLE_EQ(-1.0, cyl2.boundingBox().lower_corner.z);
    EXPECT_DOUBLE_EQ(5.0, cyl2.boundingBox().upper_corner.x);
    EXPECT_DOUBLE_EQ(4.5, cyl2.boundingBox().upper_corner.y);
    EXPECT_DOUBLE_EQ(7.0, cyl2.boundingBox().upper_corner.z);
    ...
    EXPECT_DOUBLE_EQ(-3.0, cyl3.boundingBox().lower_corner.x);
    EXPECT_DOUBLE_EQ(-0.5, cyl3.boundingBox().lower_corner.y);
    EXPECT_DOUBLE_EQ(-1.0, cyl3.boundingBox().lower_corner.z);
    EXPECT_DOUBLE_EQ(5.0, cyl3.boundingBox().upper_corner.x);
    EXPECT_DOUBLE_EQ(4.5, cyl3.boundingBox().upper_corner.y);
    EXPECT_DOUBLE_EQ(7.0, cyl3.boundingBox().upper_corner.z);
}

TEST(Cylinder3, ClosestPoint) {
    Cylinder3 cyl(Point3D(1, 2, 3), 4.0, 6.0);

    Point3D result1 = cyl.closestPoint({7, 2, 3});
    EXPECT_DOUBLE_EQ(5.0, result1.x);
    EXPECT_DOUBLE_EQ(2.0, result1.y);
    EXPECT_DOUBLE_EQ(3.0, result1.z);

    Point3D result2 = cyl.closestPoint({1, 6, 2});
    EXPECT_DOUBLE_EQ(1.0, result2.x);
    EXPECT_DOUBLE_EQ(5.0, result2.y);
    EXPECT_DOUBLE_EQ(2.0, result2.z);

    Point3D result3 = cyl.closestPoint({6, -5, 3});
...
TEST(Cylinder3, ClosestIntersection) {
    Cylinder3 cyl(Point3D(1, 2, 3), 4.0, 6.0, Transform3(Vector3D(1, -2, 3), QuaternionD()));
...
TEST(Cylinder3, BoundingBox) {
    ...
    EXPECT_DOUBLE_EQ(-3.0, bbox.lower_corner.x);
    EXPECT_DOUBLE_EQ(-1.0, bbox.lower_corner.y);
    EXPECT_DOUBLE_EQ(-1.0, bbox.lower_corner.z);
    EXPECT_DOUBLE_EQ(5.0, bbox.upper_corner.x);
    EXPECT_DOUBLE_EQ(5.0, bbox.upper_corner.y);
    EXPECT_DOUBLE_EQ(7.0, bbox.upper_corner.z);
}
...
TEST(Cylinder3, Builder) {
    Cylinder3 cyl2 = Cylinder3::builder().withCenter({1, 2, 3}).withRadius(4.0).withHeight(5.0).build();
