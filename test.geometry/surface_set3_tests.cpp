...
TEST(SurfaceSet3, Constructors) {
    ...
    auto sph1 = Sphere3::builder().withRadius(1.0).withCenter({0, 0, 0}).makeShared();
    auto sph2 = Sphere3::builder().withRadius(0.5).withCenter({0, 3, 2}).makeShared();
    auto sph3 = Sphere3::builder().withRadius(0.25).withCenter({-2, 0, 0}).makeShared();
    SurfaceSet3 sset2({sph1, sph2, sph3}, Transform3D(), false);
    EXPECT_EQ(3u, sset2.numberOfSurfaces());
    EXPECT_EQ(sph1->radius, std::dynamic_pointer_cast<Sphere3>(sset2.surfaceAt(0))->radius);
    EXPECT_EQ(sph2->radius, std::dynamic_pointer_cast<Sphere3>(sset2.surfaceAt(1))->radius);
    EXPECT_EQ(sph3->radius, std::dynamic_pointer_cast<Sphere3>(sset2.surfaceAt(2))->radius);
    EXPECT_EQ(Vector3D(), sset2.transform.translation());
    EXPECT_EQ(QuaternionD(), sset2.transform.orientation());

    SurfaceSet3 sset3({sph1, sph2, sph3}, Transform3(Vector3D(1, 2, 3), QuaternionD({1, 0, 0}, 0.5)), false);
    ...
    EXPECT_EQ(sph1->radius, std::dynamic_pointer_cast<Sphere3>(sset4.surfaceAt(0))->radius);
    EXPECT_EQ(sph2->radius, std::dynamic_pointer_cast<Sphere3>(sset4.surfaceAt(1))->radius);
    EXPECT_EQ(sph3->radius, std::dynamic_pointer_cast<Sphere3>(sset4.surfaceAt(2))->radius);
...
TEST(SurfaceSet3, AddSurface) {
    ...
    EXPECT_EQ(sph1->radius, std::dynamic_pointer_cast<Sphere3>(sset1.surfaceAt(0))->radius);
    EXPECT_EQ(sph2->radius, std::dynamic_pointer_cast<Sphere3>(sset1.surfaceAt(1))->radius);
    EXPECT_EQ(sph3->radius, std::dynamic_pointer_cast<Sphere3>(sset1.surfaceAt(2))->radius);
...
TEST(SurfaceSet3, ClosestPoint) {
    ...
    // Test empty set
    Point3D emptyPoint = sset1.closestPoint({1.0, 2.0, 3.0});
    ...
    for (size_t i = 0; i < numSamples / 2; ++i) {
        auto sph = Sphere3::builder().withRadius(0.01).withCenter(getSamplePoints3()[i]).makeShared();
    ...
    for (size_t i = 0; i < numSamples / 2; ++i) {
        auto sph = Sphere3::builder()
                           ...
                           .withCenter({0, 0, 0})
                           .withTranslation(
                                   Vector3D(getSamplePoints3()[i].x, getSamplePoints3()[i].y, getSamplePoints3()[i].z))
                           .makeShared();
...
TEST(SurfaceSet3, MixedBoundTypes) {
    BoundingBox3D domain(Point3D(), Point3D(1, 2, 1));
    ...
    auto surfaceSet = SurfaceSet3::builder().withSurfaces({plane, sphere}).makeShared();

    EXPECT_FALSE(surfaceSet->isBounded());

    auto cp = surfaceSet->closestPoint(Point3D(0.5, 0.4, 0.5));
...
TEST(SurfaceSet3, IsValidGeometry) {
    ...
    auto surfaceSet2 = SurfaceSet3::builder().withSurfaces({plane, sphere}).makeShared();
...
TEST(SurfaceSet3, IsInside) {
    ...
    EXPECT_TRUE(surfaceSet->isInside(Point3D(0.5, 0.25, 0.5) + offset));
    EXPECT_TRUE(surfaceSet->isInside(Point3D(0.5, 1.0, 0.5) + offset));
    EXPECT_FALSE(surfaceSet->isInside(Point3D(0.5, 1.5, 0.5) + offset));
}

TEST(SurfaceSet3, UpdateQueryEngine) {
    auto sphere = Sphere3::builder().withCenter({-1.0, 1.0, 2.0}).withRadius(0.5).makeShared();
    ...
    EXPECT_BOUNDING_BOX2_EQ(BoundingBox3D({-0.5, 2.5, 0.5}, {0.5, 3.5, 1.5}), bbox1);

    surfaceSet->transform = Transform3({3.0, -4.0, 7.0}, QuaternionD());
    surfaceSet->updateQueryEngine();
    auto bbox2 = surfaceSet->boundingBox();
    EXPECT_BOUNDING_BOX2_EQ(BoundingBox3D({1.5, -3.5, 4.5}, {2.5, -2.5, 5.5}), bbox2);

    sphere->transform = Transform3({-6.0, 9.0, 2.0}, QuaternionD());
    surfaceSet->updateQueryEngine();
    auto bbox3 = surfaceSet->boundingBox();
    EXPECT_BOUNDING_BOX2_EQ(BoundingBox3D({-4.5, 5.5, 10.5}, {-3.5, 6.5, 11.5}), bbox3);
    ...
    EXPECT_BOUNDING_BOX2_EQ(BoundingBox3D({-4.5, 5.5, 10.5}, {-3.5, 6.5, 11.5}), bbox4);
}
