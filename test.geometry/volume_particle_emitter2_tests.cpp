...
TEST(VolumeParticleEmitter2, Constructors) {
    auto sphere = std::make_shared<SurfaceToImplicit2>(std::make_shared<Sphere2>(Point2D(1.0, 2.0), 3.0));

    BoundingBox2D region({0.0, 0.0}, {3.0, 3.0});

    VolumeParticleEmitter2 emitter(sphere, region, 0.1, {-1.0, 0.5}, {0.0, 0.0}, 0.0, 30, 0.01, false, true);
...
TEST(VolumeParticleEmitter2, Emit) {
    ...
    VolumeParticleEmitter2 emitter(sphere, box, 0.3, {-1.0, 0.5}, {3.0, 4.0}, 5.0, 30, 0.0, false, false);
    ...
    for (size_t i = 0; i < particles->numberOfParticles(); ++i) {
        EXPECT_GE(3.0, (pos[i] - Point2D(1.0, 2.0)).length());
...
TEST(VolumeParticleEmitter2, Builder) {
    auto sphere = std::make_shared<Sphere2>(Point2D(1.0, 2.0), 3.0);
