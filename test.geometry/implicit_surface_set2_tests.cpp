...
TEST(ImplicitSurfaceSet2, SurfaceAt) {
    ...
    EXPECT_EQ(std::dynamic_pointer_cast<Surface2>(box1), implicitSurfaceAt0->surface());
    EXPECT_EQ(std::dynamic_pointer_cast<Surface2>(box2), implicitSurfaceAt1->surface());
}
...
TEST(ImplicitSurfaceSet2, ClosestPoint) {
    ...
    Point2D emptyPoint = sset->closestPoint({1.0, 2.0});
    ...
    Point2D setPoint = sset->closestPoint(pt);
...
TEST(ImplicitSurfaceSet2, Intersects) {
    ...
    bool result0 = sset.intersects(Ray2D(Point2D(1, 4), Vector2D(-1, -1).normalized()));
    EXPECT_TRUE(result0);

    bool result1 = sset.intersects(Ray2D(Point2D(1, 2.5), Vector2D(-1, -1).normalized()));
    EXPECT_TRUE(result1);

    bool result2 = sset.intersects(Ray2D(Point2D(1, 1), Vector2D(-1, -1).normalized()));
...
TEST(ImplicitSurfaceSet2, ClosestIntersection) {
    ...
    SurfaceRayIntersection2 result0 = sset.closestIntersection(Ray2D(Point2D(1, 4), Vector2D(-1, -1).normalized()));
    EXPECT_TRUE(result0.isIntersecting);
    EXPECT_DOUBLE_EQ(std::sqrt(2), result0.distance);
    EXPECT_EQ(Point2D(0, 3), result0.point);

    SurfaceRayIntersection2 result1 = sset.closestIntersection(Ray2D(Point2D(1, 2.5), Vector2D(-1, -1).normalized()));
    EXPECT_TRUE(result1.isIntersecting);
    EXPECT_DOUBLE_EQ(std::sqrt(0.5), result1.distance);
    EXPECT_EQ(Point2D(0.5, 2), result1.point);

    SurfaceRayIntersection2 result2 = sset.closestIntersection(Ray2D(Point2D(1, 1), Vector2D(-1, -1).normalized()));
...
TEST(ImplicitSurfaceSet2, BoundingBox) {
    ...
    EXPECT_DOUBLE_EQ(0.0, bbox.lower_corner.y);
...
TEST(ImplicitSurfaceSet2, MixedBoundTypes) {
    ...
    auto surfaceSet = ImplicitSurfaceSet2::builder().withExplicitSurfaces({plane, sphere}).makeShared();
...
TEST(ImplicitSurfaceSet2, UpdateQueryEngine) {
    ...
    auto surfaceSet = ImplicitSurfaceSet2::builder()
                              ...
                              .withTransform(Transform2D({1.0, 2.0}, 0.0))
                              .makeShared();
