...
TEST(Quadtree, Nearest) {
    ...
    auto overlapsFunc = [](const Point2D& pt, const BoundingBox2D& bbox) { return bbox.contains(pt); };

    auto distanceFunc = [](const Point2D& a, const Point2D& b) { return a.distanceTo(b); };

    // Single point
    quadtree.build({Point2D(0.2, 0.7)}, BoundingBox2D({0, 0}, {0.9, 1.0}), overlapsFunc, 3);

    EXPECT_EQ(3u, quadtree.maxDepth());
    EXPECT_VECTOR2_EQ(Vector2D(0, 0), quadtree.boundingBox().lower_corner);
    EXPECT_VECTOR2_EQ(Vector2D(1, 1), quadtree.boundingBox().upper_corner);
    ...
    std::vector<Point2D> points(getSamplePoints2(), getSamplePoints2() + numSamples);
...
TEST(Quadtree, ForEachOverlappingItems) {
    ...
    size_t measured = 0;
    quadtree.forEachIntersectingItem(testBox, overlapsFunc, [&](const Point2D& pt) {
...
TEST(Quadtree, RayIntersects) {
    ...
    auto overlapsFunc = [](const BoundingBox2D& a, const BoundingBox2D& bbox) { return bbox.overlaps(a); };

    auto intersectsFunc = [](const BoundingBox2D& a, const Ray2D& ray) { return a.intersects(ray); };
