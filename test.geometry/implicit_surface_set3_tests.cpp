...
#include "vox.geometry/implicit_surface_set3.h"
...
TEST(ImplicitSurfaceSet3, SurfaceAt) {
    ...
    EXPECT_EQ(std::dynamic_pointer_cast<Surface3>(box1), implicitSurfaceAt0->surface());
    EXPECT_EQ(std::dynamic_pointer_cast<Surface3>(box2), implicitSurfaceAt1->surface());
}
...
TEST(ImplicitSurfaceSet3, ClosestPoint) {
    ...
    Point3D boxPoint = box->closestPoint(pt);
    Point3D setPoint = sset->closestPoint(pt);
...
TEST(ImplicitSurfaceSet3, Intersects) {
    ...
    bool result0 = sset.intersects(Ray3D(Point3D(1, 4, 5), Vector3D(-1, -1, -1).normalized()));
    EXPECT_TRUE(result0);

    bool result1 = sset.intersects(Ray3D(Point3D(1, 2.5, 6), Vector3D(-1, -1, 1).normalized()));
    EXPECT_TRUE(result1);

    bool result2 = sset.intersects(Ray3D(Point3D(1, 1, 2), Vector3D(-1, -1, -1).normalized()));
...
TEST(ImplicitSurfaceSet3, ClosestIntersection) {
    ...
    SurfaceRayIntersection3 result0 =
            sset.closestIntersection(Ray3D(Point3D(1, 4, 5), Vector3D(-1, -1, -1).normalized()));
    EXPECT_TRUE(result0.isIntersecting);
    EXPECT_DOUBLE_EQ(std::sqrt(3), result0.distance);
    EXPECT_EQ(Point3D(0, 3, 4), result0.point);

    SurfaceRayIntersection3 result1 =
            sset.closestIntersection(Ray3D(Point3D(1, 2.5, 6), Vector3D(-1, -1, 1).normalized()));
    EXPECT_TRUE(result1.isIntersecting);
    EXPECT_DOUBLE_EQ(std::sqrt(0.75), result1.distance);
    EXPECT_EQ(Point3D(0.5, 2, 6.5), result1.point);

    SurfaceRayIntersection3 result2 =
            sset.closestIntersection(Ray3D(Point3D(1, 1, 2), Vector3D(-1, -1, -1).normalized()));
...
TEST(ImplicitSurfaceSet3, BoundingBox) {
    ...
    EXPECT_DOUBLE_EQ(0.0, bbox.lower_corner.x);
    EXPECT_DOUBLE_EQ(-3.0, bbox.lower_corner.y);
    EXPECT_DOUBLE_EQ(-1.0, bbox.lower_corner.z);
    EXPECT_DOUBLE_EQ(5.0, bbox.upper_corner.x);
    EXPECT_DOUBLE_EQ(6.0, bbox.upper_corner.y);
    EXPECT_DOUBLE_EQ(9.0, bbox.upper_corner.z);
}

TEST(ImplicitSurfaceSet3, SignedDistance) {
    BoundingBox3D bbox(Point3D(1, 4, 3), Point3D(5, 6, 9));
