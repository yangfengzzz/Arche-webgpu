...
TEST(VolumeGridEmitter2, Velocity) {
    auto sphere = Sphere2::builder().withCenter({0.5, 0.75}).withRadius(0.15).makeShared();

    auto emitter = VolumeGridEmitter2::builder().withSourceRegion(sphere).makeShared();
    ...
    auto mapper = [](double sdf, const Point2D& pt, const Vector2D& oldVal) {
        if (sdf < 0.0) {
