...
TEST(CustomImplicitSurface2, SignedDistance) {
    CustomImplicitSurface2 cis([](const Point2D& pt) { return (pt - Point2D(0.5, 0.5)).length() - 0.25; },
...
TEST(CustomImplicitSurface2, CloseestPoint) {
    auto sphere = Sphere2::builder().withCenter({0.5, 0.45}).withRadius(0.3).makeShared();
    SurfaceToImplicit2 refSurf(sphere);

    CustomImplicitSurface2 cis1([&](const Point2D& pt) { return refSurf.signedDistance(pt); },
                                BoundingBox2D({0, 0}, {1, 1}), 1e-3);
