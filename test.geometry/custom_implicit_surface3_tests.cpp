...
TEST(CustomImplicitSurface3, SignedDistance) {
    CustomImplicitSurface3 cis([](const Point3D& pt) { return (pt - Point3D(0.5, 0.5, 0.5)).length() - 0.25; },
...
TEST(CustomImplicitSurface3, CloseestPoint) {
    auto sphere = Sphere3::builder().withCenter({0.5, 0.45, 0.55}).withRadius(0.3).makeShared();
    SurfaceToImplicit3 refSurf(sphere);

    CustomImplicitSurface3 cis1([&](const Point3D& pt) { return refSurf.signedDistance(pt); },
                                BoundingBox3D({0, 0, 0}, {1, 1, 1}), 1e-3);
