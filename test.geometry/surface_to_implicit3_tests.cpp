...
protected:
    MOCK_CONST_METHOD1(closestPointLocal, Point3D(const Point3D&));
    MOCK_CONST_METHOD0(boundingBoxLocal, BoundingBox3D());
    MOCK_CONST_METHOD1(closestIntersectionLocal, SurfaceRayIntersection3(const Ray3D&));
    MOCK_CONST_METHOD1(closestNormalLocal, Vector3D(const Point3D&));
};
...
TEST(SurfaceToImplicit3, ClosestPoint) {
    ...
    Point3D s2iPoint = s2i.closestPoint(pt);
...
TEST(SurfaceToImplicit3, Intersects) {
    ...
    bool result0 = s2i.intersects(Ray3D(Point3D(1, 4, 5), Vector3D(-1, -1, -1).normalized()));
    EXPECT_TRUE(result0);

    bool result1 = s2i.intersects(Ray3D(Point3D(1, 2.5, 6), Vector3D(-1, -1, 1).normalized()));
    EXPECT_TRUE(result1);

    bool result2 = s2i.intersects(Ray3D(Point3D(1, 1, 2), Vector3D(-1, -1, -1).normalized()));
...
TEST(SurfaceToImplicit3, ClosestIntersection) {
    ...
    SurfaceRayIntersection3 result0 =
            s2i.closestIntersection(Ray3D(Point3D(1, 4, 5), Vector3D(-1, -1, -1).normalized()));
    ...
    SurfaceRayIntersection3 result1 =
            s2i.closestIntersection(Ray3D(Point3D(1, 2.5, 6), Vector3D(-1, -1, 1).normalized()));
    ...
    SurfaceRayIntersection3 result2 =
            s2i.closestIntersection(Ray3D(Point3D(1, 1, 2), Vector3D(-1, -1, -1).normalized()));
...
TEST(SurfaceToImplicit3, BoundingBox) {
    ...
    EXPECT_DOUBLE_EQ(1.0, bbox.upper_corner.x);
    EXPECT_DOUBLE_EQ(2.0, bbox.upper_corner.y);
    EXPECT_DOUBLE_EQ(4.0, bbox.upper_corner.z);
}
...
TEST(SurfaceToImplicit3, IsBounded) {
    Plane3Ptr plane = Plane3::builder().withPoint({0, 0, 0}).withNormal({0, 1, 0}).makeShared();
    SurfaceToImplicit3Ptr s2i = SurfaceToImplicit3::builder().withSurface(plane).makeShared();
    EXPECT_FALSE(s2i->isBounded());
}

TEST(SurfaceToImplicit3, IsValidGeometry) {
    SurfaceSet3Ptr sset = SurfaceSet3::builder().makeShared();
    SurfaceToImplicit3Ptr s2i = SurfaceToImplicit3::builder().withSurface(sset).makeShared();
    EXPECT_FALSE(s2i->isValidGeometry());
}

TEST(SurfaceToImplicit3, IsInside) {
    Plane3Ptr plane =
            Plane3::builder().withPoint({0, 0, 0}).withNormal({0, 1, 0}).withTranslation({0, -1, 0}).makeShared();
    SurfaceToImplicit3Ptr s2i = SurfaceToImplicit3::builder().withSurface(plane).makeShared();
