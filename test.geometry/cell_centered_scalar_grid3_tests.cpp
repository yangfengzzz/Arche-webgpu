...
TEST(CellCenteredScalarGrid3, Constructors) {
    ...
    grid2.forEachDataPointIndex([&](size_t i, size_t j, size_t k) { EXPECT_DOUBLE_EQ(7.0, grid2(i, j, k)); });
    ...
    grid3.forEachDataPointIndex([&](size_t i, size_t j, size_t k) { EXPECT_DOUBLE_EQ(7.0, grid3(i, j, k)); });
}
...
TEST(CellCenteredScalarGrid3, Swap) {
    ...
    grid1.forEachDataPointIndex([&](size_t i, size_t j, size_t k) { EXPECT_DOUBLE_EQ(8.0, grid1(i, j, k)); });
...
TEST(CellCenteredScalarGrid3, Builder) {
    {
        auto grid1 = CellCenteredScalarGrid3::builder().build({3, 8, 5}, {2.0, 3.0, 1.0}, {5.0, 4.0, 7.0}, 8.0);
...
TEST(CellCenteredScalarGrid3, Fill) {
    ...
    auto func = [](const Point3D& x) { return x.sum(); };
    grid.fill(func);

    for (size_t k = 0; k < grid.dataSize().z; ++k) {
        for (size_t j = 0; j < grid.dataSize().y; ++j) {
            for (size_t i = 0; i < grid.dataSize().x; ++i) {
                EXPECT_DOUBLE_EQ(static_cast<double>(i + j + k) + 1.5, grid(i, j, k));
            }
        }
    }
}
...
TEST(CellCenteredScalarGrid3, GradientAtDataPoint) {
    ...
    grid.fill([](const Point3D& x) { return x.x + 2.0 * x.y - 3.0 * x.z; });
...
TEST(CellCenteredScalarGrid3, LaplacianAtAtDataPoint) {
    ...
    grid.fill([](const Point3D& x) { return square(x.x) + 2.0 * square(x.y) - 4.0 * square(x.z); });
