...
TEST(Octree, Nearest) {
    ...
    auto overlapsFunc = [](const Point3D& pt, const BoundingBox3D& bbox) { return bbox.contains(pt); };

    auto distanceFunc = [](const Point3D& a, const Point3D& b) { return a.distanceTo(b); };

    // Single point
    octree.build({Point3D(0.2, 0.7, 0.3)}, BoundingBox3D({0, 0, 0}, {0.9, 0.8, 1}), overlapsFunc, 3);

    EXPECT_EQ(3u, octree.maxDepth());
    EXPECT_VECTOR3_EQ(Point3D(0, 0, 0), octree.boundingBox().lower_corner);
    EXPECT_VECTOR3_EQ(Point3D(1, 1, 1), octree.boundingBox().upper_corner);
...
TEST(Octree, ForEachOverlappingItems) {
    ...
    size_t measured = 0;
    octree.forEachIntersectingItem(testBox, overlapsFunc, [&](const Point3D& pt) {
...
TEST(Octree, RayIntersects) {
    ...
    auto overlapsFunc = [](const BoundingBox3D& a, const BoundingBox3D& bbox) { return bbox.overlaps(a); };
