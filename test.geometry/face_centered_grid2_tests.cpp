...
TEST(FaceCenteredGrid2, Constructors) {
    ...
    grid2.forEachUIndex([&](size_t i, size_t j) { EXPECT_DOUBLE_EQ(5.0, grid2.u(i, j)); });
    grid2.forEachVIndex([&](size_t i, size_t j) { EXPECT_DOUBLE_EQ(6.0, grid2.v(i, j)); });
    ...
    grid3.forEachUIndex([&](size_t i, size_t j) { EXPECT_DOUBLE_EQ(5.0, grid3.u(i, j)); });
    grid3.forEachVIndex([&](size_t i, size_t j) { EXPECT_DOUBLE_EQ(6.0, grid3.v(i, j)); });
}
...
TEST(FaceCenteredGrid2, Fill) {
    ...
    auto func = [](const Point2D& x) { return Vector2D(x.x, x.y); };
    grid.fill(func);
...
TEST(FaceCenteredGrid2, DivergenceAtCellCenter) {
    ...
    grid.fill([](const Point2D& x) { return Vector2D(x.x, x.y); });
...
TEST(FaceCenteredGrid2, CurlAtCellCenter) {
    ...
    grid.fill([](const Point2D& x) { return Vector2D(-x.y, x.x); });
...
TEST(FaceCenteredGrid2, ValueAtCellCenter) {
    FaceCenteredGrid2 grid(5, 8, 2.0, 3.0);
    grid.fill([&](const Point2D& x) { return Vector2D(3.0 * x.y + 1.0, 5.0 * x.x + 7.0); });
...
TEST(FaceCenteredGrid2, Builder) {
    {
        ...
        auto grid = builder.build(Size2(5, 2), Vector2D(2.0, 4.0), Point2D(-1.0, 2.0), Vector2D(3.0, 5.0));
        EXPECT_EQ(Size2(5, 2), grid->resolution());
        EXPECT_EQ(Vector2D(2.0, 4.0), grid->gridSpacing());
        EXPECT_EQ(Point2D(-1.0, 2.0), grid->origin());
        ...
        faceCenteredGrid->forEachUIndex(
                [&faceCenteredGrid](size_t i, size_t j) { EXPECT_DOUBLE_EQ(3.0, faceCenteredGrid->u(i, j)); });
        faceCenteredGrid->forEachVIndex(
                [&faceCenteredGrid](size_t i, size_t j) { EXPECT_DOUBLE_EQ(5.0, faceCenteredGrid->v(i, j)); });
    }

    {
        ...
        grid.forEachUIndex([&](size_t i, size_t j) { EXPECT_DOUBLE_EQ(3.0, grid.u(i, j)); });
        grid.forEachVIndex([&](size_t i, size_t j) { EXPECT_DOUBLE_EQ(5.0, grid.v(i, j)); });
    }
}
