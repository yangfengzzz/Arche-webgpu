...
#include "vox.geometry/array_samplers1.h"
...
TEST(NearestArraySampler1, Sample) {
    {
        ...
        NearestArraySampler1<double, double> sampler(grid.constAccessor(), gridSpacing, gridOrigin);
...
TEST(LinearArraySampler1, Sample) {
    {
        Array1<double> grid({1.0, 2.0, 3.0, 4.0});
        double gridSpacing = 1.0, gridOrigin = 0.0;
        LinearArraySampler1<double, double> sampler(grid.constAccessor(), gridSpacing, gridOrigin);
...
TEST(CubicArraySampler1, Sample) {
    Array1<double> grid({1.0, 2.0, 3.0, 4.0});
    double gridSpacing = 1.0, gridOrigin = 0.0;
    CubicArraySampler1<double, double> sampler(grid.constAccessor(), gridSpacing, gridOrigin);
...
TEST(NearestArraySampler2, Sample) {
    {
        ...
        Point2D gridOrigin;
        NearestArraySampler2<double, double> sampler(grid.constAccessor(), gridSpacing, gridOrigin);

        double s0 = sampler(Point2D(0.45, 0.45));
        EXPECT_LT(std::fabs(s0 - 1.0), 1e-9);

        double s1 = sampler(Point2D(1.57, 4.01));
        EXPECT_LT(std::fabs(s1 - 7.0), 1e-9);

        double s2 = sampler(Point2D(3.50, 1.21));
    ...
    }
    {
        Array2<double> grid({{1.0, 2.0, 3.0, 4.0},
                             ...
                             {4.0, 5.0, 6.0, 7.0},
                             {5.0, 6.0, 7.0, 8.0}});
        Vector2D gridSpacing(0.5, 0.25);
        Point2D gridOrigin(-1.0, -0.5);
        NearestArraySampler2<double, double> sampler(grid.constAccessor(), gridSpacing, gridOrigin);

        double s0 = sampler(Point2D(0.45, 0.4));
        EXPECT_LT(std::fabs(s0 - 8.0), 1e-9);

        double s1 = sampler(Point2D(-0.05, 0.37));
...
TEST(LinearArraySampler2, Sample) {
    {
        ...
        Vector2D gridSpacing(1.0, 1.0);
        Point2D gridOrigin;
        LinearArraySampler2<double, double> sampler(grid.constAccessor(), gridSpacing, gridOrigin);

        double s0 = sampler(Point2D(0.5, 0.5));
        EXPECT_LT(std::fabs(s0 - 2.0), 1e-9);

        double s1 = sampler(Point2D(1.5, 4.0));
    ...
    }
    {
        ...
        double s1 = sampler(Point2D(-0.5, 0.375));
...
TEST(CubicArraySampler2, Sample) {
    ...
    Vector2D gridSpacing(1.0, 1.0);
    Point2D gridOrigin;
    CubicArraySampler2<double, double> sampler(grid.constAccessor(), gridSpacing, gridOrigin);

    double s0 = sampler(Point2D(1.5, 2.8));
    EXPECT_LT(4.0, s0);
    EXPECT_GT(6.0, s0);
}
...
TEST(CubicArraySampler3, Sample) {
    ...
    Point3D gridOrigin;
    CubicArraySampler3<double, double> sampler(grid.constAccessor(), gridSpacing, gridOrigin);

    double s0 = sampler(Point3D(1.5, 1.8, 1.2));
    EXPECT_LT(3.0, s0);
    EXPECT_GT(6.0, s0);
}
