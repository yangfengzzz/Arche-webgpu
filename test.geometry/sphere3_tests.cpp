...
TEST(Sphere3, ClosestIntersection) {
    ...
    auto result2 = sph.closestIntersection(Ray3D({3.0, -10.0, 2.0}, {0.0, -1.0, 0.0}));
...
TEST(Sphere3, BoundingBox) {
    ...
    EXPECT_DOUBLE_EQ(-2.0, bbox.lower_corner.x);
    EXPECT_DOUBLE_EQ(-6.0, bbox.lower_corner.y);
    EXPECT_DOUBLE_EQ(-3.0, bbox.lower_corner.z);
    EXPECT_DOUBLE_EQ(8.0, bbox.upper_corner.x);
    EXPECT_DOUBLE_EQ(4.0, bbox.upper_corner.y);
...
TEST(Sphere3, Builder) {
    Sphere3 sph = Sphere3::builder().withCenter({3.0, -1.0, 2.0}).withRadius(5.0).withIsNormalFlipped(true).build();
