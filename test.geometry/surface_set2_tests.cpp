...
TEST(SurfaceSet2, Constructors) {
    ...
    auto sph1 = Sphere2::builder().withRadius(1.0).withCenter({0, 0}).makeShared();
    auto sph2 = Sphere2::builder().withRadius(0.5).withCenter({0, 3}).makeShared();
    auto sph3 = Sphere2::builder().withRadius(0.25).withCenter({-2, 0}).makeShared();
    SurfaceSet2 sset2({sph1, sph2, sph3}, Transform2D(), false);
    EXPECT_EQ(3u, sset2.numberOfSurfaces());
    EXPECT_EQ(sph1->radius, std::dynamic_pointer_cast<Sphere2>(sset2.surfaceAt(0))->radius);
    EXPECT_EQ(sph2->radius, std::dynamic_pointer_cast<Sphere2>(sset2.surfaceAt(1))->radius);
    EXPECT_EQ(sph3->radius, std::dynamic_pointer_cast<Sphere2>(sset2.surfaceAt(2))->radius);
    EXPECT_EQ(Vector2D(), sset2.transform.translation());
    EXPECT_EQ(0.0, sset2.transform.orientation());

    SurfaceSet2 sset3({sph1, sph2, sph3}, Transform2(Vector2D(1, 2), 0.5), false);
    ...
    EXPECT_EQ(sph1->radius, std::dynamic_pointer_cast<Sphere2>(sset4.surfaceAt(0))->radius);
    EXPECT_EQ(sph2->radius, std::dynamic_pointer_cast<Sphere2>(sset4.surfaceAt(1))->radius);
    EXPECT_EQ(sph3->radius, std::dynamic_pointer_cast<Sphere2>(sset4.surfaceAt(2))->radius);
...
TEST(SurfaceSet2, AddSurface) {
    ...
    EXPECT_EQ(sph1->radius, std::dynamic_pointer_cast<Sphere2>(sset1.surfaceAt(0))->radius);
    EXPECT_EQ(sph2->radius, std::dynamic_pointer_cast<Sphere2>(sset1.surfaceAt(1))->radius);
    EXPECT_EQ(sph3->radius, std::dynamic_pointer_cast<Sphere2>(sset1.surfaceAt(2))->radius);
...
TEST(SurfaceSet2, ClosestPoint) {
    ...
    // Test empty set
    Point2D emptyPoint = sset1.closestPoint({1.0, 2.0});
    ...
    for (size_t i = 0; i < numSamples / 2; ++i) {
        auto sph = Sphere2::builder().withRadius(0.01).withCenter(getSamplePoints2()[i]).makeShared();
    ...
    for (size_t i = 0; i < numSamples / 2; ++i) {
        auto sph = Sphere2::builder()
                           .withRadius(0.01)
                           .withCenter({0, 0, 0})
                           .withTranslation(Vector2D(getSamplePoints2()[i].x, getSamplePoints2()[i].y))
                           .makeShared();
...
TEST(SurfaceSet2, MixedBoundTypes) {
    BoundingBox2D domain(Point2D(), Point2D(1, 2));

    auto plane = Plane2::builder().withNormal({0, 1}).withPoint({0.0, 0.25 * domain.height()}).makeShared();

    auto sphere = Sphere2::builder().withCenter(domain.midPoint()).withRadius(0.15 * domain.width()).makeShared();

    auto surfaceSet = SurfaceSet2::builder().withSurfaces({plane, sphere}).makeShared();

    EXPECT_FALSE(surfaceSet->isBounded());

    auto cp = surfaceSet->closestPoint(Point2D(0.5, 0.4));
...
TEST(SurfaceSet2, IsValidGeometry) {
    ...
    auto plane = Plane2::builder().withNormal({0, 1}).withPoint({0, 0.25 * domain.height()}).makeShared();

    auto sphere = Sphere2::builder().withCenter(domain.midPoint()).withRadius(0.15 * domain.width()).makeShared();

    auto surfaceSet2 = SurfaceSet2::builder().withSurfaces({plane, sphere}).makeShared();
...
TEST(SurfaceSet2, IsInside) {
    ...
    auto surfaceSet =
            SurfaceSet2::builder().withSurfaces({plane, sphere}).withTransform(Transform2(offset, 0.0)).makeShared();

    EXPECT_TRUE(surfaceSet->isInside(Point2D(0.5, 0.25) + offset));
    EXPECT_TRUE(surfaceSet->isInside(Point2D(0.5, 1.0) + offset));
    EXPECT_FALSE(surfaceSet->isInside(Point2D(0.5, 1.5) + offset));
}

TEST(SurfaceSet2, UpdateQueryEngine) {
    auto sphere = Sphere2::builder().withCenter({-1.0, 1.0}).withRadius(0.5).makeShared();

    auto surfaceSet =
            SurfaceSet2::builder().withSurfaces({sphere}).withTransform(Transform2D({1.0, 2.0}, 0.0)).makeShared();

    auto bbox1 = surfaceSet->boundingBox();
    EXPECT_BOUNDING_BOX2_EQ(BoundingBox2D({-0.5, 2.5}, {0.5, 3.5}), bbox1);

    surfaceSet->transform = Transform2D({3.0, -4.0}, 0.0);
    surfaceSet->updateQueryEngine();
    auto bbox2 = surfaceSet->boundingBox();
    EXPECT_BOUNDING_BOX2_EQ(BoundingBox2D({1.5, -3.5}, {2.5, -2.5}), bbox2);

    sphere->transform = Transform2D({-6.0, 9.0}, 0.0);
    surfaceSet->updateQueryEngine();
