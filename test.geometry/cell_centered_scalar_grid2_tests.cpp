...
TEST(CellCenteredScalarGrid2, Constructors) {
    ...
    grid2.forEachDataPointIndex([&](size_t i, size_t j) { EXPECT_DOUBLE_EQ(5.0, grid2(i, j)); });
    ...
    grid3.forEachDataPointIndex([&](size_t i, size_t j) { EXPECT_DOUBLE_EQ(5.0, grid3(i, j)); });
}
...
TEST(CellCenteredScalarGrid2, Swap) {
    ...
    grid1.forEachDataPointIndex([&](size_t i, size_t j) { EXPECT_DOUBLE_EQ(4.0, grid1(i, j)); });
...
TEST(CellCenteredScalarGrid2, Builder) {
    {
        auto grid1 = CellCenteredScalarGrid2::builder().build(Size2(3, 8), Vector2D(2.0, 3.0), Point2D(1.0, 5.0), 4.0);
    ...
    {
        auto grid1 = CellCenteredScalarGrid2::builder()
                             .withResolution(3, 8)
                             .withGridSpacing(2, 3)
                             .withOrigin(1, 5)
...
TEST(CellCenteredScalarGrid2, Fill) {
    ...
    auto func = [](const Point2D& x) { return x.sum(); };
    grid.fill(func);
...
TEST(CellCenteredScalarGrid2, GradientAtAtDataPoint) {
    ...
    grid.fill([](const Point2D& x) { return x.x + 2.0 * x.y; });
...
TEST(CellCenteredScalarGrid2, LaplacianAtAtDataPoint) {
    ...
    grid.fill([](const Point2D& x) { return square(x.x) + 2.0 * square(x.y); });
