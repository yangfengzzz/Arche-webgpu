...
TEST(Triangle3, Constructors) {
    ...
    std::array<Point3D, 3> points = {{Point3D(1, 2, 3), Point3D(4, 5, 6), Point3D(7, 8, 9)}};
...
TEST(Triangle3, BasicGetters) {
    Triangle3 tri;
    tri.points = {{Point3D(0, 0, -1), Point3D(1, 0, -1), Point3D(0, 1, -1)}};

    EXPECT_DOUBLE_EQ(0.5, tri.area());

    double b0, b1, b2;
    tri.getBarycentricCoords(Point3D(0.5, 0.5, -1), &b0, &b1, &b2);
...
TEST(Triangle3, SurfaceGetters) {
    ...
    Point3D cp1 = tri.closestPoint({0.4, 0.4, 3.0});
    EXPECT_VECTOR3_EQ(Vector3D(0.4, 0.4, -1), cp1);

    Point3D cp2 = tri.closestPoint({-3.0, -3.0, 0.0});
...
TEST(Triangle3, Builder) {
    ...
    Triangle3 tri = Triangle3::builder().withPoints(points).withNormals(normals).withUvs(uvs).build();
