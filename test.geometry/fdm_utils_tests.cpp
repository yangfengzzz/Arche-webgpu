...
TEST(FdmUtils, ScalarToGradient2) {
    ...
    grid.fill([&](const Point2D& x) { return -5.0 * x.x + 4.0 * x.y; });

    Vector2D grad = gradient2(grid.constDataAccessor(), grid.gridSpacing(), 5, 3);
...
TEST(FdmUtils, VectorToGradient2) {
    ...
    grid.fill([&](const Point2D& x) { return Vector2D(-5.0 * x.x + 4.0 * x.y, 2.0 * x.x - 7.0 * x.y); });

    auto grad = gradient2(grid.constDataAccessor(), grid.gridSpacing(), 5, 3);
...
TEST(FdmUtils, ScalarToGradient3) {
    ...
    grid.fill([&](const Point3D& x) { return -5.0 * x.x + 4.0 * x.y + 2.0 * x.z; });

    Vector3D grad = gradient3(grid.constDataAccessor(), grid.gridSpacing(), 5, 3, 4);
    EXPECT_DOUBLE_EQ(-5.0, grad.x);
    EXPECT_DOUBLE_EQ(4.0, grad.y);
...
TEST(FdmUtils, VectorToGradient3) {
    ...
    grid.fill([&](const Point3D& x) {
        return Vector3D(-5.0 * x.x + 4.0 * x.y + 2.0 * x.z, 2.0 * x.x - 7.0 * x.y, x.y + 3.0 * x.z);
    });

    auto grad = gradient3(grid.constDataAccessor(), grid.gridSpacing(), 5, 3, 4);
    EXPECT_DOUBLE_EQ(-5.0, grad[0].x);
    EXPECT_DOUBLE_EQ(4.0, grad[0].y);
...
TEST(FdmUtils, ScalarToLaplacian2) {
    CellCenteredScalarGrid2 grid(10, 10, 2.0, 3.0, -1.0, 4.0);
    grid.fill([&](const Point2D& x) { return -5.0 * x.x * x.x + 4.0 * x.y * x.y; });

    double lapl = laplacian2(grid.constDataAccessor(), grid.gridSpacing(), 5, 3);
    EXPECT_DOUBLE_EQ(-2.0, lapl);
}

TEST(FdmUtils, VectorToLaplacian2) {
    CellCenteredVectorGrid2 grid(10, 10, 2.0, 3.0, -1.0, 4.0);
    grid.fill([&](const Point2D& x) {
        return Vector2D(-5.0 * x.x * x.x + 4.0 * x.y * x.y, 2.0 * x.x * x.x - 7.0 * x.y * x.y);
    });

    auto lapl = laplacian2(grid.constDataAccessor(), grid.gridSpacing(), 5, 3);
...
TEST(FdmUtils, ScalarToLaplacian3) {
    CellCenteredScalarGrid3 grid(10, 10, 10, 2.0, 3.0, 0.5, -1.0, 4.0, 2.0);
    grid.fill([&](const Point3D& x) { return -5.0 * x.x * x.x + 4.0 * x.y * x.y - 3.0 * x.z * x.z; });

    double lapl = laplacian3(grid.constDataAccessor(), grid.gridSpacing(), 5, 3, 4);
...
TEST(FdmUtils, VectorToLaplacian3) {
    CellCenteredVectorGrid3 grid(10, 10, 10, 2.0, 3.0, 0.5, -1.0, 4.0, 2.0);
    grid.fill([&](const Point3D& x) {
        return Vector3D(-5.0 * x.x * x.x + 4.0 * x.y * x.y + 2.0 * x.z * x.z, 2.0 * x.x * x.x - 7.0 * x.y * x.y,
                        x.y * x.y + 3.0 * x.z * x.z);
    });

    auto lapl = laplacian3(grid.constDataAccessor(), grid.gridSpacing(), 5, 3, 4);
    EXPECT_DOUBLE_EQ(2.0, lapl.x);
    EXPECT_DOUBLE_EQ(-10.0, lapl.y);
