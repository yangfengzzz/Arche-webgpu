...
struct SoaFloat4x4 {
    ...
    static VOX_INLINE SoaFloat4x4 FromQuaternion(const SoaQuaternion& _q) {
        ...
        const SoaFloat4x4 ret = {{{one - two * (yy + zz), two * (xy + zw), two * (xz - yw), zero},
    ...
    static VOX_INLINE SoaFloat4x4 FromAffine(const SoaFloat3& _translation,
        ...
        const SoaFloat4x4 ret = {
                {{_scale.x * (one - two * (yy + zz)), _scale.x * two * (xy + zw), _scale.x * two * (xz - yw), zero},
                 {_scale.y * two * (xy - zw), _scale.y * (one - two * (xx + zz)), _scale.y * two * (yz + xw), zero},
                 {_scale.z * two * (xz + yw), _scale.z * two * (yz - xw), _scale.z * (one - two * (xx + yy)), zero},
...
VOX_INLINE SoaFloat4x4 Transpose(const SoaFloat4x4& _m) {
    const SoaFloat4x4 ret = {{{_m.cols[0].x, _m.cols[1].x, _m.cols[2].x, _m.cols[3].x},
...
VOX_INLINE SoaFloat4x4 Invert(const SoaFloat4x4& _m, SimdInt4* _invertible = nullptr) {
    ...
    const SimdFloat4 det = cols[0].x * b0x + cols[0].y * b1x + cols[0].z * b2x + cols[0].w * b3x;
    ...
    const SimdFloat4 inv_det = Select(invertible, RcpEstNR(det), simd_float4::zero());

    const SoaFloat4x4 ret = {{{b0x * inv_det, b0y * inv_det, b0z * inv_det, b0w * inv_det},
...
VOX_INLINE SoaFloat4x4 Scale(const SoaFloat4x4& _m, const SoaFloat4& _v) {
    const SoaFloat4x4 ret = {{{_m.cols[0].x * _v.x, _m.cols[0].y * _v.x, _m.cols[0].z * _v.x, _m.cols[0].w * _v.x},
