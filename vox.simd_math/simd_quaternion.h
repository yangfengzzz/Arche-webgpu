...
struct SimdQuaternion {
    ...
    static VOX_INLINE SimdQuaternion identity() {
        const SimdQuaternion quat = {simd_float4::w_axis()};
        return quat;
    }

    // the angle in radian.
    static VOX_INLINE SimdQuaternion FromAxisAngle(_SimdFloat4 _axis, _SimdFloat4 _angle);
    ...
    static VOX_INLINE SimdQuaternion FromAxisCosAngle(_SimdFloat4 _axis, _SimdFloat4 _cos);
    ...
    static VOX_INLINE SimdQuaternion FromVectors(_SimdFloat4 _from, _SimdFloat4 _to);

    // Returns the quaternion that will rotate vector _from into vector _to,
    // around their plan perpendicular axis. The input vectors must be normalized.
    static VOX_INLINE SimdQuaternion FromUnitVectors(_SimdFloat4 _from, _SimdFloat4 _to);
...
VOX_INLINE SimdQuaternion operator*(const SimdQuaternion& _a, const SimdQuaternion& _b) {
    ...
    const SimdFloat4 p1 = Swizzle<3, 3, 3, 2>(_a.xyzw) * Swizzle<0, 1, 2, 2>(_b.xyzw);
    const SimdFloat4 p2 = Swizzle<0, 1, 2, 0>(_a.xyzw) * Swizzle<3, 3, 3, 0>(_b.xyzw);
    const SimdFloat4 p13 = MAdd(Swizzle<1, 2, 0, 1>(_a.xyzw), Swizzle<2, 0, 1, 1>(_b.xyzw), p1);
    const SimdFloat4 p24 = NMAdd(Swizzle<2, 0, 1, 3>(_a.xyzw), Swizzle<1, 2, 0, 3>(_b.xyzw), p2);
...
VOX_INLINE SimdQuaternion Conjugate(const SimdQuaternion& _q) {
...
VOX_INLINE SimdQuaternion operator-(const SimdQuaternion& _q) {
    const SimdQuaternion quat = {Xor(_q.xyzw, simd_int4::mask_sign())};
    return quat;
}

// Returns the normalized quaternion _q.
VOX_INLINE SimdQuaternion Normalize(const SimdQuaternion& _q) {
...
VOX_INLINE SimdQuaternion NormalizeSafe(const SimdQuaternion& _q, const SimdQuaternion& _safer) {
    const SimdQuaternion quat = {NormalizeSafe4(_q.xyzw, _safer.xyzw)};
    return quat;
}

// Returns the estimated normalized quaternion _q.
VOX_INLINE SimdQuaternion NormalizeEst(const SimdQuaternion& _q) {
...
// Otherwise, returns _safer.
VOX_INLINE SimdQuaternion NormalizeSafeEst(const SimdQuaternion& _q, const SimdQuaternion& _safer) {
...
// set to 0.
VOX_INLINE SimdInt4 IsNormalized(const SimdQuaternion& _q) { return IsNormalized4(_q.xyzw); }
...
// Returns the result in the x component of the returned vector. y, z and w are
// set to 0.
VOX_INLINE SimdInt4 IsNormalizedEst(const SimdQuaternion& _q) { return IsNormalizedEst4(_q.xyzw); }

VOX_INLINE SimdQuaternion SimdQuaternion::FromAxisAngle(_SimdFloat4 _axis, _SimdFloat4 _angle) {
...
VOX_INLINE SimdQuaternion SimdQuaternion::FromAxisCosAngle(_SimdFloat4 _axis, _SimdFloat4 _cos) {
    const SimdFloat4 one = simd_float4::one();
    const SimdFloat4 half = simd_float4::Load1(.5f);

    assert(AreAllTrue1(IsNormalizedEst3(_axis)) && "axis is not normalized.");
    assert(AreAllTrue1(And(CmpGe(_cos, -one), CmpLe(_cos, one))) && "cos is not in [-1,1] range.");
...
VOX_INLINE SimdFloat4 ToAxisAngle(const SimdQuaternion& _q) {
    ...
    return Select(low, x_axis, SetW(_q.xyzw * RcpEstNR(s), half_angle + half_angle));
}

VOX_INLINE SimdQuaternion SimdQuaternion::FromVectors(_SimdFloat4 _from, _SimdFloat4 _to) {
    // http://lolengine.net/blog/2014/02/24/quaternion-from-two-vectors-final
    const SimdFloat4 norm_from_norm_to = SqrtX(Length3Sqr(_from) * Length3Sqr(_to));
...
VOX_INLINE SimdFloat4 TransformVector(const SimdQuaternion& _q, _SimdFloat4 _v) {
