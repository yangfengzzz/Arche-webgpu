...
TEST(Octree, Nearest) {
    ...
    // Single point
    octree.build({Point3D(0.2, 0.7, 0.3)},
                 BoundingBox3D({0, 0, 0}, {0.9, 0.8, 1}), overlapsFunc, 3);
    
    EXPECT_EQ(3u, octree.maxDepth());
    EXPECT_VECTOR3_EQ(Point3D(0, 0, 0), octree.boundingBox().lowerCorner);
    EXPECT_VECTOR3_EQ(Point3D(1, 1, 1), octree.boundingBox().upperCorner);
