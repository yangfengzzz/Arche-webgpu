...
wgpu::TextureView EditorResources::getTexture(const std::string& p_id) {
    if (_textures.find(p_id) != _textures.end()) return _textures.at(p_id).CreateView();
...
wgpu::Texture EditorResources::_createFromPixelBuffer(const std::vector<uint64_t>& data, uint8_t width) {
    ...
    _device.GetQueue().WriteTexture(&imageCopyTexture, data.data(), sizeof(uint64_t) * data.size(), &textureLayout,
                                    &copySize);
