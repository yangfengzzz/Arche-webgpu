...
static inline int GetKeyIndexForF() { return KeyTester_ImGuiKey_F::Get<ImGuiKey_>(nullptr); }

static inline int GetKeyIndexForD() { return KeyTester_ImGuiKey_D::Get<ImGuiKey_>(nullptr); }
...
static void ImDrawListSplitter_Grow(ImDrawList *draw_list, ImDrawListSplitter *splitter, int channels_count) {
    ...
    if (old_channels_count < channels_count) splitter->_Channels.resize(channels_count);
...
static void ImDrawList_PathBezierOffset(
        ImDrawList *drawList, float offset, const ImVec2 &p0, const ImVec2 &p1, const ImVec2 &p2, const ImVec2 &p3) {
    using namespace ed;
...
static void ImDrawList_AddBezierWithArrows(ImDrawList *drawList,
    ...
    if (fill) {
        ...
        if (startArrowSize > 0.0f) {
            ...
            if (half_width > half_thickness) drawList->PathLineTo(curve.P0 - start_n * half_width);
            drawList->PathLineTo(tip);
            if (half_width > half_thickness) drawList->PathLineTo(curve.P0 + start_n * half_width);
        }
        ...
        if (endArrowSize > 0.0f) {
            ...
            if (half_width > half_thickness) drawList->PathLineTo(curve.P3 + end_n * half_width);
            drawList->PathLineTo(tip);
            if (half_width > half_thickness) drawList->PathLineTo(curve.P3 - end_n * half_width);
        }
    ...
    if (flags == Detail::Object::None) {
        ...
        if (IsGroup(this)) {
            ...
            if (m_GroupBorderWidth > 0.0f) {
                ...
                drawList->AddRect(m_GroupBounds.Min, m_GroupBounds.Max, m_GroupBorderColor, m_GroupRounding, 15,
                                  m_GroupBorderWidth);
            }
        }
...
void ed::Node::DrawBorder(ImDrawList *drawList, ImU32 color, float thickness) {
    if (thickness > 0.0f) {
        drawList->AddRect(m_Bounds.Min, m_Bounds.Max, color, m_Rounding, 15, thickness);
    }
}
...
void ed::Node::GetGroupedNodes(std::vector<Node *> &result, bool append) {
    ...
    for (auto index = firstNodeIndex; index < result.size(); ++index) result[index]->GetGroupedNodes(result, true);
}
...
ImRect ed::Node::GetRegionBounds(NodeRegion region) const {
    if (m_Type == NodeType::Node) {
        ...
        if (bounds.GetWidth() < minimumSize) bounds.Expand(ImVec2(minimumSize - bounds.GetWidth(), 0.0f));
        if (bounds.GetHeight() < minimumSize) bounds.Expand(ImVec2(0.0f, minimumSize - bounds.GetHeight()));
...
void ed::Link::Draw(ImDrawList *drawList, ImU32 color, float extraThickness) const {
    ...
    ImDrawList_AddBezierWithArrows(
            ...
            m_EndPin && m_EndPin->m_ArrowWidth > 0.0f ? m_EndPin->m_ArrowWidth + extraThickness : 0.0f, true, color,
            1.0f);
}
...
ImCubicBezierPoints ed::Link::GetCurve() const {
    auto easeLinkStrength = [](const ImVec2 &a, const ImVec2 &b, float strength) {
        ...
        if (halfDistance < strength) strength = strength * ImSin(IM_PI * 0.5f * halfDistance / strength);
...
bool ed::Link::TestHit(const ImRect &rect, bool allowIntersect) const {
    ...
    if (ImCubicBezierLineIntersect(bezier.P0, bezier.P1, bezier.P2, bezier.P3, p0, p1).Count > 0) return true;
    if (ImCubicBezierLineIntersect(bezier.P0, bezier.P1, bezier.P2, bezier.P3, p1, p2).Count > 0) return true;
    if (ImCubicBezierLineIntersect(bezier.P0, bezier.P1, bezier.P2, bezier.P3, p2, p3).Count > 0) return true;
    if (ImCubicBezierLineIntersect(bezier.P0, bezier.P1, bezier.P2, bezier.P3, p3, p0).Count > 0) return true;
...
void ed::EditorContext::Begin(const char *id, const ImVec2 &size) {
    ...
    if (m_CurrentAction && m_CurrentAction->IsDragging() && m_NavigateAction.MoveOverEdge()) {
        ...
        for (int i = 0; i < 5; ++i) io.MouseClickedPos[i] = io.MouseClickedPos[i] - offset;
    } else
...
void ed::EditorContext::End() {
    ...
    // Highlight selected objects
    {
        ...
        for (auto selectedObject : *selectedObjects)
            if (selectedObject->IsVisible()) selectedObject->Draw(drawList, Object::Selected);
    }
    ...
    if (m_CurrentAction && !m_CurrentAction->Process(control)) m_CurrentAction = nullptr;
    ...
    // node drawing order.
    {
        ...
        auto liveNodeCount = static_cast<int>(
                std::count_if(m_Nodes.begin(), m_Nodes.end(), [](Node *node) { return node->m_IsLive; }));
        ...
        ImDrawList_ChannelsGrow(drawList,
                                drawList->_Splitter._Count + c_ChannelsPerNode * liveNodeCount + c_LinkChannelCount);
    ...
    // Draw border
    {
        ...
        drawList->AddRect(m_Canvas.Rect().Min + ImVec2(1, 1), m_Canvas.Rect().Max - ImVec2(1, 1),
...
bool ed::EditorContext::DoLink(LinkId id, PinId startPinId, PinId endPinId, ImU32 color, float thickness) {
    ...
    if (!startPin || !startPin->m_IsLive || !endPin || !endPin->m_IsLive) return false;
...
void ed::EditorContext::MarkNodeToRestoreState(Node *node) { node->m_RestoreState = true; }
...
void ed::EditorContext::SelectObject(Object *object) { m_SelectedObjects.push_back(object); }
...
const ed::vector<ed::Object *> &ed::EditorContext::GetSelectedObjects() { return m_SelectedObjects; }
...
bool ed::EditorContext::HasSelectionChanged() { return m_LastSelectedObjects != m_SelectedObjects; }
...
void ed::EditorContext::FindNodesInRect(const ImRect &r,
                                        vector<Node *> &result,
                                        bool append,
...
void ed::EditorContext::FindLinksForNode(NodeId nodeId, vector<Link *> &result, bool add) {
    ...
    for (auto link : m_Links) {
        ...
        if (link->m_StartPin->m_Node->m_ID == nodeId || link->m_EndPin->m_Node->m_ID == nodeId) result.push_back(link);
    }
}
...
ed::Node *ed::EditorContext::FindNode(NodeId id) { return FindItemInLinear(m_Nodes, id); }

ed::Pin *ed::EditorContext::FindPin(PinId id) { return FindItemIn(m_Pins, id); }

ed::Link *ed::EditorContext::FindLink(LinkId id) { return FindItemIn(m_Links, id); }
...
void ed::EditorContext::SaveSettings() {
    ...
    for (auto &object : m_SelectedObjects) m_Settings.m_Selection.push_back(object->ID());

    m_Settings.m_ViewScroll = m_NavigateAction.m_Scroll;
    m_Settings.m_ViewZoom = m_NavigateAction.m_Zoom;

    if (m_Config.Save(m_Settings.Serialize(), m_Settings.m_DirtyReason)) m_Settings.ClearDirty();

    m_Config.EndSave();
}

void ed::EditorContext::MakeDirty(SaveReasonFlags reason) { m_Settings.MakeDirty(reason); }

void ed::EditorContext::MakeDirty(SaveReasonFlags reason, Node *node) { m_Settings.MakeDirty(reason, node); }
...
ImU32 ed::EditorContext::GetColor(StyleColor colorIndex) const { return ImColor(m_Style.Colors[colorIndex]); }
...
void ed::EditorContext::RegisterAnimation(Animation *animation) { m_LiveAnimations.push_back(animation); }
...
void ed::EditorContext::Flow(Link *link) { m_FlowAnimationController.Flow(link); }
...
void ed::EditorContext::EnableShortcuts(bool enable) { m_ShortcutsEnabled = enable; }
...
ed::Control ed::EditorContext::BuildControl(bool allowOffscreen) {
    ...
    auto checkInteractionsInArea = [&emitInteractiveArea, &hotObject, &activeObject, &clickedObject,
        ...
        if (!hotObject && ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenBlockedByActiveItem)) hotObject = object;
    ...
    auto backgroundDoubleClicked =
            !doubleClickedObject && ImGui::IsItemHovered() ? ImGui::IsMouseDoubleClicked(0) : false;
    ...
    if (!isDragging && isBackgroundActive && hotLink && !m_LastActiveLink) m_LastActiveLink = hotLink;
    ...
    return Control(hotObject, activeObject, clickedObject, doubleClickedObject, isBackgroundHot, isBackgroundActive,
                   backgroundClicked, backgroundDoubleClicked);
}
...
bool ed::NodeSettings::Parse(const json::value &data, NodeSettings &result) {
    ...
    if (data.contains("group_size") && !tryParseVector(data["group_size"], result.m_GroupSize)) return false;
...
bool ed::Settings::Parse(const std::string &string, Settings &settings) {
    ...
    if (selectionValue.is_array()) {
        ...
        for (auto &selection : selectionArray) {
            if (selection.is_string()) result.m_Selection.push_back(deserializeObjectId(selection.get<json::string>()));
        }
    }
    ...
    if (viewValue.is_object()) {
        ...
        if (!tryParseVector(viewScrollValue, result.m_ViewScroll)) result.m_ViewScroll = ImVec2(0, 0);
...
void ed::NavigateAnimation::OnStop() { Editor->MakeDirty(SaveReasonFlags::Navigation); }
...
bool ed::FlowAnimation::IsLinkValid() const { return m_Link && m_Link->m_IsLive; }
...
void ed::FlowAnimationController::Release(FlowAnimation *animation) { IM_UNUSED(animation); }
...
//
//------------------------------------------------------------------------------
const float ed::NavigateAction::s_ZoomLevels[] = {0.1f,  0.15f, 0.20f, 0.25f, 0.33f, 0.5f, 0.75f, 1.0f, 1.25f,
...
ed::EditorAction::AcceptResult ed::NavigateAction::Accept(const Control &control) {
    ...
    if (ImGui::IsWindowFocused() && ImGui::IsKeyPressed(GetKeyIndexForF()) && Editor->AreShortcutsEnabled()) {
        ...
        if (navigateToContent) NavigateTo(Editor->GetContentBounds(), true, -1.0f, NavigationReason::Content);
    }
...
bool ed::NavigateAction::Process(const Control &control) {
    ...
    if (ImGui::IsMouseDragging(c_ScrollButtonIndex, 0.0f)) {
        ...
        if (m_Scroll != m_ScrollStart) Editor->MakeDirty(SaveReasonFlags::Navigation);
...
ImGuiEx::CanvasView ed::NavigateAction::GetView() const { return ImGuiEx::CanvasView(-m_Scroll, m_Zoom); }
...
ImRect ed::NavigateAction::GetViewRect() const { return m_Canvas.CalcViewRect(GetView()); }
...
ed::NodeRegion ed::SizeAction::GetRegion(Node *node) { return node->GetRegion(ImGui::GetMousePos()); }
    ...
    if (control.ActiveObject && ImGui::IsMouseDragging(0)) {
        ...
        if (!io.KeyShift) {
            ...
            for (auto object : m_Objects)
                if (auto node = object->AsNode()) node->GetGroupedNodes(groupedNodes, true);
            ...
            for (auto candidate : groupedNodes)
                if (!isAlreadyPicked(candidate) && candidate->AcceptDrag()) m_Objects.push_back(candidate);
        }
...
ed::EditorAction::AcceptResult ed::ShortcutAction::Accept(const Control &control) {
    ...
    if (io.KeyCtrl && !io.KeyShift && !io.KeyAlt && ImGui::IsKeyPressed(GetKeyIndexForD())) candidateAction = Duplicate;
    ...
    if (candidateAction != None) {
        if (candidateAction != Paste && candidateAction != CreateNode) {
            ...
            if (!hasAnyLinks && m_Context.size() > 1)  // one node cannot make connection to anything
            {
                ...
                for (auto node : nodes) Editor->FindLinksForNode(node.m_ID, links, true);
...
void ed::CreateItemAction::End() {
    ...
    if (m_IsInGlobalSpace) {
        ...
        if (currentChannel != m_LastChannel) ImGui::GetWindowDrawList()->ChannelsSetCurrent(m_LastChannel);
...
ed::CreateItemAction::Result ed::CreateItemAction::RejectItem() {
    ...
    if (!m_InActive || m_CurrentStage == None || m_ItemType == NoItem) return Indeterminate;
...
ed::CreateItemAction::Result ed::CreateItemAction::QueryLink(PinId *startId, PinId *endId) {
    ...
    if (!m_InActive || m_CurrentStage == None || m_ItemType != Link) return Indeterminate;
...
ed::CreateItemAction::Result ed::CreateItemAction::QueryNode(PinId *pinId) {
    ...
    if (!m_InActive || m_CurrentStage == None || m_ItemType != Node) return Indeterminate;
...
void ed::NodeBuilder::End() {
    ...
    if (m_IsGroup) {
        ...
        for (auto pin = m_CurrentNode->m_LastPin; pin; pin = pin->m_PreviousPin) pin->Reset();
...
void ed::NodeBuilder::EndPin() {
    ...
    // ImGui::GetWindowDrawList()->AddRect(m_CurrentPin->m_Bounds.Min, m_CurrentPin->m_Bounds.Max, IM_COL32(255, 255, 0,
    // 255));

    // #debug: Draw pin pivot rectangle
    // ImGui::GetWindowDrawList()->AddRect(m_CurrentPin->m_Pivot.Min, m_CurrentPin->m_Pivot.Max, IM_COL32(255, 0, 255,
    // 255));
...
ImDrawList *ed::NodeBuilder::GetUserBackgroundDrawList() const { return GetUserBackgroundDrawList(m_CurrentNode); }
...
ed::Config::Config(const NodeEditor::Config *config) {
    if (config) *static_cast<NodeEditor::Config *>(this) = *config;
}
...
bool ed::Config::SaveNode(NodeId nodeId, const std::string &data, SaveReasonFlags flags) {
    if (SaveNodeSettings) return SaveNodeSettings(nodeId, data.c_str(), data.size(), flags, UserPointer);
