...
//------------------------------------------------------------------------------
inline bool ImRect_IsEmpty(const ImRect &rect) { return rect.Min.x >= rect.Max.x || rect.Min.y >= rect.Max.y; }

inline ImVec2 ImRect_ClosestPoint(const ImRect &rect, const ImVec2 &p, bool snap_to_edge) {
    if (!snap_to_edge && rect.Contains(p)) return p;

    return ImVec2((p.x > rect.Max.x) ? rect.Max.x : (p.x < rect.Min.x ? rect.Min.x : p.x),
...
inline ImLine ImRect_ClosestLine(const ImRect &rect_a, const ImRect &rect_b, float radius_a, float radius_b) {
    ...
    if (radius_a < 0) radius_a = 0;
    if (radius_b < 0) radius_b = 0;

    if (radius_a == 0 && radius_b == 0) return line;
