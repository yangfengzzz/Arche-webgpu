...
inline float ImCubicBezierLength(const T &p0, const T &p1, const T &p2, const T &p3) {
    // Legendre-Gauss abscissae with n=24 (x_i values, defined at i=n as the roots of the nth order Legendre polynomial
    // Pn(x))
...
inline ImProjectResult ImProjectOnCubicBezier(const ImVec2 &point,
                                              const ImVec2 &p0,
                                              const ImVec2 &p1,
                                              const ImVec2 &p2,
                                              const ImVec2 &p3,
...
inline ImCubicBezierIntersectResult ImCubicBezierLineIntersect(
        const ImVec2 &p0, const ImVec2 &p1, const ImVec2 &p2, const ImVec2 &p3, const ImVec2 &a0, const ImVec2 &a1) {
    ...
    for (int i = 0; i < rootCount; ++i) {
        ...
        if (0 <= root && root <= 1) {
            ...
            if (a0.x == a1.x) {
                if (min.y <= p.y && p.y <= max.y) *points++ = p;
            } else if (a0.y == a1.y) {
                if (min.x <= p.x && p.x <= max.x) *points++ = p;
...
inline void ImCubicBezierSubdivide(ImCubicBezierSubdivideCallback callback,
    ...
    if (tess_tol < 0) tess_tol = 1.118f;  // sqrtf(1.25f)
...
template <typename F>
inline void ImCubicBezierSubdivide(F &callback,
                                   const ImVec2 &p0,
                                   const ImVec2 &p1,
                                   const ImVec2 &p2,
                                   const ImVec2 &p3,
...
inline void ImCubicBezierFixedStep(ImCubicBezierFixedStepCallback callback,
                                   ...
                                   float max_t_error) {
    if (step <= 0.0f || !callback || max_value_error <= 0 || max_t_error <= 0) return;
