...
value &value::operator[](size_t index) {
    if (is_null()) m_Type = construct(m_Storage, type_t::array);

    if (is_array()) {
        auto &v = *array_ptr(m_Storage);
        if (index >= v.size()) v.insert(v.end(), index - v.size() + 1, value());
...
const value &value::operator[](size_t index) const {
    if (is_array()) return (*array_ptr(m_Storage))[index];
...
value &value::operator[](const string &key) {
    if (is_null()) m_Type = construct(m_Storage, type_t::object);

    if (is_object()) return (*object_ptr(m_Storage))[key];
    ...
    switch (m_Type) {
        ...
        case type_t::object:
            context.out << '{';
            {
                ...
                if (!first) context.write_newline();
            }
            context.write_indent(level);
            context.out << '}';
            break;
...
struct value::parser {
    ...
    value parse() {
        ...
        if (!accept_element(v) || !eof()) v = value(type_t::discarded);

        if (previous_locale && strcmp(previous_locale, "C") != 0) std::setlocale(LC_NUMERIC, previous_locale);
...
private:
    struct cursor_state {
        ...
        void reset() { m_Owner->m_Cursor = m_LastCursor; }
    ...
    cursor_state state() { return cursor_state(this); }
    ...
    bool accept_members(object &o) {
        if (!accept_member(o)) return false;

        while (true) {
            auto s = state();
            if (!s(accept(',') && accept_member(o))) break;
        }
    ...
    bool accept_elements(array &a) {
        value v;
        if (!accept_element(v)) return false;

        a.emplace_back(std::move(v));
        while (true) {
            auto s = state();
            v = nullptr;
            if (!s(accept(',') && accept_element(v))) break;
            a.emplace_back(std::move(v));
        }
    ...
    bool accept_hex(string &result) {
        if (accept_digit(result)) return true;
    ...
    bool accept_number(value &result) {
        ...
        if (s(accept_int(n) && accept_frac(n) && accept_exp(n))) {
            ...
            if (end != n.c_str() + n.size()) return false;

            if (v != 0 && !std::isnormal(v)) return false;
    ...
    bool accept_digits(string &result) {
        string part;
        if (!accept_digit(part)) return false;
    ...
    bool accept_ws() {
        while (expect('\x09') || expect('\x0A') || expect('\x0D') || expect('\x20')) advance();
        return true;
    }
