...
AssetProperties::AssetProperties(const std::string &p_title,
    ...
    _targetChanged += [this]() { setTarget(_assetSelector->content); };
...
void AssetProperties::refresh() {
    ...
    _headerSeparator->enabled =
            _applyButton->enabled || _resetButton->enabled || _revertButton->enabled || _previewButton->enabled;
...
void AssetProperties::createTextureSettings() {
    ...
    minFilter.valueChangedEvent += [this](int p_choice) { _metadata->set("MIN_FILTER", p_choice); };
    ...
    magFilter.valueChangedEvent += [this](int p_choice) { _metadata->set("MAG_FILTER", p_choice); };
