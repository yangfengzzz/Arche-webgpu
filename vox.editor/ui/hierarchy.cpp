...
public:
    HierarchyContextualMenu(Entity *p_target, TreeNode &p_treeNode, bool p_panelMenu = false)
        ...
        if (_target) {
            ...
            focusButton.clickedEvent += [this] { EditorActions::getSingleton().moveToTarget(_target); };
            ...
            deleteButton.clickedEvent += [this] { EditorActions::getSingleton().destroyEntity(_target); };
        }
    ...
    virtual void execute() override {
        if (_widgets.size() > 0) ContextualMenu::execute();
    }
...
void Hierarchy::unselectEntitiesWidgets() {
    for (auto &widget : _widgetEntityLink) widget.second->selected = false;
}
...
void Hierarchy::addEntityByInstance(Entity *p_entity) {
    ...
    textSelectable.doubleClickedEvent +=
